#!/usr/bin/env python3
"""Conformance checker for BME280 (environmental category). See
specs/testing_framework.md, "Conformance Implementation", and
specs/environmental/bme280_timing.conf for the timing bound checked here.

Checks (see specs/environmental/bme280.md, Timing Constraints):
  - measurement_trigger: from the ctrl_meas write that sets mode=Forced to
    the ADC data burst read (0xF7, 8 bytes) - the driver's per-call forced-
    conversion cycle - bounded by the weather-monitoring preset's max
    measurement time (9.3 ms, osrs_t=x1/osrs_p=x1/osrs_h=x1, the Minimal
    class's baked-in defaults).

Unlike ENS160/AHT21 (which predate the named-annotation convention - see
conformance/gas/ens160_conformance.py's docstring), BME280 is new work:
sigrok/bme280/pd.py was extended with explicit
...
environmental/bme280 - see specs/testing_framework.md, "Chip Wiring &
...
TIMING_CONF = REPO_ROOT / 'specs' / 'environmental' / 'bme280_timing.conf'
DECODER_ID = 'bme280'
...
def build_trigger(args):
    """Return trigger(check_name) -> None: drives one real BME280
    temperature/pressure/humidity read (each triggers a fresh forced-mode
    conversion) so it happens during the capture window. Which mechanism
    ...
    if args.lang == 'python':
        test_file = REPO_ROOT / 'python' / 'tests' / 'environmental' / 'bme280_test_linux.py'
        return lambda name: subprocess.run(
            [sys.executable, str(test_file)], check=False, cwd=str(REPO_ROOT / 'python'))
    if args.lang == 'nodejs':
        test_file = REPO_ROOT / 'nodejs' / 'tests' / 'environmental' / 'bme280_test.js'
...
def main():
    ...
    if not sigrok_driver or not sigrok_channels:
        print("ERROR: SIGROK_DRIVER and SIGROK_CHANNELS must be set "
              "(testconfig_wiring's per-chip case block for environmental/bme280) "
              "to run conformance checks.", file=sys.stderr)
        sys.exit(2)
    sigrok_conn = os.environ.get('SIGROK_CONN') or None

    trigger = build_trigger(args)
    rc = sc.run_checks('BME280', DECODER_ID, sigrok_channels, CHECKS, trigger,
