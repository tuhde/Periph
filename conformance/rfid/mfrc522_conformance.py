#!/usr/bin/env python3
"""Conformance checker for MFRC522 (rfid category). See
specs/testing_framework.md, "Conformance Implementation", and
specs/rfid/mfrc522_timing.conf for the timing bound checked here.

Check (see specs/rfid/mfrc522.md, Timing Constraints):
  - reset_recovery: from the SoftReset command write to the first
    CommandReg read showing PowerDown clear, <= reset_recovery_max_ms.

Unlike ENS160/AHT21 (which predate the named-annotation convention and
key off generic pre-existing decoder text as a documented bridge), this
check uses clean, explicitly-named `reset_recovery_start`/
`reset_recovery_done` annotations added to sigrok/mfrc522/pd.py
specifically for it, per specs/_template_chip.md's convention.
...
rfid/mfrc522 - see specs/testing_framework.md, "Chip Wiring & Sigrok
Configuration").
"""
import argparse
import os
...
TIMING_CONF = REPO_ROOT / 'specs' / 'rfid' / 'mfrc522_timing.conf'
DECODER_ID = 'mfrc522'
...
def _reset_via_serial(port):
    ...
    firmware's test sequence (which re-initializes the chip, issuing a
    fresh SoftReset) runs again inside the capture window - the same
    auto-reset trick cpp/read_serial_zephyr.py already uses."""
    import serial
...
def build_trigger(args):
    """Return trigger(check_name) -> None: drives one real MFRC522
    initialization (SoftReset + recovery poll) so it happens during the
    capture window. Which mechanism this is depends on how the calling
    script invoked us."""
    if args.binary:
    ...
    if args.lang == 'python':
        test_file = REPO_ROOT / 'python' / 'tests' / 'rfid' / 'mfrc522_test_linux.py'
        return lambda name: subprocess.run(
            [sys.executable, str(test_file)], check=False, cwd=str(REPO_ROOT / 'python'))
    if args.lang == 'nodejs':
        test_file = REPO_ROOT / 'nodejs' / 'tests' / 'rfid' / 'mfrc522_test.js'
...
def main():
    ...
    if not sigrok_driver or not sigrok_channels:
        print("ERROR: SIGROK_DRIVER and SIGROK_CHANNELS must be set "
              "(testconfig_wiring's per-chip case block for rfid/mfrc522) "
              "to run conformance checks.", file=sys.stderr)
        sys.exit(2)
    sigrok_conn = os.environ.get('SIGROK_CONN') or None

    trigger = build_trigger(args)
    rc = sc.run_checks('MFRC522', DECODER_ID, sigrok_channels, CHECKS, trigger,
                        str(TIMING_CONF), sigrok_driver, sigrok_conn, protocol='spi')
    sys.exit(rc)
