#!/usr/bin/env python3
"""Conformance checker for 24AA02UID (memory category). See
specs/testing_framework.md, "Conformance Implementation", and
specs/memory/24aa02uid_timing.conf for the timing bound checked here.

Checks (see specs/memory/24aa02uid.md, Timing Constraints):
  - write_cycle: from the STOP that ends a byte/page write to the first
    subsequent bus transaction, <= write_cycle_max_ms. Drivers must not
    issue the next command until ACK polling confirms the internal write
    cycle is complete (see the chip spec's "ACK Polling" section) - a
    driver that starts its next transaction too early, or one whose ACK
    polling takes longer than the chip's rated write-cycle time, both show
    up as this check's delta being out of the expected range.

This is new work (not a retrofit of a pre-existing decoder): unlike
ENS160/AHT21, sigrok/24aa02uid/pd.py emits dedicated "write_cycle_start" /
"write_cycle_done" annotations specifically for this check (see
...
memory/24aa02uid - see specs/testing_framework.md, "Chip Wiring and Sigrok
Configuration").
"""
import argparse
import os
...
TIMING_CONF = REPO_ROOT / 'specs' / 'memory' / '24aa02uid_timing.conf'
DECODER_ID = '24aa02uid'

CHECKS = {
    'write_cycle': (
        lambda t: 'write_cycle_start' in t,
        lambda t: 'write_cycle_done' in t,
    ),
}
...
def build_trigger(args):
    """Return trigger(check_name) -> None: drives one real byte/page write
    (followed by a readback, per the HIL tests' own round-trip flow) so it
    happens during the capture window. Which mechanism this is depends on
    how the calling script invoked us."""
    if args.binary:
    ...
    if args.lang == 'python':
        test_file = REPO_ROOT / 'python' / 'tests' / 'memory' / '24aa02uid_test_linux.py'
        return lambda name: subprocess.run(
            [sys.executable, str(test_file)], check=False, cwd=str(REPO_ROOT / 'python'))
    if args.lang == 'nodejs':
        test_file = REPO_ROOT / 'nodejs' / 'tests' / 'memory' / '24aa02uid_test.js'
...
def main():
    ...
    if not sigrok_driver or not sigrok_channels:
        print("ERROR: SIGROK_DRIVER and SIGROK_CHANNELS must be set "
              "(testconfig_wiring's per-chip case block for memory/24aa02uid) "
              "to run conformance checks.", file=sys.stderr)
        sys.exit(2)
    sigrok_conn = os.environ.get('SIGROK_CONN') or None

    trigger = build_trigger(args)
    rc = sc.run_checks('24AA02UID', DECODER_ID, sigrok_channels, CHECKS, trigger,
