#!/usr/bin/env python3
"""Conformance checker for PCF8575 (io_expander category). See
specs/testing_framework.md, "Conformance Implementation", and
specs/io_expander/pcf8575_timing.conf for the timing bound checked here.

Check (see specs/io_expander/pcf8575.md, Timing Constraints):
  - read_cycle: a plain 2-byte read transaction (ADDRESS READ through the
    terminating STOP) completes within read_cycle_max_us (~45 us at
    400 kHz per the spec). The chip's other timing figures (t_iv/t_ir
    INT-assert/reset latency, ~4 us each) require an INT channel that is
    not captured by default (only SCL/SDA), so they are intentionally not
    checked here.
...
io_expander/pcf8575 - see specs/testing_framework.md, "Chip Wiring and
...
TIMING_CONF = REPO_ROOT / 'specs' / 'io_expander' / 'pcf8575_timing.conf'
DECODER_ID = 'pcf8575'
...
def build_trigger(args):
    """Return trigger(check_name) -> None: drives one real PCF8575 read
    ...
    if args.lang == 'python':
        test_file = REPO_ROOT / 'python' / 'tests' / 'io_expander' / 'pcf8575_test_linux.py'
        return lambda name: subprocess.run(
            [sys.executable, str(test_file)], check=False, cwd=str(REPO_ROOT / 'python'))
    if args.lang == 'nodejs':
        test_file = REPO_ROOT / 'nodejs' / 'tests' / 'io_expander' / 'pcf8575_test.js'
...
def main():
    ...
    if not sigrok_driver or not sigrok_channels:
        print("ERROR: SIGROK_DRIVER and SIGROK_CHANNELS must be set "
              "(testconfig_wiring's per-chip case block for io_expander/pcf8575) "
              "to run conformance checks.", file=sys.stderr)
        sys.exit(2)
    sigrok_conn = os.environ.get('SIGROK_CONN') or None

    trigger = build_trigger(args)
    rc = sc.run_checks('PCF8575', DECODER_ID, sigrok_channels, CHECKS, trigger,
