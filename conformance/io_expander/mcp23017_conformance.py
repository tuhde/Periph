#!/usr/bin/env python3
"""Conformance checker for MCP23017 (io_expander category). See
specs/testing_framework.md, "Conformance Implementation", and
specs/io_expander/mcp23017_timing.conf for the timing bound checked here.

Check (see specs/io_expander/mcp23017.md, Timing Constraints):
  - register_read_cycle: a single-register read (repeated-start ADDRESS
    READ through the terminating STOP) completes within
    register_read_cycle_max_us. The spec quotes ~30-40 us for the full
    address-phase-plus-read transaction; the named annotation pair this
    keys off (sigrok/mcp23017/pd.py's register_read_start/
    register_read_done) marks only the read phase itself, a subset of that
    full transaction, so the bound stays a safe upper bound rather than an
    invented number. Most of MCP23017's other datasheet timing figures
    (IOC-to-INT latency, GP input settling, glitch filter) are sub-bus-cycle
    electrical parameters with no INT channel captured by default — not
    decodable from a SCL/SDA-only capture, so they are intentionally not
    checked here (see specs/_template_chip_io_expander.md's guidance: don't
    invent constraints the datasheet doesn't state a bus-decodable way to
    check).
...
io_expander/mcp23017 - see specs/testing_framework.md, "Chip Wiring and
...
TIMING_CONF = REPO_ROOT / 'specs' / 'io_expander' / 'mcp23017_timing.conf'
DECODER_ID = 'mcp23017'

CHECKS = {
    'register_read_cycle': (
        lambda t: 'register_read_start' in t,
        lambda t: 'register_read_done' in t,
    ),
}
...
def build_trigger(args):
    """Return trigger(check_name) -> None: drives one real MCP23017
    register-read sequence so it happens during the capture window. Which
    ...
    if args.lang == 'python':
        test_file = REPO_ROOT / 'python' / 'tests' / 'io_expander' / 'mcp23017_test_linux.py'
        return lambda name: subprocess.run(
            [sys.executable, str(test_file)], check=False, cwd=str(REPO_ROOT / 'python'))
    if args.lang == 'nodejs':
        test_file = REPO_ROOT / 'nodejs' / 'tests' / 'io_expander' / 'mcp23017_test.js'
...
def main():
    ...
    if not sigrok_driver or not sigrok_channels:
        print("ERROR: SIGROK_DRIVER and SIGROK_CHANNELS must be set "
              "(testconfig_wiring's per-chip case block for io_expander/mcp23017) "
              "to run conformance checks.", file=sys.stderr)
        sys.exit(2)
    sigrok_conn = os.environ.get('SIGROK_CONN') or None

    trigger = build_trigger(args)
    rc = sc.run_checks('MCP23017', DECODER_ID, sigrok_channels, CHECKS, trigger,
