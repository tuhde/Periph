#!/usr/bin/env python3
"""Conformance checker for INA226 (power category). See
specs/testing_framework.md, "Conformance Implementation", and
specs/power/ina226_timing.conf for the timing bounds checked here.

Checks (see specs/power/ina226.md, Timing Constraints):
  - conversion_ready: from a Configuration-register write (clears CVRF) to
    the next Mask/Enable read showing CVRF (bit 3) set, at the default
    configuration (VBUSCT=1.1 ms, VSHCT=1.1 ms) this should land within
    ~2.2 ms.

Keys off sigrok/power/ina226/pd.py's explicitly-named `conversion_ready_start`
/ `conversion_ready_done` annotations (added alongside the decoder's existing
generic reg-write/reg-read annotations) - see specs/_template_chip.md's
Sigrok Decoder checklist item and specs/testing_framework.md, "Conformance
Implementation".
...
power/ina226 - see specs/testing_framework.md, "Chip Wiring & Sigrok
Configuration").
"""
import argparse
import os
...
TIMING_CONF = REPO_ROOT / 'specs' / 'power' / 'ina226_timing.conf'
DECODER_ID = 'ina226'

CHECKS = {
    'conversion_ready': (
        lambda t: 'conversion_ready_start' in t,
        lambda t: 'conversion_ready_done' in t,
    ),
}
...
def build_trigger(args):
    """Return trigger(check_name) -> None: drives one real INA226
    ...
    if args.lang == 'python':
        test_file = REPO_ROOT / 'python' / 'tests' / 'power' / 'ina226_test_linux.py'
        return lambda name: subprocess.run(
            [sys.executable, str(test_file)], check=False, cwd=str(REPO_ROOT / 'python'))
    if args.lang == 'nodejs':
        test_file = REPO_ROOT / 'nodejs' / 'tests' / 'power' / 'ina226_test.js'
...
def main():
    ...
    if not sigrok_driver or not sigrok_channels:
        print("ERROR: SIGROK_DRIVER and SIGROK_CHANNELS must be set "
              "(testconfig_wiring's per-chip case block for power/ina226) "
              "to run conformance checks.", file=sys.stderr)
        sys.exit(2)
    sigrok_conn = os.environ.get('SIGROK_CONN') or None

    trigger = build_trigger(args)
    rc = sc.run_checks('INA226', DECODER_ID, sigrok_channels, CHECKS, trigger,
