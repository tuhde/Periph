#!/usr/bin/env python3
"""Conformance checker for APDS-9960 (light category). See
specs/testing_framework.md, "Conformance Implementation", and
specs/light/apds9960_timing.conf for the timing bound checked here.

Checks (see specs/light/apds9960.md, Timing Constraints):
  - als_integration: from the ENABLE write that sets AEN (ALS/Color engine
    enable) to the next STATUS read showing AVALID set, one complete ALS
    integration cycle (~200 ms at the driver's default ATIME=0xB6).

Unlike ENS160/AHT21 (which predate the named-annotation convention and key
off generic existing annotation text - see conformance/gas/ens160_conformance.py's
docstring), APDS-9960 is new work: sigrok/apds9960/pd.py was extended with
explicit ANN_ALS_INTEGRATION_START / ANN_ALS_INTEGRATION_DONE annotations
(emitted as 'als_integration_start (AEN)' / 'als_integration_done (AVALID)')
specifically for this check, added additively alongside the decoder's
existing register-level annotations per specs/_template_chip.md.
...
light/apds9960 - see specs/testing_framework.md, "Chip Wiring & Sigrok
Configuration").
"""
import argparse
import os
...
TIMING_CONF = REPO_ROOT / 'specs' / 'light' / 'apds9960_timing.conf'
DECODER_ID = 'apds9960'

CHECKS = {
    'als_integration': (
        lambda t: 'als_integration_start' in t,
        lambda t: 'als_integration_done' in t,
    ),
}
...
def build_trigger(args):
    """Return trigger(check_name) -> None: drives one real APDS-9960
    construction (init sequence writes ENABLE with AEN, then waits one
    integration cycle) so it happens during the capture window. Which
    ...
    if args.lang == 'python':
        test_file = REPO_ROOT / 'python' / 'tests' / 'light' / 'apds9960_test_linux.py'
        return lambda name: subprocess.run(
            [sys.executable, str(test_file)], check=False, cwd=str(REPO_ROOT / 'python'))
    if args.lang == 'nodejs':
        test_file = REPO_ROOT / 'nodejs' / 'tests' / 'light' / 'apds9960_test.js'
...
def main():
    ...
    if not sigrok_driver or not sigrok_channels:
        print("ERROR: SIGROK_DRIVER and SIGROK_CHANNELS must be set "
              "(testconfig_wiring's per-chip case block for light/apds9960) "
              "to run conformance checks.", file=sys.stderr)
        sys.exit(2)
    sigrok_conn = os.environ.get('SIGROK_CONN') or None

    trigger = build_trigger(args)
    rc = sc.run_checks('APDS-9960', DECODER_ID, sigrok_channels, CHECKS, trigger,
