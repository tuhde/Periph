#!/usr/bin/env python3
"""Conformance checker for AS5600 (magnetometer category). See
specs/testing_framework.md, "Conformance Implementation", and
specs/magnetometer/as5600_timing.conf for the timing bounds checked here.

Checks (see specs/magnetometer/as5600.md, Timing Constraints):
  - burn_settle: from the BURN command write (Burn_Angle 0x80 or
    Burn_Setting 0x40) to the first byte of the OTP reload/verify sequence
    (0x01 written to BURN), >= burn_settle_min_ms. Note: burn_angle()/
    burn_setting() are destructive (OTP has a limited write budget - see
    the chip spec's Implementation Notes) and the standard HIL tests
    deliberately never call them, so this check only produces a real
    pass/fail once a dedicated conformance trigger that actually exercises
    a burn is wired up; until then it correctly reports "start/end
    annotation pair not found" rather than a false pass.
  - poweron_ready: from power-on to the bus being usable, <=
    poweron_ready_max_ms. No dedicated power-sense channel exists in this
    ...
    (same pattern as conformance/environmental/aht21_conformance.py's
    poweron_ready check).

Unlike ENS160/AHT21 (which predate the named-annotation convention and key
off generic reg-read/-write text), this is new work: sigrok/as5600/pd.py
emits dedicated "burn_command" / "otp_reload_start" annotations
specifically for the burn_settle check (see specs/_template_chip.md).
...
magnetometer/as5600 - see specs/testing_framework.md, "Chip Wiring and
...
TIMING_CONF = REPO_ROOT / 'specs' / 'magnetometer' / 'as5600_timing.conf'
DECODER_ID = 'as5600'

CHECKS = {
    'burn_settle': (
        lambda t: 'burn_command' in t,
        lambda t: 'otp_reload_start' in t,
    ),
    'poweron_ready': (
...
def build_trigger(args):
    ...
    def rerun():
        if args.binary:
            ...
            test_file = REPO_ROOT / 'python' / 'tests' / 'magnetometer' / 'as5600_test_linux.py'
            subprocess.run([sys.executable, str(test_file)], check=False, cwd=str(REPO_ROOT / 'python'))
        elif args.lang == 'nodejs':
            test_file = REPO_ROOT / 'nodejs' / 'tests' / 'magnetometer' / 'as5600_test.js'
            subprocess.run(['node', str(test_file)], check=False)
        else:
            raise SystemExit(f"ERROR: don't know how to trigger a transaction for --lang {args.lang}")

    def trigger(name):
        if name == 'poweron_ready':
            print('=== Power-cycle the AS5600 now (capture is running) ===', file=sys.stderr)
...
def main():
    ...
    if not sigrok_driver or not sigrok_channels:
        print("ERROR: SIGROK_DRIVER and SIGROK_CHANNELS must be set "
              "(testconfig_wiring's per-chip case block for magnetometer/as5600) "
              "to run conformance checks.", file=sys.stderr)
        sys.exit(2)
    sigrok_conn = os.environ.get('SIGROK_CONN') or None

    trigger = build_trigger(args)
    rc = sc.run_checks('AS5600', DECODER_ID, sigrok_channels, CHECKS, trigger,
