#!/usr/bin/env python3
"""Conformance checker for BMP581 (pressure category). See
specs/testing_framework.md, "Conformance Implementation", and
specs/pressure/bmp581_timing.conf for the timing bounds checked here.

Checks (see specs/pressure/bmp581.md, Timing Constraints):
  - power_up: from power-on to STATUS.nvm_rdy=1, <= 2 ms.
  - soft_reset: from CMD=0xB6 write to STATUS.nvm_rdy=1 after reset,
    <= 2 ms.
  - forced_conversion: from set_mode(FORCED) to the burst data read at
    0x1D-0x22, <= 80.4 ms at OSR x128 (worst case per datasheet).

Keys off sigrok/pressure/bmp581/pd.py's named annotations (added alongside
the decoder's existing generic reg-write/data-read annotations).
...
TIMING_CONF = REPO_ROOT / 'specs' / 'pressure' / 'bmp581_timing.conf'
DECODER_ID = 'bmp581'
...
def build_trigger(args):
    ...
    if args.lang == 'python':
        test_file = REPO_ROOT / 'python' / 'tests' / 'pressure' / 'bmp581_test_linux.py'
        return lambda name: subprocess.run(
            [sys.executable, str(test_file)], check=False, cwd=str(REPO_ROOT / 'python'))
    if args.lang == 'nodejs':
        test_file = REPO_ROOT / 'nodejs' / 'tests' / 'pressure' / 'bmp581_test.js'
...
def main():
    ...
    if not sigrok_driver or not sigrok_channels:
        print("ERROR: SIGROK_DRIVER and SIGROK_CHANNELS must be set "
              "(testconfig_wiring's per-chip case block for pressure/bmp581) "
              "to run conformance checks.", file=sys.stderr)
        sys.exit(2)
    sigrok_conn = os.environ.get('SIGROK_CONN') or None

    trigger = build_trigger(args)
    rc = sc.run_checks('BMP581', DECODER_ID, sigrok_channels, CHECKS, trigger,
