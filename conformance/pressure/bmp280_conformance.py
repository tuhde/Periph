#!/usr/bin/env python3
"""Conformance checker for BMP280 (pressure category). See
specs/testing_framework.md, "Conformance Implementation", and
specs/pressure/bmp280_timing.conf for the timing bound checked here.

Checks (see specs/pressure/bmp280.md, Timing Constraints):
  - conversion: from the ctrl_meas write that triggers a forced-mode
    measurement to the 6-byte data burst read (adc_P + adc_T), <= 6.4 ms
    at the Minimal class's default Ultra Low Power preset (osrs_t=x1,
    osrs_p=x1).

Keys off sigrok/pressure/bmp280/pd.py's explicitly-named
`conversion_start`/`conversion_done` annotations (added alongside the
...
pressure/bmp280 - see specs/testing_framework.md, "Chip Wiring & Sigrok
Configuration").
"""
import argparse
import os
...
TIMING_CONF = REPO_ROOT / 'specs' / 'pressure' / 'bmp280_timing.conf'
DECODER_ID = 'bmp280'

CHECKS = {
    'conversion': (
        lambda t: 'conversion_start' in t,
        lambda t: 'conversion_done' in t,
    ),
}
...
def build_trigger(args):
    """Return trigger(check_name) -> None: drives one real BMP280
    ...
    if args.lang == 'python':
        test_file = REPO_ROOT / 'python' / 'tests' / 'pressure' / 'bmp280_test_linux.py'
        return lambda name: subprocess.run(
            [sys.executable, str(test_file)], check=False, cwd=str(REPO_ROOT / 'python'))
    if args.lang == 'nodejs':
        test_file = REPO_ROOT / 'nodejs' / 'tests' / 'pressure' / 'bmp280_test.js'
...
def main():
    ...
    if not sigrok_driver or not sigrok_channels:
        print("ERROR: SIGROK_DRIVER and SIGROK_CHANNELS must be set "
              "(testconfig_wiring's per-chip case block for pressure/bmp280) "
              "to run conformance checks.", file=sys.stderr)
        sys.exit(2)
    sigrok_conn = os.environ.get('SIGROK_CONN') or None

    trigger = build_trigger(args)
    rc = sc.run_checks('BMP280', DECODER_ID, sigrok_channels, CHECKS, trigger,
