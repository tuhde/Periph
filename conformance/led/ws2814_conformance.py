#!/usr/bin/env python3
"""Conformance checker for WS2814 (led category). See
specs/testing_framework.md, "Conformance Implementation", and
specs/led/ws2814_timing.conf for the timing bound checked here.

Check (see specs/led/ws2814.md, Timing Constraints):
  - reset_pulse: the low period after a pixel transmission ends, >=280us
    (longer than both WS2812B's 50us and SK6812RGBW's 80us).
...
different `reset_us` passed to the shared `neopixel` connection decoder
...
led/ws2814 - see specs/testing_framework.md, "Chip Wiring & Sigrok
...
TIMING_CONF = REPO_ROOT / 'specs' / 'led' / 'ws2814_timing.conf'
DECODER_ID = 'ws2814'
RESET_US = 280
...
def build_trigger(args):
    ...
    if args.lang == 'python':
        test_file = REPO_ROOT / 'python' / 'tests' / 'led' / 'ws2814_test_linux.py'
        return lambda name: subprocess.run(
            [sys.executable, str(test_file)], check=False, cwd=str(REPO_ROOT / 'python'))
    if args.lang == 'nodejs':
        test_file = REPO_ROOT / 'nodejs' / 'tests' / 'led' / 'ws2814_test.js'
...
def main():
    ...
    if not sigrok_driver or not sigrok_channels:
        print("ERROR: SIGROK_DRIVER and SIGROK_CHANNELS must be set "
              "(testconfig_wiring's per-chip case block for led/ws2814) "
              "to run conformance checks.", file=sys.stderr)
        sys.exit(2)
    sigrok_conn = os.environ.get('SIGROK_CONN') or None

    trigger = build_trigger(args)
    rc = sc.run_checks('WS2814', DECODER_ID, sigrok_channels, CHECKS, trigger,
