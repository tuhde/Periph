#!/usr/bin/env python3
"""Conformance checker for SK6812RGBW (led category). See
specs/testing_framework.md, "Conformance Implementation", and
specs/led/sk6812rgbw_timing.conf for the timing bound checked here.

Check (see specs/led/sk6812rgbw.md, Timing Constraints):
  - reset_pulse: the low period after a pixel transmission ends, >=80us
    (longer than WS2812B's 50us).

See conformance/led/ws2812b_conformance.py's docstring for the full
rationale - this chip's checker is otherwise identical, just with a
different `reset_us` passed to the shared `neopixel` transport decoder
and its own timing.conf/decoder id.
...
led/sk6812rgbw - see specs/testing_framework.md, "Chip Wiring & Sigrok
...
TIMING_CONF = REPO_ROOT / 'specs' / 'led' / 'sk6812rgbw_timing.conf'
DECODER_ID = 'sk6812rgbw'
RESET_US = 80
...
def build_trigger(args):
    ...
    if args.lang == 'python':
        test_file = REPO_ROOT / 'python' / 'tests' / 'led' / 'sk6812rgbw_test_linux.py'
        return lambda name: subprocess.run(
            [sys.executable, str(test_file)], check=False, cwd=str(REPO_ROOT / 'python'))
    if args.lang == 'nodejs':
        test_file = REPO_ROOT / 'nodejs' / 'tests' / 'led' / 'sk6812rgbw_test.js'
...
def main():
    ...
    if not sigrok_driver or not sigrok_channels:
        print("ERROR: SIGROK_DRIVER and SIGROK_CHANNELS must be set "
              "(testconfig_wiring's per-chip case block for led/sk6812rgbw) "
              "to run conformance checks.", file=sys.stderr)
        sys.exit(2)
    sigrok_conn = os.environ.get('SIGROK_CONN') or None

    trigger = build_trigger(args)
    rc = sc.run_checks('SK6812RGBW', DECODER_ID, sigrok_channels, CHECKS, trigger,
