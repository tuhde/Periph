#!/usr/bin/env python3
"""Conformance checker for VL53L0X (tof category). See
specs/testing_framework.md, "Conformance Implementation", and
specs/tof/vl53l0x_timing.conf for the timing bound checked here.

Checks (see specs/tof/vl53l0x.md, Timing Constraints):
  - single_ranging: from the SYSRANGE_START = 0x01 write on page 0 (outside
    a reference calibration) to the STOP of the result-block read at 0x14
    that consumes it, within single_ranging_min_ms..single_ranging_max_ms at
    the default ~33 ms timing budget. tBOOT is not checked (the XSHUT edge is
    not on the I2C capture).

sigrok/vl53l0x/pd.py emits a dedicated `timing` annotation row for this
...
tof/vl53l0x - see specs/testing_framework.md, "Chip Wiring & Sigrok
...
TIMING_CONF = REPO_ROOT / 'specs' / 'tof' / 'vl53l0x_timing.conf'
DECODER_ID = 'vl53l0x'
    ...
    def rerun():
        if args.binary:
            ...
            subprocess.run([sys.executable,
                             str(REPO_ROOT / 'python' / 'tests' / 'tof' / 'vl53l0x_test_linux.py')],
                            check=False, cwd=str(REPO_ROOT / 'python'))
        elif args.lang == 'nodejs':
            subprocess.run(['node', str(REPO_ROOT / 'nodejs' / 'tests' / 'tof' / 'vl53l0x_test.js')],
                            check=False)
        elif args.lang == 'go':
            subprocess.run(['go', 'run', str(REPO_ROOT / 'go' / 'tests' / 'tof' / 'vl53l0x_test')],
...
def main():
    ...
    if not sigrok_driver or not sigrok_channels:
        print("ERROR: SIGROK_DRIVER and SIGROK_CHANNELS must be set "
              "(testconfig_wiring's per-chip case block for tof/vl53l0x) "
              "to run conformance checks.", file=sys.stderr)
        sys.exit(2)
    sigrok_conn = os.environ.get('SIGROK_CONN') or None

    trigger = build_trigger(args)
    rc = sc.run_checks('VL53L0X', DECODER_ID, sigrok_channels, CHECKS, trigger,
