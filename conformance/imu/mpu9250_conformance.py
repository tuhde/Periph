#!/usr/bin/env python3
"""Conformance checker for MPU9250 (imu category). See
specs/testing_framework.md, "Conformance Implementation", and
specs/imu/mpu9250_timing.conf for the timing bounds checked here.

Checks (see specs/imu/mpu9250.md, Timing Constraints):
...
sigrok/mpu9250/pd.py already emits - this is new chip work, not a
retrofit of a decoder that predates the named-annotation convention
(contrast conformance/gas/ens160_conformance.py's docstring, which
explains why ENS160 still uses generic text matching).
...
imu/mpu9250 - see specs/testing_framework.md, "Chip Wiring and Sigrok
Configuration").
"""
import argparse
import os
...
TIMING_CONF = REPO_ROOT / 'specs' / 'imu' / 'mpu9250_timing.conf'
DECODER_ID = 'mpu9250'
...
def build_trigger(args):
    """Return trigger(check_name) -> None: drives one real MPU9250 HIL run
    ...
    if args.lang == 'python':
        # MPU9250 has a dedicated Linux HIL test - test_linux.sh's run_hil()
        # prefers _test_linux.py, falling back to _test.py.
        test_file = REPO_ROOT / 'python' / 'tests' / 'imu' / 'mpu9250_test_linux.py'
        return lambda name: subprocess.run(
            [sys.executable, str(test_file)], check=False, cwd=str(REPO_ROOT / 'python'))
    if args.lang == 'nodejs':
        test_file = REPO_ROOT / 'nodejs' / 'tests' / 'imu' / 'mpu9250_test.js'
...
def main():
    ...
    if not sigrok_driver or not sigrok_channels:
        print("ERROR: SIGROK_DRIVER and SIGROK_CHANNELS must be set "
              "(testconfig_wiring's per-chip case block for imu/mpu9250) "
              "to run conformance checks.", file=sys.stderr)
        sys.exit(2)
    sigrok_conn = os.environ.get('SIGROK_CONN') or None

    trigger = build_trigger(args)
    rc = sc.run_checks('MPU9250', DECODER_ID, sigrok_channels, CHECKS, trigger,
