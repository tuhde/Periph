#!/usr/bin/env python3
"""Conformance checker for HX710B (adc_dac category). See
specs/testing_framework.md, "Conformance Implementation", and
specs/adc_dac/hx710b_timing.conf for the timing bound checked here.

Check (see specs/adc_dac/hx710b.md / specs/transport_hx711.md, Timing
Constraints):
  - powerdown_pulse: PD_SCK held HIGH continuously for >60us to enter
    power-down mode.

HX710B reuses the HX711 2-wire bit-bang transport and has no shared
I2C/SPI/NeoPixel-style bus to stack a chip decoder on - sigrok/hx710b/pd.py
...
protocol='hx710b' with decoder_id=None (see decode_annotations()'s
...
adc_dac/hx710b - see specs/testing_framework.md, "Chip Wiring & Sigrok
...
TIMING_CONF = REPO_ROOT / 'specs' / 'adc_dac' / 'hx710b_timing.conf'
...
def build_trigger(args):
    ...
    construction - see each language's hx710b_test_linux.py /
    hx710b_test.js / etc.) so that happens during the capture window. Which
    ...
    if args.lang == 'python':
        test_file = REPO_ROOT / 'python' / 'tests' / 'adc_dac' / 'hx710b_test_linux.py'
        return lambda name: subprocess.run(
            [sys.executable, str(test_file)], check=False, cwd=str(REPO_ROOT / 'python'))
    if args.lang == 'nodejs':
        test_file = REPO_ROOT / 'nodejs' / 'tests' / 'adc_dac' / 'hx710b_test.js'
...
def main():
    ...
    if not sigrok_driver or not sigrok_channels:
        print("ERROR: SIGROK_DRIVER and SIGROK_CHANNELS must be set "
              "(testconfig_wiring's per-chip case block for adc_dac/hx710b) "
              "to run conformance checks.", file=sys.stderr)
        sys.exit(2)
    sigrok_conn = os.environ.get('SIGROK_CONN') or None

    trigger = build_trigger(args)
    rc = sc.run_checks('HX710B', None, sigrok_channels, CHECKS, trigger,
