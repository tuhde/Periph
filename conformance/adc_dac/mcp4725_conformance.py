#!/usr/bin/env python3
"""Conformance checker for MCP4725 (adc_dac category). See
specs/testing_framework.md, "Conformance Implementation", and
specs/adc_dac/mcp4725_timing.conf for the timing bound checked here.

Checks (see specs/adc_dac/mcp4725.md, Timing Constraints):
  - eeprom_write_time: from a "Write DAC + EEPROM" command to the next read
    observing RDY/BSY=1 (EEPROM write complete), <=
    eeprom_write_time_max_ms.

Keys off sigrok/adc_dac/mcp4725/pd.py's named "eeprom_write_start" /
...
adc_dac/mcp4725 - see specs/testing_framework.md, "Chip Wiring & Sigrok
Configuration").
"""
import argparse
import os
...
TIMING_CONF = REPO_ROOT / 'specs' / 'adc_dac' / 'mcp4725_timing.conf'
DECODER_ID = 'mcp4725'
...
def build_trigger(args):
    """Return trigger(check_name) -> None: drives one real MCP4725
    transaction sequence (a Write DAC+EEPROM followed by a status read) so
    it happens during the capture window. Which mechanism this is depends
    on how the calling script invoked us."""
    if args.binary:
    ...
    if args.lang == 'python':
        test_file = REPO_ROOT / 'python' / 'tests' / 'adc_dac' / 'mcp4725_test_linux.py'
        return lambda name: subprocess.run(
            [sys.executable, str(test_file)], check=False, cwd=str(REPO_ROOT / 'python'))
    if args.lang == 'nodejs':
        test_file = REPO_ROOT / 'nodejs' / 'tests' / 'adc_dac' / 'mcp4725_test.js'
...
def main():
    ...
    if not sigrok_driver or not sigrok_channels:
        print("ERROR: SIGROK_DRIVER and SIGROK_CHANNELS must be set "
              "(testconfig_wiring's per-chip case block for adc_dac/mcp4725) "
              "to run conformance checks.", file=sys.stderr)
        sys.exit(2)
    sigrok_conn = os.environ.get('SIGROK_CONN') or None

    trigger = build_trigger(args)
    rc = sc.run_checks('MCP4725', DECODER_ID, sigrok_channels, CHECKS, trigger,
