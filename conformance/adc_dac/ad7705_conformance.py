#!/usr/bin/env python3
"""Conformance checker for AD7705 (adc_dac category). See
specs/testing_framework.md, "Conformance Implementation", and
specs/adc_dac/ad7705_timing.conf.

CHECKS is intentionally minimal: the AD7705's documented timing
constraints (see specs/adc_dac/ad7705.md, Timing Constraints) are all
...
TIMING_CONF = REPO_ROOT / 'specs' / 'adc_dac' / 'ad7705_timing.conf'
DECODER_ID = 'ad7705'
...
def build_trigger(args):
    ...
    if args.lang == 'python':
        test_file = REPO_ROOT / 'python' / 'tests' / 'adc_dac' / 'ad7705_test_linux.py'
        return lambda name: subprocess.run(
            [sys.executable, str(test_file)], check=False, cwd=str(REPO_ROOT / 'python'))
    if args.lang == 'nodejs':
        test_file = REPO_ROOT / 'nodejs' / 'tests' / 'adc_dac' / 'ad7705_test.js'
        return lambda name: subprocess.run(['node', str(test_file)], check=False)
    if args.lang == 'cpp':
        return lambda name: subprocess.run(
            [str(REPO_ROOT / 'cpp' / 'test_linux.sh'), 'adc_dac/ad7705_test_linux'],
            check=False, cwd=str(REPO_ROOT / 'cpp'))
    if args.lang == 'go':
        return lambda name: subprocess.run(
            ['go', 'run', './main.go'], check=False,
            cwd=str(REPO_ROOT / 'go' / 'tests' / 'adc_dac' / 'ad7705_test'))
...
def main():
    ...
    if not sigrok_driver or not sigrok_channels:
        print("ERROR: SIGROK_DRIVER and SIGROK_CHANNELS must be set "
              "(testconfig_wiring's per-chip case block for adc_dac/ad7705) "
              "to run conformance checks.", file=sys.stderr)
        sys.exit(2)
    sigrok_conn = os.environ.get('SIGROK_CONN') or None

    trigger = build_trigger(args)
    rc = sc.run_checks('AD7705', DECODER_ID, sigrok_channels, CHECKS, trigger,
