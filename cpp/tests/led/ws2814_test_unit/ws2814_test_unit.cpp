...
// sk6812rgbw_test_unit.cpp) - the generic I2CConnectionMock works unchanged;
...
int main() {
    ...
    WS2814Full sensor(connection, N);
    ...
    check_true(connection.writes().back().size() == N * 4 + 90, "fill_length");
    {
        const auto& w = connection.writes().back();
        check_true(w[0] == 0x11 && w[1] == 0x22 && w[2] == 0x33 && w[3] == 0x44, "fill_rgbw_order");
        bool allPixels = true;
        for (size_t i = 0; i < N; i++) {
            if (w[i*4] != 0x11 || w[i*4+1] != 0x22 || w[i*4+2] != 0x33 || w[i*4+3] != 0x44) allPixels = false;
        }
        check_true(allPixels, "fill_all_pixels");
        check_true(allZero(w, N * 4, N * 4 + 90), "fill_reset_tail");
    }
    ...
    {
        const auto& w = connection.writes().back();
        check_true(w[0] == 0x10 && w[1] == 0x20 && w[2] == 0x30 && w[3] == 0x00, "fill_white_defaults_zero");
    }
    ...
    sensor.off();
    {
        ...
        check_true(allZero(w, N * 4, N * 4 + 90), "off_reset_tail");
    }
    ...
    {
        const auto& w = connection.writes().back();
        check_true(w[4] == 0xAA && w[5] == 0xBB && w[6] == 0xCC && w[7] == 0xDD, "set_pixel_then_show");
    ...
    {
        const auto& w = connection.writes().back();
        check_true(w[0] == 0x01 && w[1] == 0x02 && w[2] == 0x03 && w[3] == 0x00, "set_pixel_white_defaults_zero");
    }
    ...
    {
        const auto& w = connection.writes().back();
        check_true(w[(N-1)*4] == 0x05 && w[(N-1)*4+1] == 0x06 && w[(N-1)*4+2] == 0x07 && w[(N-1)*4+3] == 0x08,
    ...
    sensor.show();
    {
        ...
        check_true(w[0] == sr && w[1] == sg && w[2] == sb && w[3] == sw, "brightness_scaling");
    }
    ...
    {
        const auto& w = connection.writes().back();
        check_true(w[0] == 2 && w[2*4] == 1, "rotate_shifts_left");
    }
    ...
    {
        const auto& w = connection.writes().back();
        check_true(w[0] == 255 && w[1] == 0 && w[2] == 0 && w[3] == 0, "fill_hsv_red");
    }
