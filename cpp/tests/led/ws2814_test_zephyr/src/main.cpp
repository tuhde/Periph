...
#ifndef WS2814_SPI_NODE
#define WS2814_SPI_NODE DT_NODELABEL(spi0)
#endif
...
int main(void) {
    const struct device *spi_dev = DEVICE_DT_GET(WS2814_SPI_NODE);
    NeoPixelConnectionZephyr connection(spi_dev);

    // --- WS2814Minimal ---
    {
        WS2814Minimal strip(connection, 8);
