...
int main(void) {
    ...
    SiPoConnectionZephyr connection(spi_dev, cfg, rck, srclr, g);

    const uint8_t data1[1] = {0xA5};
    connection.write(data1, 1);
    check_true(true, "write accepted");

    const uint8_t data2[2] = {0x00, 0xFF};
    connection.write(data2, 2);
    check_true(true, "write multi-byte accepted");

    check_true(connection.clear() == 0, "clear returns 0 when configured");

    check_true(connection.set_output_enable(false) == 0,
               "set_output_enable(false) returns 0 when configured");
    check_true(connection.set_output_enable(true) == 0,
