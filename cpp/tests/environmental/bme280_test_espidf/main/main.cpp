// Auto-generated ESP-IDF test for BME280.
// Mirrors the Zephyr test for BME280; prints PASS/FAIL and exits.
...
extern "C" void app_main(void) {
    ...
    BME280Full inst(transport, false);  // Create BME280 driver
    check_eq_u8(inst.chip_id(), 0x60, "chip_id 0x60");
    float t = inst.temperature();
    check_near(t, -40.0f, 85.0f, "temperature range");
    float p = inst.pressure();
    check_near(p, 300.0f, 1100.0f, "pressure range");
    float h = inst.humidity();
    check_near(h, 0.0f, 100.0f, "humidity range");
    inst.configure(1, 1, 1, 0, 0, 0);
    (void)inst.status();
