...
int main() {
    ...
    adc.configure(1, AD7706Full::GAIN_2, true, false, 60);           // Configure channel 1, (channel=1, gain=GAIN_2, bipolar=true, buffered=false, output_rate_hz=60) → None
    check_true("configure(1, gain=2) accepted", true);
    adc.configure(2, AD7706Full::GAIN_4, false, true, 60);           // Configure channel 2, (channel=2, gain=GAIN_4, bipolar=false, buffered=true, output_rate_hz=60) → None
    check_true("configure(2, gain=4) accepted", true);
    adc.configure(3, AD7706Full::GAIN_4, false, true, 60);           // Configure channel 3, (channel=3, gain=GAIN_4, bipolar=false, buffered=true, output_rate_hz=60) → None
    ...
    uint32_t off3 = adc.get_offset_calibration(3);                   // Read offset calibration, (channel=3) → uint32_t 24-bit
    check_true("get_offset_calibration(3) in [0, 2^24-1]", off3 <= 0xFFFFFF);
    uint32_t gain3 = adc.get_gain_calibration(3);                    // Read gain calibration, (channel=3) → uint32_t 24-bit
