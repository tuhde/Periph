...
int main(void) {
    ...
    while (!sensor.dataReady()) {                           // Check for a result, () → bool
        k_msleep(10);                                       // GPIO1 line asserted
    }
    ...
    printk("calibrated offset %.2f mm, crosstalk %.4f MCPS\n", (double)calOffset, (double)calXtalk);
