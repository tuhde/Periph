...
int main(void) {
    ...
    printk("optical centre SPAD %u, zone centres %u/%u\n",
    ...
    printk("baseline left %.0f mm, right %.0f mm\n", (double)baseline[0], (double)baseline[1]);
    ...
    while (events < MAX_EVENTS && elapsedMs < MAX_MS) {
        ...
        if (elapsedMs - lastSnapshot >= SNAPSHOT_MS) {
            ...
            if (m.ambientRateMcps > BRIGHT_MCPS &&
                ...
                printk("bright ambient light: switched to short distance mode\n");
            }
        }
    }
