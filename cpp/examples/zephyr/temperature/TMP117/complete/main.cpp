...
int main(void) {
    ...
    while (!sensor.isDataReady()) {                         // Check for a fresh result, () → bool
        k_msleep(10);                              // reading Data_Ready clears it
    }
    printk("one-shot %.4f C\n", (double)sensor.readTemperature());  // Read temperature, () → °C
                                                            // the one-shot result
    ...
    printk("above high %d below low %d\n",
