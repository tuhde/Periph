...
int main() {
    ...
    rtc.forceTemperatureConversion();                       // Force a fresh conversion, () → None (blocks up to 200 ms)
    ...
    printf("%04u-%02u-%02u %02u:%02u:%02u  %.2fC  alarm1=%u alarm2_min=%u  status=0x%02X  32khz=%d  osf=%d  aging=%d\n",
           dt.year, dt.month, dt.day, dt.hour, dt.minute, dt.second, (double)tempC,
           got1.hour, got2.minute, status, en32, stopped, aging);
    return 0;
}
