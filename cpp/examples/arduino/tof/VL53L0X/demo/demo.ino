...
void setup() {
    ...
    if (first.ambientRateMcps < 0.5f) {
        ...
        logPrintf("dark scene (%.2f MCPS ambient): long range profile\n", (double)first.ambientRateMcps);
    } else {
        sensor.setProfile(VL53L0XFull::Profile::Default);   // Apply ranging profile, (profile) → bool
        logPrintf("bright scene (%.2f MCPS ambient): default profile\n", (double)first.ambientRateMcps);
    }
    ...
    logPrintf("mean %lu mm, min %u mm, max %u mm, signal %.2f MCPS\n",
