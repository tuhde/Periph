...
void setup() {
    ...
    if (CALIBRATE) {
        ...
        logPrintf("calibrated, offset %.4f C\n", (double)offset);
    }
    ...
    while (alerts < MAX_ALERTS) {
        ...
        if (pending) {
            ...
            if (status & TMP117Full::SOURCE_HIGH)     logPrintf("%.2f C  too warm - cargo above 8 C\n", (double)t);
            else if (status & TMP117Full::SOURCE_LOW) logPrintf("%.2f C  too cold - cargo below -25 C\n", (double)t);
            alerts++;
        }
        delay(1000);
    }
