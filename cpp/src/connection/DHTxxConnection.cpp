...
DHTxxConnection::DHTxxConnection(uint8_t data_pin) : _pin(data_pin) {
    pinMode(_pin, INPUT);
}

void DHTxxConnection::_drive_low() {
    pinMode(_pin, OUTPUT);
    digitalWrite(_pin, LOW);
}

void DHTxxConnection::_release_bus() {
    pinMode(_pin, INPUT);
}

int32_t DHTxxConnection::_measure_pulse(uint8_t level, uint32_t timeout_us) {
...
bool DHTxxConnection::read(uint8_t* out) {
