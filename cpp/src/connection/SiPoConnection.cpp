...
SiPoConnection::SiPoConnection(SPIClass& spi, int rck_pin, int srclr_pin, int g_pin,
...
SiPoConnection::SiPoConnection(int ser_in_pin, int srck_pin, int rck_pin,
...
void SiPoConnection::write(const uint8_t* data, size_t len) {
    if (!_enabled) return;
    if (_spi != nullptr) {
...
void SiPoConnection::_latch() {
    digitalWrite(_rck, HIGH);
    digitalWrite(_rck, LOW);
}

bool SiPoConnection::clear() {
...
bool SiPoConnection::set_output_enable(bool enabled) {
