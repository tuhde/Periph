...
/** @brief SiPo (serial-in/parallel-out shift register) connection for Zephyr RTOS.
 *
 ...
 */
class SiPoConnectionZephyr {
public:
    SiPoConnectionZephyr(const struct device *dev, const struct spi_config &config,
     ...
     *  data into the storage register that drives the outputs. No-op if
