...
UARTConnectionLinux::~UARTConnectionLinux() {
    close();
}
...
void UARTConnectionLinux::_write(const uint8_t* data, size_t len) {
    if (!_rs485_kernel)
        _de_set(1);
...
void UARTConnectionLinux::_read(uint8_t* buf, size_t len) {
    size_t got = 0;
    while (got < len) {
...
size_t UARTConnectionLinux::available() const {
    int n = 0;
...
void UARTConnectionLinux::_write_read(const uint8_t* data, size_t data_len,
                                       uint8_t* buf, size_t buf_len) {
    _write(data, data_len);
    _read(buf, buf_len);
}

void UARTConnectionLinux::close() {
    if (_fd >= 0) {
        ::close(_fd);
        _fd = -1;
    }
