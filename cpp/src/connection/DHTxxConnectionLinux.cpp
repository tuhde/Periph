...
DHTxxConnectionLinux::DHTxxConnectionLinux(const char* chip_path, unsigned int line_num)
...
DHTxxConnectionLinux::DHTxxConnectionLinux(const char* chip_path, unsigned int line_num, unsigned int line_num_out)
    : _chip_path(strdup(chip_path)),
      _line_num(line_num),
...
DHTxxConnectionLinux::~DHTxxConnectionLinux() {
    close();
}

void DHTxxConnectionLinux::_drive_low() {
    if (_two_pin) {
        if (_output_req) {
...
void DHTxxConnectionLinux::_release_bus() {
    if (_two_pin) {
        if (_output_req) {
...
int32_t DHTxxConnectionLinux::_measure_pulse(int level, uint32_t timeout_us) {
...
bool DHTxxConnectionLinux::read(uint8_t* out) {
...
void DHTxxConnectionLinux::close() {
    if (_closed) return;
    _closed = true;
