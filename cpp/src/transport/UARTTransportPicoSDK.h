...
/** @brief UART transport for the Raspberry Pi Pico SDK (wraps `hardware_uart`).
 *
 * Bare-metal `pico-sdk` — no Arduino core, no RTOS. One transport instance
 ...
 */
class UARTTransportPicoSDK : public Transport {
public:
    UARTTransportPicoSDK(uart_inst_t* uart, uint baud, int de_pin = -1)
        : _uart(uart), _baud(baud), _de_pin(de_pin)
    {
        if (_de_pin >= 0) {
            gpio_init(_de_pin);
