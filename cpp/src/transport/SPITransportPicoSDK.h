...
/** @brief SPI transport for the Raspberry Pi Pico SDK (wraps `hardware_spi`).
 *
 * Bare-metal `pico-sdk` — no Arduino core, no RTOS, no devicetree. One
 * transport instance represents one device on one SPI controller; the
 ...
 * does, so CS is a plain GPIO the transport drives itself — the same
 * convention `SPITransport` (Arduino) already uses.
 *
 ...
 */
class SPITransportPicoSDK : public Transport {
public:
    SPITransportPicoSDK(spi_inst_t* spi, uint cs)
        : _spi(spi), _cs(cs)
    {
        gpio_init(_cs);
