...
/** @brief SiPo (serial-in/parallel-out shift register) transport for the
 ...
 *   The transport shifts data via `spi_write_blocking` and latches with
 *   `gpio_put()` on RCK.
 * - **Software:** pass `ser_in` and `srck` pin numbers instead of an
 *   `spi_inst_t*`. The transport bit-bangs the MSB-first mode-0 loop
 *   directly with `gpio_put()`.
 *
 * Write-only: no `read` or `write_read` exists.
 *
...
public:
     ...
     */
    SiPoTransportPicoSDK(spi_inst_t* spi, uint rck, int srclr = -1, int g = -1)
     ...
     *  @param g      GPIO pin number for G (output enable); `-1` to disable.
     */
    SiPoTransportPicoSDK(uint ser_in, uint srck, uint rck, int srclr = -1, int g = -1)
