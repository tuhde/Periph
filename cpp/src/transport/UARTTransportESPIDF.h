...
/** @brief UART transport for ESP-IDF (`driver/uart.h`).
 *
 ...
 * as the Arduino and Linux (C++) transports.
 *
 ...
 * `uart_port_t` to construct the transport.
 *
 ...
 *               RS-485 mode.
 */
class UARTTransportESPIDF : public Transport {
public:
    UARTTransportESPIDF(uart_port_t port, int de_pin = -1)
        : _port(port), _de_pin(static_cast<gpio_num_t>(de_pin))
    {
