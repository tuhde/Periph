...
/** @brief SiPo (serial-in/parallel-out shift register) transport for ESP-IDF.
 *
 ...
 * `SiPoTransport` (Arduino) and `SiPoTransportPicoSDK` already use.
 *
...
public:
     ...
     */
    SiPoTransportESPIDF(spi_device_handle_t spi, gpio_num_t rck,
                        int srclr = -1, int g = -1)
     ...
     *  @param g      GPIO pin number for G (output enable); `-1` to disable.
     */
    SiPoTransportESPIDF(gpio_num_t ser_in, gpio_num_t srck, gpio_num_t rck,
                        int srclr = -1, int g = -1)
