...
/** @brief BMP085 piezo-resistive pressure + temperature sensor — minimal interface.
 *
 *  Provides calibrated temperature (°C) and pressure (Pa) with no configuration
 *  beyond the connection. The BMP085 has a fixed I²C address (0x77) and no
...
class BMP085Minimal {
public:
    explicit BMP085Minimal(Connection& connection);
...
/** @brief BMP085 full interface — extends BMP085Minimal with OSS control and altitude helpers.
 *
 ...
 */
class BMP085Full : public BMP085Minimal {
public:
    ...
    explicit BMP085Full(Connection& connection, uint8_t oss = 0);
     ...
     *  @param sea_level_pa Reference sea-level pressure in Pa (default 101325.0).
