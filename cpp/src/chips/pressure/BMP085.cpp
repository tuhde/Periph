...
BMP085Minimal::BMP085Minimal(Connection& connection)
    : _connection(connection) {
    _read_calibration();
}

void BMP085Minimal::_read_calibration() {
    uint8_t buf[22];
...
void BMP085Minimal::_write_reg(uint8_t reg, uint8_t value) {
    uint8_t buf[2] = { reg, value };
    _connection.write(buf, 2);
}

uint16_t BMP085Minimal::_read_raw_temp() {
...
uint32_t BMP085Minimal::_read_raw_pressure() {
...
int32_t BMP085Minimal::_compensate_temp(int32_t ut) {
...
int32_t BMP085Minimal::_compensate_pressure(int32_t up) {
...
float BMP085Minimal::temperature() {
...
BMP085Full::BMP085Full(Connection& connection, uint8_t oss)
    : BMP085Minimal(connection) {
    _oss = oss & 0x03;
}

uint8_t BMP085Full::oversampling() {
    return _oss;
}

void BMP085Full::set_oversampling(uint8_t oss) {
    _oss = oss & 0x03;
}

float BMP085Full::altitude(float sea_level_pa) {
    float p = pressure();
    return 44330.0f * (1.0f - powf(p / sea_level_pa, 0.1903f));
}

float BMP085Full::sea_level_pressure(float altitude_m) {
    float p = pressure();
    return p / powf(1.0f - ((float)altitude_m / 44330.0f), 5.255f);
}

uint8_t BMP085Full::chip_id() {
    uint8_t reg = REG_ID;
    uint8_t buf[1];
