...
/** @brief AD7705 2-channel, 16-bit sigma-delta ADC — base class.
 *
 * Two-phase register-access protocol: write the Communication Register
 * (selects target register + read/write direction + channel) and then
...
protected:
     ...
     */
    _AD7705Base(Connection& connection, float vref, uint32_t mclk_hz, OutputPin* reset_pin);
...
/** @brief AD7705 minimal driver — Channel 1 voltage read with sensible defaults. */
class AD7705Minimal : public _AD7705Base {
public:
    /** @brief Construct and initialise the AD7705 with default configuration.
     ...
     */
    AD7705Minimal(Connection& connection, float vref, uint32_t mclk_hz, OutputPin* reset_pin = nullptr)
        : _AD7705Base(connection, vref, mclk_hz, reset_pin) {}
...
/** @brief AD7705 full driver — adds per-channel configuration, calibration, and power control. */
class AD7705Full : public AD7705Minimal {
public:
