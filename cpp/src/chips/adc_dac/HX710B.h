...
/** @brief HX710B 24-bit ADC — minimal interface.
 *
 ...
 *                   or HX711ConnectionZephyr) — the HX710B reuses the HX711 transport.
 */
...
public:
     ...
     */
    explicit HX710BMinimal(Connection& connection) : _connection(connection) {
...
/** @brief HX710B full interface — extends HX710BMinimal with rate selection,
 *  tare, calibration, supply-difference monitoring, and power management.
 *
 * The HX710B has no software-selectable gain and no second input channel —
 * unlike the HX711, only the output rate (10 or 40 SPS) is selectable for
 * the differential-input reading. Pulse count 26 reads the DVDD−AVDD
 * supply-voltage difference for battery-monitoring applications.
 *
 * @tparam Connection HX711 connection type.
 */
template<typename Connection>
class HX710BFull : public HX710BMinimal<Connection> {
public:
    /** @brief Initialize with default 10 SPS rate, offset 0, and scale 1.0.
     *  @param connection Reference to a configured HX711 connection.
     */
    explicit HX710BFull(Connection& connection)
        : HX710BMinimal<Connection>(connection), _pulses(25), _offset(0), _scale(1.0f)
    {}
    ...
    /** @brief Block until data is ready and return the raw DVDD−AVDD code.
     *
     ...
     *
     *  @return Signed 24-bit raw ADC code from the supply-difference channel.
     */
    int32_t read_supply_diff_raw() { return this->_connection.read_raw(26); }
