...
constexpr uint8_t _AD7705Base::_GAIN_BITS[8];
constexpr uint16_t _AD7705Base::_FS_RATES_1MHZ[4];
constexpr uint16_t _AD7705Base::_FS_RATES_2_4MHZ[4];
...
uint8_t _AD7705Base::_comm_byte(uint8_t reg, bool read, uint8_t channel) {
    return (uint8_t)(reg | (read ? _RW_READ : _RW_WRITE) | (channel & 0x03));
}

void _AD7705Base::_wait_drdy() {
    while (true) {
...
void _AD7705Base::_hardware_reset() {
...
uint32_t _AD7705Base::_read_reg_channel(uint8_t reg, uint8_t channel, uint8_t n_bytes) {
...
float AD7705Minimal::read_voltage() {
    uint16_t code = read_raw();
    return _code_to_voltage(code, _gain, _bipolar);
}

void AD7705Full::configure(uint8_t channel, uint8_t gain, bool bipolar, bool buffered, uint16_t output_rate_hz) {
...
uint16_t AD7705Full::read_raw(uint8_t channel) {
...
uint32_t AD7705Full::get_offset_calibration(uint8_t channel) {
    if (channel != 1 && channel != 2) return 0;
    uint8_t ch = (channel == 1) ? _CH1 : _CH2;
    return _read_reg_channel(_REG_OFFSET, ch, 3);
}

void AD7705Full::set_offset_calibration(uint32_t value, uint8_t channel) {
    if (channel != 1 && channel != 2) return;
    uint8_t ch = (channel == 1) ? _CH1 : _CH2;
    _write_reg_channel(_REG_OFFSET, value & 0xFFFFFF, ch, 3);
}

uint32_t AD7705Full::get_gain_calibration(uint8_t channel) {
    if (channel != 1 && channel != 2) return 0;
    uint8_t ch = (channel == 1) ? _CH1 : _CH2;
    return _read_reg_channel(_REG_GAIN, ch, 3);
}

void AD7705Full::set_gain_calibration(uint32_t value, uint8_t channel) {
    if (channel != 1 && channel != 2) return;
    uint8_t ch = (channel == 1) ? _CH1 : _CH2;
    _write_reg_channel(_REG_GAIN, value & 0xFFFFFF, ch, 3);
}

void AD7705Full::standby() {
    uint8_t comm = _comm_byte(_REG_COMM, false, _CH1) | _STBY_SLEEP;
    _connection.write(&comm, 1);
}

void AD7705Full::wakeup() {
    uint8_t comm = _comm_byte(_REG_COMM, false, _CH1) | _STBY_RUN;
    _connection.write(&comm, 1);
    _wait_drdy();
}

void AD7705Full::reset() {
