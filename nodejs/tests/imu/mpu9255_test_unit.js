...
const _REG_MOT_DETECT_CTRL = 0x69;
...
async function main() {
    ...
    // own, mirroring how the real driver is wired (see MPU9255Full's
    // doc comment).
    const connection = new I2CConnectionMock();
    connection.setRegister(_REG_WHO_AM_I, [0x73]);
    const magConnection = new I2CConnectionMock();

    const sensor = new MPU9255Full(connection, magConnection);
    ...
    const womWrites = connection.writes;
    ...
    checkTrue('wom_lp_accel_odr',     womWrites[womN - 2].equals(Buffer.from([_REG_LP_ACCEL_ODR, 0x07])));
    checkTrue('wom_pwr_mgmt_1_cycle', womWrites[womN - 1].equals(Buffer.from([_REG_PWR_MGMT_1, 0x21])));

    // threshold_mg clamping: 0 -> 1 LSB, 4000 -> 255 LSB.
    await sensor.configureWakeOnMotion(0, 31.25);
    const womWritesLow = connection.writes;
    checkTrue('wom_threshold_low_clamped', womWritesLow[womWritesLow.length - 3].equals(Buffer.from([_REG_WOM_THR, 1])));
    await sensor.configureWakeOnMotion(4000, 31.25);
    const womWritesHigh = connection.writes;
    checkTrue('wom_threshold_high_clamped', womWritesHigh[womWritesHigh.length - 3].equals(Buffer.from([_REG_WOM_THR, 255])));

    // motionDetected(): True when WOM_INT (bit 6) set in INT_STATUS.
    connection.setRegister(_REG_INT_STATUS, [0x40]);
    checkTrue('motion_detected_true', (await sensor.motionDetected()) === true);
    connection.setRegister(_REG_INT_STATUS, [0x00]);
    checkTrue('motion_detected_false', (await sensor.motionDetected()) === false);

    // mag() / magRaw() before enableMag() raise.
    const unenabledConnection = new I2CConnectionMock();
    unenabledConnection.setRegister(_REG_WHO_AM_I, [0x73]);
    const unenabledSensor = new MPU9255Full(unenabledConnection, new I2CConnectionMock());
