...
async function main() {
    ...
    const chip       = new Pcf8575Minimal(connection);

    check_eq('init_shadow_0', chip._shadow[0], 0xFF);
    check_eq('init_shadow_1', chip._shadow[1], 0xFF);

    const port0 = await chip.readPort(0);
    const port1 = await chip.readPort(1);
    ...
    await chip.writePort(0, 0xAA);
    check_eq('loopback_0xAA', await chip.readPort(1), 0x55);

    await chip.writePort(0, 0xF0);
    check_eq('loopback_0xF0', await chip.readPort(1), 0x0F);

    await chip.writePort(0, 0x00);
    check_eq('loopback_0x00', await chip.readPort(1), 0x00);

    await chip.writePort(0, 0xFF);
    await chip.writePort(1, 0xFF);

    const full = new Pcf8575Full(connection);
    const changed = await full.pollInterrupt();
    check_true('poll_interrupt_range', changed >= 0 && changed <= 0xFFFF);
