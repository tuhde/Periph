...
async function main() {
    ...
    const lcd = new PCF8576Minimal(connection);
    ...
    try {
        await lcd.setDigit7seg(0, 0xED);
        await lcd.setDigit7seg(1, 0x60);
        console.log('PASS set_digit_7seg');
        passed++;
    } catch (e) { console.log('FAIL set_digit_7seg: ' + e.message); failed++; }

    try {
        await lcd.writeRaw(0, [0xED, 0x60, 0xA7, 0xE3]);
        console.log('PASS write_raw');
        passed++;
    } catch (e) { console.log('FAIL write_raw: ' + e.message); failed++; }

    const lcdFull = new PCF8576Full(connection);
    try {
        await lcdFull.enable();
        await lcdFull.disable();
        await lcdFull.enable();
    ...
    try {
        await lcdFull.setMode(PCF8576Full.BACKPLANES_4, PCF8576Full.BIAS_1_3);
        await lcdFull.setMode(PCF8576Full.BACKPLANES_2, PCF8576Full.BIAS_1_2);
        await lcdFull.setMode(PCF8576Full.BACKPLANES_1, PCF8576Full.BIAS_1_3);
        console.log('PASS set_mode');
        passed++;
    } catch (e) { console.log('FAIL set_mode: ' + e.message); failed++; }

    try {
        await lcdFull.setBlink(PCF8576Full.BLINK_2_HZ);
        await lcdFull.setBlink(PCF8576Full.BLINK_OFF);
        console.log('PASS set_blink');
        passed++;
    } catch (e) { console.log('FAIL set_blink: ' + e.message); failed++; }

    try {
        await lcdFull.setBank(0, 0);
        await lcdFull.setBank(1, 1);
        console.log('PASS set_bank');
        passed++;
    } catch (e) { console.log('FAIL set_bank: ' + e.message); failed++; }

    try {
        await lcdFull.deviceSelect(0);
        await lcdFull.deviceSelect(7);
