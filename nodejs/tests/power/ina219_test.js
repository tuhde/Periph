...
function checkNear(label, val, lo, hi) {
    if (val >= lo && val <= hi) { console.log('PASS', label); passed++; }
    else { console.log(`FAIL ${label}: ${val} not in [${lo}, ${hi}]`); failed++; }
}
...
checkTrue('configure: voltage still valid', ina.voltage() >= 0.0);
