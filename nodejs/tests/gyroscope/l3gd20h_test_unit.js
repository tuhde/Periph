const { I2CMock } = require('../../packages/periph/src/connection/i2c_mock');
...
async function runTests() {
    ...
    {
        const mock = new I2CMock();
        mock.setReg(0x0F, Buffer.from([0xD7]));
        mock.setReg(0x20, Buffer.from([0x0F]));
        mock.setReg(0x23, Buffer.from([0x80]));
    ...
    {
        const mock = new I2CMock();
        mock.setReg(0x0F, Buffer.from([0xD4]));
        mock.setReg(0x20, Buffer.from([0x0F]));
        mock.setReg(0x23, Buffer.from([0x80]));
        const gyro = new L3GD20HMinimal(mock);
        await new Promise(r => setTimeout(r, 260));
    ...
    {
        const mock = new I2CMock();
        mock.setReg(0x0F, Buffer.from([0x00]));
        const gyro = new L3GD20HMinimal(mock);
        await new Promise(r => setTimeout(r, 260));
    ...
    // Test 4: gyro() returns array of 3 numbers
    {
        ...
        mock.setReg(0x20, Buffer.from([0x0F]));
        mock.setReg(0x23, Buffer.from([0x80]));
        mock.setReg(0x28, Buffer.from([0x00, 0x01, 0x00, 0x02, 0x00, 0x03]));
        const gyro = new L3GD20HMinimal(mock);
        await new Promise(r => setTimeout(r, 260));
    ...
    // Test 6: configure() sets registers
    {
        ...
        check('configure() sets CTRL_REG1', mock.getReg(0x20)[0] === 0x4F);
        check('configure() sets CTRL_REG4', mock.getReg(0x23)[0] === 0x90);
    }
    ...
    // Test 7: gyroRaw() returns signed int16
    {
        ...
        mock.setReg(0x20, Buffer.from([0x0F]));
        mock.setReg(0x23, Buffer.from([0x80]));
        mock.setReg(0x28, Buffer.from([0x00, 0x80, 0xFF, 0x7F, 0x00, 0x00]));
        const gyro = new L3GD20HFull(mock);
        await new Promise(r => setTimeout(r, 260));
    ...
    // Test 8: temperature() returns signed 8-bit
    {
        ...
        mock.setReg(0x20, Buffer.from([0x0F]));
        mock.setReg(0x23, Buffer.from([0x80]));
        mock.setReg(0x26, Buffer.from([0x80]));
        const gyro = new L3GD20HFull(mock);
        await new Promise(r => setTimeout(r, 260));
        check('temperature() negative', await gyro.temperature() === -128);
        mock.setReg(0x26, Buffer.from([0x7F]));
    ...
    // Test 9: dataReady() returns ZYXDA bit
    {
        ...
        mock.setReg(0x20, Buffer.from([0x0F]));
        mock.setReg(0x23, Buffer.from([0x80]));
        mock.setReg(0x27, Buffer.from([0x08]));
        const gyro = new L3GD20HFull(mock);
        await new Promise(r => setTimeout(r, 260));
        check('dataReady() true', await gyro.dataReady() === true);
        mock.setReg(0x27, Buffer.from([0x00]));
    ...
    // Test 10: configureHpFilter()
    {
        ...
        check('configureHpFilter() sets CTRL_REG2', mock.getReg(0x21)[0] === 0x15);
    }
    ...
    // Test 11: enableHpFilter()
    {
        ...
        check('enableHpFilter(true) sets HPen', (mock.getReg(0x24)[0] & 0x10) === 0x10);
        await gyro.enableHpFilter(false);
        check('enableHpFilter(false) clears HPen', (mock.getReg(0x24)[0] & 0x10) === 0);
    }
    ...
    // Test 12: configureFifo()
    {
        ...
        check('configureFifo() sets FIFO_EN', (mock.getReg(0x24)[0] & 0x40) === 0x40);
        check('configureFifo() sets FIFO_CTRL_REG', mock.getReg(0x2E)[0] === 0x2A);
    }
    ...
    // Test 13: enableFifo()
    {
        ...
        check('enableFifo(true) sets FIFO_EN', (mock.getReg(0x24)[0] & 0x40) === 0x40);
        await gyro.enableFifo(false);
        check('enableFifo(false) clears FIFO_EN', (mock.getReg(0x24)[0] & 0x40) === 0);
        check('enableFifo(false) sets FIFO_CTRL_REG=0', mock.getReg(0x2E)[0] === 0x00);
    }
    ...
    // Test 14: fifoLevel()
    {
        ...
        mock.setReg(0x20, Buffer.from([0x0F]));
        mock.setReg(0x23, Buffer.from([0x80]));
        mock.setReg(0x2F, Buffer.from([0x05]));
        const gyro = new L3GD20HFull(mock);
        await new Promise(r => setTimeout(r, 260));
    ...
    // Test 15: setPowerMode()
    {
        ...
        check('setPowerMode(normal) enables all axes', (mock.getReg(0x20)[0] & 0x0F) === 0x0F);
        await gyro.setPowerMode('sleep');
        check('setPowerMode(sleep) disables axes', (mock.getReg(0x20)[0] & 0x0F) === 0x08);
        await gyro.setPowerMode('power_down');
        check('setPowerMode(power_down) clears PD', (mock.getReg(0x20)[0] & 0x08) === 0);
    }
