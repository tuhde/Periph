...
const { WS2814Full } = require('../../packages/periph/src/chips/led/ws2814');
...
// WS2814 is write-only, same reasoning as SK6812RGBW (see
// sk6812rgbw_test_unit.js) - the generic I2CConnectionMock works unchanged;
// only its .writes list is used. WS2814 wire order is RGBW (identity - no
// reorder) and appends 90 zero-bytes (~300us reset) after the pixel buffer
// on every transmit.
...
async function main() {
    ...
    const sensor = new WS2814Full(connection, N);
    checkTrue('init', true);

    // fill(): RGBW wire order (identity), all N pixels, transmits
    // immediately, with the 90-byte extended reset tail appended.
    ...
    checkTrue('fill_length', w.length === N * 4 + 90);
    checkTrue('fill_rgbw_order', w[0] === 0x11 && w[1] === 0x22 && w[2] === 0x33 && w[3] === 0x44);
    let allPixels = true;
    for (let i = 0; i < N; i++) {
        if (w[i * 4] !== 0x11 || w[i * 4 + 1] !== 0x22 || w[i * 4 + 2] !== 0x33 || w[i * 4 + 3] !== 0x44) allPixels = false;
    }
    checkTrue('fill_all_pixels', allPixels);
    checkTrue('fill_reset_tail', allZero(w, N * 4, N * 4 + 90));

    // fill() white channel defaults to 0.
    await sensor.fill(0x10, 0x20, 0x30);
    w = connection.writes[connection.writes.length - 1];
    checkTrue('fill_white_defaults_zero', w[0] === 0x10 && w[1] === 0x20 && w[2] === 0x30 && w[3] === 0x00);

    // fill() clamps out-of-range channels.
    await sensor.fill(-10, 300, 128, 999);
    w = connection.writes[connection.writes.length - 1];
    checkTrue('fill_clamps', w[0] === 0 && w[1] === 255 && w[2] === 128 && w[3] === 255);

    // off(): equivalent to fill(0, 0, 0, 0).
    await sensor.off();
    w = connection.writes[connection.writes.length - 1];
    checkTrue('off', allZero(w, 0, N * 4));
    checkTrue('off_reset_tail', allZero(w, N * 4, N * 4 + 90));
    ...
    await sensor.show();
    w = connection.writes[connection.writes.length - 1];
    checkTrue('set_pixel_then_show', w[4] === 0xAA && w[5] === 0xBB && w[6] === 0xCC && w[7] === 0xDD);
    checkTrue('set_pixel_other_pixels_unchanged', w[0] === 0 && w[1] === 0 && w[2] === 0 && w[3] === 0);

    sensor.set_pixel(0, 0x01, 0x02, 0x03);
    await sensor.show();
    w = connection.writes[connection.writes.length - 1];
    checkTrue('set_pixel_white_defaults_zero', w[0] === 0x01 && w[1] === 0x02 && w[2] === 0x03 && w[3] === 0x00);
    ...
    checkTrue('set_pixel_clamps_index',
        w[(N - 1) * 4] === 0x05 && w[(N - 1) * 4 + 1] === 0x06 && w[(N - 1) * 4 + 2] === 0x07 && w[(N - 1) * 4 + 3] === 0x08);
    ...
    await sensor.show();
    w = connection.writes[connection.writes.length - 1];
    const expected = [0x10, 0x20, 0x30, 0x00, 0x40, 0x50, 0x60, 0x70, 0x80, 0x90, 0xA0, 0xB0];
    ...
    checkTrue('brightness_scaling', w[0] === sr && w[1] === sg && w[2] === sb && w[3] === sw);
    ...
    await sensor.show();
    w = connection.writes[connection.writes.length - 1];
    checkTrue('rotate_shifts_left', w[0] === 2 && w[2 * 4] === 1);

    // fill_hsv(): pure red (h=0, s=1, v=1) -> RGB (255, 0, 0), white=0.
    await sensor.fill_hsv(0.0, 1.0, 1.0);
    w = connection.writes[connection.writes.length - 1];
    checkTrue('fill_hsv_red', w[0] === 255 && w[1] === 0 && w[2] === 0 && w[3] === 0);
