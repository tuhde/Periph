...
const { HX710BFull }      = require('../../packages/periph/src/chips/adc_dac/hx710b');

const GPIO_CHIP  = parseInt(process.env.GPIO_CHIP     || '0',  10);
const DOUT_LINE   = parseInt(process.env.HX710B_DOUT   || '5',  10);
const PD_SCK_LINE = parseInt(process.env.HX710B_PD_SCK || '6',  10);
...
const chip = new HX710BFull(connection);
...
const suppRaw = chip.readSupplyDiffRaw();
checkTrue('readSupplyDiffRaw returns number', typeof suppRaw === 'number');
checkTrue('readSupplyDiffRaw in 24-bit signed range', suppRaw >= -8388608 && suppRaw <= 8388607);
