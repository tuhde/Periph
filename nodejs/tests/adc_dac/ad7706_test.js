...
const { AD7706Full, AD7706Minimal } = require('../../packages/periph/src/chips/adc_dac/ad7706');
...
async function main() {
    ...
    const adcMin = new AD7706Minimal(connection, 2.5, 2_457_600);
    const adc = new AD7706Full(connection, 2.5, 2_457_600);
    ...
    const raw3 = await adc.readRawChannel(3);
    checkTrue('Full.readRaw(3) returns int', Number.isInteger(raw3));
    checkTrue('Full.readRaw(3) in [0, 65535]', raw3 >= 0 && raw3 <= 65535);

    const v3 = await adc.readVoltageChannel(3);
    checkTrue('Full.readVoltage(3) returns number', typeof v3 === 'number');
    ...
    await adc.configure(3, 4, false, true, 60);
    ...
    const off3 = await adc.getOffsetCalibration(3);
    checkTrue('getOffsetCalibration(3) returns int', Number.isInteger(off3));
    const gain3 = await adc.getGainCalibration(3);
    checkTrue('getGainCalibration(3) returns int', Number.isInteger(gain3));
