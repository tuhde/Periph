...
module.exports = function(RED) {
    ...
    const THRESHOLD_SOURCES = {
        level_low: VL53L1XFull.SOURCE_LEVEL_LOW,
        level_high: VL53L1XFull.SOURCE_LEVEL_HIGH,
        out_of_window: VL53L1XFull.SOURCE_OUT_OF_WINDOW,
        in_window: VL53L1XFull.SOURCE_IN_WINDOW,
    };
    const SOURCE_NAMES = {};
    for (const [name, value] of Object.entries(THRESHOLD_SOURCES)) SOURCE_NAMES[value] = name;

    function VL53L1XNode(config) {
        ...
        const distanceMode = config.distanceMode === 'short' ? 'short' : 'long';
        const budgetUs = (parseInt(config.timingBudget, 10) || 100) * 1000;
        const roiWidth = parseInt(config.roiWidth, 10) || 16;
        const roiHeight = parseInt(config.roiHeight, 10) || 16;
        ...
        try {
            const address = parseInt(config.address, 16) || VL53L1XFull.I2C_ADDRESS;
            let intPin = null;
            ...
            node.driver = new VL53L1XFull(connection);
            node.connection = connection;

            serial(async () => {
                await node.driver.init();
                await node.driver.setDistanceMode(distanceMode);
                await node.driver.setTimingBudget(budgetUs);
                await node.driver.setRoi(roiWidth, roiHeight);
                if (threshold) {
                    ...
                    }).catch((e) => node.error('VL53L1X interrupt read failed: ' + e.message)));
                }
                ...
                node.error('VL53L1X init failed: ' + e.message);
            });
        } catch (e) {
            node.error('VL53L1X init failed: ' + e.message);
        }
        ...
        node.on('input', function(msg, send, done) {
            ...
            serial(async () => {
                ...
                msg.payload = {
                    ...
                    valid: m.rangeStatus === VL53L1XFull.RANGE_STATUS_VALID,
    ...
    RED.nodes.registerType('periph-vl53l1x', VL53L1XNode);
};
