...
dac.set_raw(2, 3000);                              // Set channel C raw code, (channel=0–3, code=0–4095) → None
...
dac.set_voltage_eeprom(0, 0.5, 0, 1);              // Set channel A and persist to EEPROM, (channel=0–3, fraction, vref=0/1, gain=1/2) → None
                                                    // Single Write updates DAC register and nonvolatile EEPROM
dac.set_raw_eeprom(1, 2048, 0, 1);                 // Set channel B raw code and persist, (channel=0–3, code, vref=0/1, gain=1/2) → None
...
const state = dac.read();                          // Read all four channels' DAC and EEPROM state, () → {channel[], eeprom_ready}
