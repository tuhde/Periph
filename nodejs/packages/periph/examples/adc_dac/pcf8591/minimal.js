...
const adc = new PCF8591Minimal(connection);

(async () => {
    const ch0 = await adc.read_channel(0);                  // Read single channel, (channel=0–3) → number
    const allRaw = await adc.read_all();                    // Read all four channels, () → number[]
