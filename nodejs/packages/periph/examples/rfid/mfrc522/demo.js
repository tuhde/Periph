...
const connection = new SPIConnection(0, 0, { maxSpeedHz: 1_000_000, mode: 0 });
...
(async () => {
    ...
    const uid = await mfrc.selectCard();                                           // Anticollision/Select only, () → Buffer | null
    if (!uid) {
        ...
        if (!(await mfrc.authenticate(CREDITS_BLOCK, MFRC522Full.KEY_A, factoryKey, uid.subarray(0, 4)))) { // MFAuthent, (block, key, uid) → bool
            console.log('authentication failed');
        } else {
            // --- Read the current value block; initialise it if unprogrammed ---
            let block = await mfrc.readBlock(CREDITS_BLOCK);                        // Read 16-byte block, (blockAddress) → Buffer
            ...
            if (allZero) {
                ...
                await mfrc.writeBlock(CREDITS_BLOCK, valueBlock);                   // Write 16 bytes, (block, data=16 B) → bool
                await mfrc.restoreValue(CREDITS_BLOCK);                            // Restore + Transfer, (block) → bool
                                                                                   // normalises the value-block layout
            }

            // --- "Spend" one credit; refuse if balance is zero ---
            block = await mfrc.readBlock(CREDITS_BLOCK);                           // Read current value, (block) → Buffer
            if (block) {
                const credits = block.readUInt32LE(0);
                if (credits <= 0) {
                    console.log('Access denied — no credits remaining');
                } else {
                    await mfrc.decrementValue(CREDITS_BLOCK, 1);                   // Decrement + Transfer, (block, delta) → bool
                    const updated = await mfrc.readBlock(CREDITS_BLOCK);            // Read updated value, (block) → Buffer
                    if (updated) {
