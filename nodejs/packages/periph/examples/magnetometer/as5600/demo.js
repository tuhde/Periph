...
const as5600 = new AS5600Full(connection);
...
(async () => {
    let prevStatus = await as5600.statusByte();

    for (let n = 0; n < 10; n++) {
        const a = await as5600.angle();                        // Read absolute angle, () → float degrees
        const r = await as5600.rawAngle();                     // Read raw unscaled angle, () → int 0-4095
        const g = await as5600.agc();                          // Read AGC value, () → int

        // --- Check for status changes (magnet inserted/removed) ---
        const status = await as5600.statusByte();
        if (status !== prevStatus) {
            if (!(await as5600.isMagnetDetected())) {
                console.log('[MAGNET REMOVED] MD=0');
            } else {
                console.log('[MAGNET DETECTED] MD=1  MH=%d  ML=%d',
                    await as5600.isMagnetTooStrong(), await as5600.isMagnetTooWeak());
            }
            prevStatus = status;
        }
