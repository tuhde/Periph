...
const fm = new RDA5807MFull(connection, 87.5, 10);
...
(async () => {
    ...
    const stations = [];
    while (true) {
        const freq = await fm.seek(true);
        if (freq === null) break;
        if (!(await fm.isStation())) continue;

        const rssi = await fm.signalStrength();
        const stereo = (await fm.isStereo()) ? 'stereo' : 'mono';
        let name = null;
        ...
        while (Date.now() < deadline) {
            if (await fm.rdsReady()) {
                const group = await fm.readRdsGroup();
                if (group !== null) {
