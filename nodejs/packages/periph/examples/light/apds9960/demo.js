...
let atime = 0xB6;
(async () => {
    await apds.configureAls(atime, 1);                      // Configure ALS, (atime 0-255, again 0-3) → void
})();

setInterval(async () => {
    while (!(await apds.isAlsValid())) {}                  // Check ALS data valid, () → boolean
    ...
    if ((await apds.isAlsSaturated()) && atime < 0xFE) {    // Check ALS saturated, () → boolean
