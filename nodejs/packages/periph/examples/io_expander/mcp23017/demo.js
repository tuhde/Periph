...
async function main() {
    ...
    await chip.configurePullup(1, 0b01111111);                      // Configure pull-ups, (port=1, mask) → Promise<void>

    // --- Wire interrupt for responsive button detection ---
    await chip.onInterruptPort(1, (changed) => {                    // Subscribe to INT on one port, (port, callback, options) → Promise<void>
    ...
    setInterval(async () => {
        ...
        await chip.writePort(0, output);                            // Write all 8 pins, (port=0, mask) → Promise<void>
