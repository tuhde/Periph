...
(async () => {
    ...
    while (true) {                                       // Wait for valid data, () → blocks until warm
        try { await sensor.readAirQuality(); break; } catch (e) {
            const s = await sensor.status();
    ...
    await sensor.setCompensation(22.0, 45.0);            // Set compensation, (tempCelsius=22.0, rhPercent=45.0) → undefined
