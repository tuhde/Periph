...
(async () => {
    ...
    for (let n = 0; n < 30; n++) {
        ...
        const a = await bmp.altitude();                  // Compute altitude, (seaLevelHpa=1013.25) → number m
        temps.push(t);
        pressures.push(p);
        alts.push(a);
    ...
    await bmp.configure(BMP280Full.OSRS_X2, BMP280Full.OSRS_X16, BMP280Full.MODE_NORMAL, BMP280Full.FILTER_16, BMP280Full.T_SB_0_5_MS);  // Configure chip, (osrsT=×2, osrsP=×16, mode=normal, filter=16, tSb=0.5ms) → undefined
