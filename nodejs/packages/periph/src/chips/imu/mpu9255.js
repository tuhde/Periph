...
/**
 * MPU-9255 9-axis MotionTracking device (accelerometer + gyroscope) — minimal interface.
 *
...
class MPU9255Minimal {
    ...
    async _init() {
        ...
        if (who !== _WHO_AM_I_VALUE) {
            throw new Error('MPU9255 WHO_AM_I: expected 0x' + _WHO_AM_I_VALUE.toString(16) + ', got 0x' + who.toString(16));
        }
...
class MPU9255Full extends MPU9255Minimal {
    /**
     * @param {import('../../connection/connection').Connection} connection - Configured I²C or SPI connection pointing at the MPU-9255.
     ...
     *
     * @param {number} [thresholdMg=64] - Motion threshold in milligrams (4–1020 mg, quantized to 4 mg steps).
     * @param {number} [odrHz=31.25] - Wake-up output data rate in Hz (0.24–500 Hz).
     * @returns {Promise<void>}
     */
    async configureWakeOnMotion(thresholdMg = 64, odrHz = 31.25) {
        let thresholdLsb = Math.round(thresholdMg / 4.0);
        ...
        let bestSel = 0;
        let bestDiff = Math.abs(odrHz - lposcTable[0]);
        for (let sel = 1; sel < lposcTable.length; ++sel) {
            const diff = Math.abs(odrHz - lposcTable[sel]);
            if (diff < bestDiff) {
                bestDiff = diff;
                bestSel = sel;
            }
        ...
        await this._writeReg(_REG_LP_ACCEL_ODR, bestSel & 0x0F);
        await this._writeReg(_REG_PWR_MGMT_1, 0x21);
    }

    /**
     * Check if a wake-on-motion interrupt has fired.
     * @returns {Promise<boolean>} True when WOM_INT (bit 6) is set in INT_STATUS.
     */
    async motionDetected() {
        return !!((await this._readReg(_REG_INT_STATUS)) & 0x40);
    }
}

module.exports = { MPU9255Minimal, MPU9255Full };