 ...
 *
 * @param {import('../../connection/connection').Connection} connection - Configured I²C connection pointing at the device (0x48–0x4F).
 */
class PCF8591Minimal {
     ...
     * @returns {Promise<number>} Raw 8-bit value (0–255).
     */
    async read_channel(channel) {
        let ch = (channel >= 0 && channel < NUM_CHANNELS) ? channel : 0;
        const ctrl = CONTROL_DEFAULT | (ch & 0x03);
        await this._conn.write(Buffer.from([ctrl]));
        const buf = await this._conn.read(2);
        return buf[1];
    }
     ...
     *
     * @returns {Promise<number[]>} Four raw 8-bit values [ch0, ch1, ch2, ch3].
     */
    async read_all() {
        const ctrl = CONTROL_DEFAULT | 0x04;  // AI=1
        await this._conn.write(Buffer.from([ctrl]));
        const buf = await this._conn.read(NUM_CHANNELS + 1);
...
class PCF8591Full extends PCF8591Minimal {
     ...
     */
    async configure(input_mode, auto_increment, dac_enabled) {
        ...
        await this._conn.write(Buffer.from([this._control]));
    }
     ...
     * @returns {Promise<number>} Channel voltage in volts.
     */
    async read_channel_voltage(channel, vref, vagnd) {
        const raw = await this.read_channel(channel);
     ...
     * @param {number} vref - Reference voltage in volts.
     * @param {number} vagnd - Analog ground voltage in volts.
     * @returns {Promise<number[]>} Four channel voltages in volts [ch0, ch1, ch2, ch3].
     */
    async read_all_voltage(vref, vagnd) {
        const raws = await this.read_all();
     ...
     * @returns {Promise<number>} Signed 8-bit value (-128 to 127).
     */
    async read_differential(channel) {
    ...
    async set_dac(value) {
        ...
        await this._conn.write(Buffer.from([ctrl, v]));
    }
     ...
     * @returns {Promise<void>}
     */
    async set_dac_voltage(voltage_fraction) {
        let f = voltage_fraction;
        if (f < 0.0) f = 0.0;
        if (f > 1.0) f = 1.0;
        const value = Math.round(f * 255);
        await this.set_dac(value);
    }
