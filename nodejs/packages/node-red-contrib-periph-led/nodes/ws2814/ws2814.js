...
module.exports = function(RED) {
    const { NeoPixelConnection }  = require('periph/src/connection/neopixel');
    const { WS2814Full }         = require('periph/src/chips/led/ws2814');

    function WS2814DeviceNode(config) {
        RED.nodes.createNode(this, config);
        const node = this;
        try {
            node.connection = new NeoPixelConnection(
                parseInt(config.spiBus),
                parseInt(config.spiDevice)
            );
            node.driver = new WS2814Full(node.connection, parseInt(config.pixelCount));
            node.driver.brightness = Math.max(0, Math.min(255, parseInt(config.brightness) || 255));
        } catch (e) {
            node.error('WS2814 init failed: ' + e.message);
        }
    ...
    RED.nodes.registerType('ws2814-device', WS2814DeviceNode);

    function WS2814WriteNode(config) {
        RED.nodes.createNode(this, config);
        const node = this;
        ...
        node.on('input', async function(msg, send, done) {
            if (!node.device || !node.device.driver) {
                node.error('No WS2814 device configured', msg);
                done();
                return;
            }
            ...
            try {
                if (typeof payload === 'object' && payload !== null) {
                    if (payload.command === 'off') {
                        ...
                        await strip.fill(payload.r || 0, payload.g || 0,
                                   payload.b || 0, payload.w || 0);
                    }
                }
                done();
    ...
    RED.nodes.registerType('periph-ws2814', WS2814WriteNode);
};
