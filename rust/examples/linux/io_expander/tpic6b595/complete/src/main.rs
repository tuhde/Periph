...
fn main() {
    ...
    let srclr = srclr_line.map(|o| CdevPin::open("/dev/gpiochip0", o, true).expect("open srclr"));
    let g     = g_line.map(|o| CdevPin::open("/dev/gpiochip0", o, false).expect("open g"));

    // --- Tpic6b595Minimal ---
    let rck_min = CdevPin::open("/dev/gpiochip0", rck_line, false).expect("open rck line (minimal)");
    let chip1 = Tpic6b595Minimal::new(open_spi(), rck_min, None, None, 1)             // Create TPIC6B595 minimal driver, (spi, rck, srclr=None, g=None, num_devices=1) → Result
    ...
    // --- Tpic6b595Full ---
    let rck_full = CdevPin::open("/dev/gpiochip0", rck_line, false).expect("open rck line (full)");
    let srclr_full = srclr_line.map(|o| CdevPin::open("/dev/gpiochip0", o, true).expect("open srclr (full)"));
    let g_full     = g_line.map(|o| CdevPin::open("/dev/gpiochip0", o, false).expect("open g (full)"));
    ...
    if srclr.is_some() { chip2.clear().expect("clear"); }                           // Pulse SRCLR, () → Result<(), E>
