...
use periph::chips::adc_dac::{AD7706Full, GAIN_8, MCLK_2_4576MHZ};
...
fn main() {
    ...
    chip.configure(3, GAIN_8, true, true, 60).expect("configure ch3");                      // Configure channel 3, (channel=3, gain=GAIN_8, bipolar=true, buffered=true, output_rate_hz=60) → Result
    chip.self_calibrate(3).expect("self calibrate ch3");                                   // Self-calibrate channel, (channel=3) → Result
    ...
    let v3 = chip.read_voltage_channel(3).expect("v3");                                     // Read voltage, (channel=3) → Result<f32>
    println!("ch1 raw={} ch1 v={} ch2 v={} ch3 v={}", raw1, v1, v2, v3);
