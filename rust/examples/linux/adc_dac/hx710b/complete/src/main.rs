...
fn main() {
    ...
    let dout_handle = chip.get_line(dout_offset).expect("get dout line")
        .request(LineRequestFlags::INPUT, 0, "hx710b_complete").expect("request dout");
    let sck_handle = chip.get_line(sck_offset).expect("get sck line")
        .request(LineRequestFlags::OUTPUT, 0, "hx710b_complete").expect("request pd_sck");
    ...
    loop {
        ...
        let supp_raw = chip.read_supply_diff_raw().expect("read_supply_diff_raw");  // Read raw DVDD−AVDD supply-difference code, () → Result<i32, _>
                                                                         // uncalibrated ADC code, no absolute LSB-to-volts scale
        println!("raw={} avg={} offset={} scale={:.1} weight={:.1} supp_raw={}", raw, avg, offset, scale, weight, supp_raw);
