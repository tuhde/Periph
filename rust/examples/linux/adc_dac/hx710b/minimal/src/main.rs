...
use periph::chips::adc_dac::Hx710bMinimal;
...
fn main() {
    ...
    let dout_handle = chip.get_line(dout_offset).expect("get dout line")
        .request(LineRequestFlags::INPUT, 0, "hx710b_minimal").expect("request dout");
    let sck_handle = chip.get_line(sck_offset).expect("get sck line")
        .request(LineRequestFlags::OUTPUT, 0, "hx710b_minimal").expect("request pd_sck");
    ...
    let mut chip = Hx710bMinimal::new(connection).expect("init HX710B");  // Create HX710B driver — discards first conversion, (connection) → Result<Hx710bMinimal, _>
