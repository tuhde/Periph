...
fn main() {
    ...
    let spi = SpidevBus::open(format!("/dev/spidev{}.{}", spi_bus, spi_device))
        .expect("open spidev");
    let mut strip = Sk6812RgbwFull::new(spi, 8);                    // Create SK6812RGBW full driver, (spi, n=8 pixels) → Self
                                                                    // allocates internal GRBW buffer of n*4 bytes; default brightness = 255
    ...
    println!("SK6812RGBW complete example done");
}
