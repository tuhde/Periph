...
fn main() -> ! {
    ...
    let sv_ok = chip.shunt_voltage().map(|v| v >= -0.32 && v <= 0.32).unwrap_or(false);
    ...
    chip.configure(1, 3, 3, 3, 7).ok();
    check_true!(chip.voltage().map(|v| v >= 0.0).unwrap_or(false), "voltage_after_configure", passed, failed);
