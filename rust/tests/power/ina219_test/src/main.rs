...
fn main() {
    ...
    check_true!(sv >= -0.320 && sv <= 0.320, "shunt_voltage_range", passed, failed);
    ...
    chip.wake().unwrap();
    check_true!(chip.voltage().unwrap() >= 0.0, "shutdown_wake_voltage_ok", passed, failed);

    chip.reset().unwrap();
    check_true!(chip.voltage().unwrap() >= 0.0, "reset_voltage_ok", passed, failed);
