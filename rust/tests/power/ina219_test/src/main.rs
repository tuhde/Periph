...
fn main() {
    ...
    check_true!(v >= 0.0 && v <= 32.0, "voltage_range", passed, failed);
    ...
    check_true!(chip.voltage().unwrap() >= 0.0, "configure: voltage valid", passed, failed);

    chip.shutdown().unwrap();
    chip.wake().unwrap();
    check_true!(chip.voltage().unwrap() >= 0.0, "shutdown_wake: voltage valid", passed, failed);

    chip.reset().unwrap();
    check_true!(chip.voltage().unwrap() >= 0.0, "reset: voltage valid", passed, failed);
