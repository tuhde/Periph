...
fn main() {
    ...
    // --- Ws2812bMinimal ---
    {
        ...
        check_true!(strip.fill(255, 0, 0).is_ok(), "fill(255,0,0) accepted", passed, failed);
        check_true!(strip.fill(0, 255, 0).is_ok(), "fill(0,255,0) accepted", passed, failed);
        check_true!(strip.fill(0, 0, 255).is_ok(), "fill(0,0,255) accepted", passed, failed);
    ...
    // --- Ws2812bFull ---
    {
        let spi = SpidevBus::open(format!("/dev/spidev{}.{}", spi_bus, spi_device))
            .expect("open spidev");
        let mut strip = Ws2812bFull::new(spi, 8);
        ...
        check_true!(strip.show().is_ok(), "set_pixel at last index + show accepted", passed, failed);
