...
fn main() {
    ...
    if mode == "hw" {
        ...
        let mut connection =
            SiPoConnection::new(bus, rck, Some(srclr), Some(g)).expect("new connection");
        run_checks(&mut connection, &mut passed, &mut failed);
    } else {
...
fn run_checks<SPI, RCK, SRCLR, G>(
    connection: &mut SiPoConnection<SPI, RCK, SRCLR, G>,
    passed: &mut i32,
    failed: &mut i32,
) where
    ...
    G: OutputPin,
{
    check_true!(connection.write(&[0xA5]).is_ok(), "write accepted", *passed, *failed);
    check_true!(
        connection.write(&[0x00, 0xFF]).is_ok(),
        "write multi-byte accepted",
        *passed,
        *failed
    );
    check_true!(connection.clear().is_ok(), "clear accepted", *passed, *failed);
    check_true!(
        connection.set_output_enable(false).is_ok(),
        "set_output_enable(false) accepted",
        *passed,
        *failed
    );
    check_true!(
        connection.set_output_enable(true).is_ok(),
