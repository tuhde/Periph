...
fn main() -> ! {
    ...
    let mut connection = match SmBusConnection::new(i2c, TEST_ADDR, false) {
        Ok(t) => t,
        Err(_) => {
            println!("FAIL init: could not create SmBusConnection at 0x{:02X}", TEST_ADDR);
    ...
    let mut buf = [0u8; 1];
    check_true!(connection.read(TEST_ADDR, &mut buf).is_ok(), "read_ok", passed, failed);
    check_true!(connection.write(TEST_ADDR, &[0x00]).is_ok(), "write_ok", passed, failed);
    check_true!(
        connection.write_read(TEST_ADDR, &[0x00], &mut buf).is_ok(),
        "write_read_ok",
        passed,
        failed
    );
    ...
    check_true!(
        connection_pec.write(TEST_ADDR, &[0x00]).is_ok(),
        "write_with_pec_ok",
        passed,
        failed
    );
