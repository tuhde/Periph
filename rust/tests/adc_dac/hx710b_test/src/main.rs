...
use periph::chips::adc_dac::{Hx710bFull, Hx710bMinimal};
...
fn main() {
    ...
    let dout_offset: u32 = std::env::var("HX710B_DOUT")
        .ok()
        .and_then(|v| v.parse().ok())
        .unwrap_or(5);
    let pd_sck_offset: u32 = std::env::var("HX710B_PD_SCK")
        .ok()
        .and_then(|v| v.parse().ok())
        .unwrap_or(6);
    ...
    // --- Hx710bMinimal ---
    {
        let dout_handle = chip.get_line(dout_offset).expect("get dout line")
            .request(LineRequestFlags::INPUT, 0, "hx710b_test_minimal").expect("request dout");
        let pd_sck_handle = chip.get_line(pd_sck_offset).expect("get pd_sck line")
            .request(LineRequestFlags::OUTPUT, 0, "hx710b_test_minimal").expect("request pd_sck");
        ...
        let mut minimal = Hx710bMinimal::new(connection).expect("init Hx710bMinimal");
    ...
    {
        let dout_handle = chip.get_line(dout_offset).expect("get dout line")
            .request(LineRequestFlags::INPUT, 0, "hx710b_test_full").expect("request dout");
        let pd_sck_handle = chip.get_line(pd_sck_offset).expect("get pd_sck line")
            .request(LineRequestFlags::OUTPUT, 0, "hx710b_test_full").expect("request pd_sck");

        let dout   = CdevPin::new(dout_handle).expect("dout pin");
        let pd_sck = CdevPin::new(pd_sck_handle).expect("pd_sck pin");

        let connection = HX711Connection::new(dout, pd_sck);
        let mut full = Hx710bFull::new(connection).expect("init Hx710bFull");
        ...
        let supp_raw = full.read_supply_diff_raw();
        check_true!(supp_raw.is_ok(), "read_supply_diff_raw ok", passed, failed);
        if let Ok(v) = supp_raw {
            check_true!(
                v >= -8_388_608 && v <= 8_388_607,
                "read_supply_diff_raw in 24-bit signed range",
                passed,
                failed
            );
        }
