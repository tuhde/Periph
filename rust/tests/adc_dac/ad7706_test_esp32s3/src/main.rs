...
fn main() -> ! {
    ...
    if raw3 <= 65535 { println!("PASS read_raw_channel3_in_range"); passed += 1; }
    else             { println!("FAIL read_raw_channel3_in_range"); failed += 1; }

    let v3 = chip.read_voltage_channel(3).expect("read voltage ch3");
    if v3 >= -2.5 && v3 <= 2.5 { println!("PASS read_voltage_channel3_in_range"); passed += 1; }
    else                       { println!("FAIL read_voltage_channel3_in_range"); failed += 1; }
    ...
    if off3 <= 0xFFFFFF { println!("PASS get_offset_calibration_ch3_in_range"); passed += 1; }
    else                { println!("FAIL get_offset_calibration_ch3_in_range"); failed += 1; }
    let gain3 = chip.get_gain_calibration(3).expect("gain3");
    if gain3 <= 0xFFFFFF { println!("PASS get_gain_calibration_ch3_in_range"); passed += 1; }
    else                 { println!("FAIL get_gain_calibration_ch3_in_range"); failed += 1; }
