//! DHTxx single-wire connection for chip drivers.
//!
...
//! response. The connection handles all GPIO direction switching, timing, and
...
//! connection cannot be generic over a standard trait. Two platform-specific
//! structs are provided:
//!
//! - [`DHTxxConnectionLinux`] — Linux host. Uses `std::thread::sleep` for the
//!   20 ms start delay and a busy-loop for µs-scale measurement.
//! - [`DHTxxConnectionEsp32s3`] — ESP32-S3 bare-metal. The caller provides a
//!   delay primitive via [`DHTxxConnectionEsp32s3::read_with_delay`].
//!
//! ## Protocol summary
//!
...
/// Error type for DHTxx connection operations.
#[derive(Debug)]
...
/// Linux DHTxx connection. Owns a single pin and switches its direction as
    ...
    P: embedded_hal::digital::OutputPin + InputPin,
{
    ...
    /// Consume the connection and return the pin.
...
/// ESP32-S3 DHTxx connection. Caller provides a delay primitive that the
/// connection uses to time the 20 ms host-start pulse and the µs-scale
