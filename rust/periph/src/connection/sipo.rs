//! SiPo (serial-in/parallel-out shift register) connection for chip drivers.
//!
//! Drives cascadable SIPO shift registers (TPIC6B595, SN74HC595, etc.) whose
//! SER IN/SRCK pins are electrically an SPI MOSI/SCK pair. The connection is
...
//! are interchangeable, since the connection's type signature doesn't
...
//!
//! ```rust,ignore
//! use periph::connection::sipo::SiPoConnection;
//!
//! let mut connection = SiPoConnection::new(spi, rck, Some(srclr), Some(g))?;
//! connection.write(&[0xA5])?;
//! connection.clear()?;
...
/// Error type for [`SiPoConnection`] operations.
#[derive(Debug)]
pub enum SiPoError<SE, RE, CE, GE> {
    ...
    G(GE),
    /// [`SiPoConnection::clear`] was called but SRCLR was not configured.
    SrclrNotConfigured,
    /// [`SiPoConnection::set_output_enable`] was called but G was not configured.
    GNotConfigured,
}

/// SiPo (serial-in/parallel-out shift register) connection.
///
...
/// SRCLR/G.
pub struct SiPoConnection<SPI, RCK, SRCLR, G> {
    spi: SPI,
    rck: RCK,
    srclr: Option<SRCLR>,
    g: Option<G>,
...
impl<SPI, RCK, SRCLR, G> SiPoConnection<SPI, RCK, SRCLR, G>
where
    SPI: SpiBus,
    RCK: OutputPin,
    SRCLR: OutputPin,
    G: OutputPin,
{
    /// Create a new connection: drive RCK LOW, SRCLR HIGH (if configured), and
    /// G LOW (if configured).
    ///
    /// # Arguments
    ///
    ...
    pub fn new(
        ...
        Ok(Self { spi, rck, srclr, g, enabled: true })
    }

    /// Resume writes.
    pub fn enable(&mut self) { self.enabled = true; }

    /// Gate [`SiPoConnection::write`]/[`SiPoConnection::clear`]/
    /// [`SiPoConnection::set_output_enable`] — they become no-ops while disabled.
    ...
    /// the next [`SiPoConnection::write`]. No-op if this connection is disabled.
    ///
    /// # Errors
    ///
    ...
    /// Consume the connection and return the SPI bus and pins.
