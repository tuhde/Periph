//! SMBus connection wrapper for chip drivers.
//!
...
//! [`SmBusConnection`] directly without knowing about SMBus specifics.
//!
//! ## Usage
//!
//! ```rust,ignore
//! use linux_embedded_hal::I2cdev;
//! use periph::connection::smbus::SmBusConnection;
//!
//! let i2c = I2cdev::new("/dev/i2c-1")?;
//! let smbus = SmBusConnection::new(i2c, 0x40, false)?;
...
/// Error type returned by [`SmBusConnection`] operations.
#[derive(Debug)]
...
/// `SmBusConnection` directly. The per-call `address` parameter in each
/// [`I2c`] method is used for PEC computation.
pub struct SmBusConnection<I2C> {
    i2c: I2C,
    pec: bool,
}

impl<I2C: I2c> SmBusConnection<I2C> {
    /// Create a new SMBus connection, validating the device address range.
    ///
    ...
    /// Release the underlying I²C bus, consuming the connection.
    pub fn release(self) -> I2C {
        self.i2c
    }
}

impl<I2C: I2c> ErrorType for SmBusConnection<I2C> {
    type Error = SmBusError<I2C::Error>;
}

impl<I2C: I2c> I2c for SmBusConnection<I2C> {
