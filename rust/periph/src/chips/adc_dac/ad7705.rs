//! AD7705 2-channel, 16-bit sigma-delta ADC driver (Analog Devices).
//!
//! Communicates over SPI (Mode 3, up to 5 MHz, MSB first). Two-phase register
//! access: write the Communication Register (selects target register +
//! read/write direction + channel), then transfer the data bytes in a single
//! CS-held transaction. `DRDY` is polled over SPI by inspecting bit 7 of the
//! Communication Register, matching the datasheet's 3-wire microcontroller
...
pub struct AD7705Minimal<SPI: SpiDevice> {
    spi: SPI,
    vref: f32,
    mclk_hz: u32,
    gain: u8,
...
///
/// Wraps [`AD7705Minimal`] via composition (Rust has no inheritance); one-line
/// forwards expose the inherited API. No register-level logic is duplicated.
pub struct AD7705Full<SPI: SpiDevice> {
    inner: AD7705Minimal<SPI>,
}

impl<SPI: SpiDevice> AD7705Full<SPI> {
    /// Construct and initialise the AD7705.
    pub fn new(spi: SPI, vref: f32, mclk_hz: u32) -> Result<Self, SPI::Error> {
