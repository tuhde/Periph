//! MPU-9250 — 9-axis MotionTracking device (accelerometer + gyroscope + magnetometer).
//!
...
/// MPU-9250 minimal driver — 3-axis acceleration and 3-axis angular rate.
///
/// Performs device reset, WHO_AM_I check, and configures defaults at construction.
/// Magnetometer is not included in Minimal — it requires a separate initialization path.
pub struct Mpu9250Minimal<I2C> {
    i2c: I2C,
    addr: u8,
    accel_fs: u8,
    gyro_fs: u8,
...
impl<I2C: I2c> Mpu9250Minimal<I2C> {
    /// Create a new `Mpu9250Minimal`, reset the device, and configure defaults.
    ///
    /// # Arguments
...
/// MPU-9250 full driver — extends [`Mpu9250Minimal`] with complete functionality.
///
/// Provides access to full-scale range configuration, DLPF, sample rate,
/// temperature, magnetometer (AK8963), raw data, data-ready polling, sleep/standby, and FIFO management.
pub struct Mpu9250Full<I2C> {
    inner: Mpu9250Minimal<I2C>,
    mag_enabled: bool,
    mag_bits: u8,
    mag_scale_x: f32,
    mag_scale_y: f32,
...
impl<I2C: I2c> Mpu9250Full<I2C> {
    /// Create a new `Mpu9250Full`, reset the device, and configure defaults.
    ///
    /// Same arguments as [`Mpu9250Minimal::new`].
    pub fn new<D: DelayNs>(i2c: I2C, addr: u8, delay: &mut D) -> Result<Self, I2C::Error> {
        let inner = Mpu9250Minimal::new(i2c, addr, delay)?;
        Ok(Self {
            inner,
            mag_enabled: false,
            mag_bits: 16,
    ...
    /// Read 3-axis acceleration. Delegates to the inner [`Mpu9250Minimal`].
    pub fn accel(&mut self) -> Result<(f32, f32, f32), I2C::Error> {
        self.inner.accel()
    }

    /// Read 3-axis angular rate. Delegates to the inner [`Mpu9250Minimal`].
    pub fn gyro(&mut self) -> Result<(f32, f32, f32), I2C::Error> {
        self.inner.gyro()
    }
...
mod tests {
    ...
    fn full_api() {
        ...
        let mut sensor = Mpu9250Full::new(i2c, ADDR, &mut delay).expect("init");
    ...
    fn who_am_i_mismatch_panics() {
        ...
        let i2c = I2cMock::new(&transactions);
        let mut delay = NoopDelay::new();
        let _ = Mpu9250Minimal::new(i2c, ADDR, &mut delay);
    }
}