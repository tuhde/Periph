//! BMP085 — piezo-resistive pressure + temperature sensor (Bosch Sensortec).
//!
//! Communicates over I²C (fixed address 0x77). The compensation algorithm
//! is implemented exactly as documented in the datasheet (Figure 3).
//!
//! ## OSS constants
//!
//! Pass one of [`OSS_ULP`], [`OSS_STANDARD`], [`OSS_HIGH_RES`], or [`OSS_ULTRA_HIGH_RES`]
//! to [`Bmp085Full::new`] or [`Bmp085Full::set_oversampling`].
...
/// BMP085 minimal driver — temperature (°C) and pressure (Pa).
///
/// Default OSS = 0 (Ultra Low Power, 4.5 ms conversion).
pub struct Bmp085Minimal<I2C> {
    i2c: I2C,
    addr: u8,
    oss: u8,
...
impl<I2C: I2c> Bmp085Minimal<I2C> {
    /// Create a new `Bmp085Minimal` and read calibration coefficients.
    ///
    /// # Arguments
...
/// BMP085 full driver — extends minimal with OSS control and altitude helpers.
pub struct Bmp085Full<I2C> {
    inner: Bmp085Minimal<I2C>,
}

impl<I2C: I2c> Bmp085Full<I2C> {
    /// Create a new `Bmp085Full`.
    ///
    /// # Arguments
    /// * `i2c` — Configured I²C bus.
    /// * `oss` — Oversampling mode 0–3 (default 0 = ULP).
    pub fn new(i2c: I2C, addr: u8, oss: u8) -> Result<Self, I2C::Error> {
        let mut inner = Bmp085Minimal::new(i2c, addr)?;
        inner.oss = oss & 0x03;
        Ok(Self { inner })
    }
    ...
    ///
    /// # Arguments
    /// * `sea_level_pa` — Reference sea-level pressure in Pa (default 101325.0).
    ///
    ...
    ///
    /// Returns sea-level pressure in Pa.
...
mod tests {
    ...
    // Datasheet worked example: AC1=408, AC2=-72,
    ...
    fn full_api() {
        ...
        let mut sensor = Bmp085Full::new(i2c, ADDR, 0).expect("init");

        assert_eq!(sensor.temperature().unwrap(), 15.0);
        // Pressure in Pa for BMP085 (not hPa like BMP180)
        assert!((sensor.pressure().unwrap() - 82080.0).abs() < 1.0);
        ...
        assert!((slp - 83059.9010429).abs() < 0.5, "sea_level_pressure = {}", slp);
