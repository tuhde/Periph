...
def connection = new I2CConnection(bus, VL53L0XFull.DEFAULT_ADDRESS)                      // open I²C bus, device address, (bus, address=0x29) → I2CConnection
try {
    def sensor = new VL53L0XFull(connection)                                     // Create VL53L0X Full driver, (connection) → VL53L0XFull
                                                                                 // runs init: ID check, tuning, SPADs, VHV + phase calibration

    printf("model 0x%02X%n", sensor.modelId())                                   // Read model ID, () → int
                                                                                 // IDENTIFICATION_MODEL_ID, always 0xEE
    printf("revision 0x%02X%n", sensor.revisionId())                             // Read revision ID, () → int
    ...
    def m = sensor.readMeasurement()                                             // Read result block, () → Measurement
                                                                                 // distance, status, signal/ambient MCPS, SPAD count
    printf("signal %.2f MCPS, ambient %.2f MCPS%n", m.signalRateMcps, m.ambientRateMcps)
    ...
    println "signal limit ${sensor.signalRateLimit()} MCPS"                      // Read signal-rate limit, () → double MCPS
                                                                                 // 9.7 fixed point
    sensor.setSignalRateLimit(0.1d)                                              // Set signal-rate limit, (limitMcps MCPS) → void
    ...
    def original = sensor.offset()                                               // Read range offset, () → double mm
                                                                                 // NVM factory value, 0.25 mm steps
    sensor.setOffset(original - 5.0d)                                            // Set range offset, (offsetMm mm) → void
    ...
    sensor.setCrosstalkCompensation(0.0d)                                        // Set crosstalk compensation, (rateMcps MCPS) → void
                                                                                 // 0 = compensation off
    ...
    sensor.setInterruptThresholds(100, 800)                                      // Set distance thresholds, (lowMm mm, highMm mm) → void
                                                                                 // 2 mm resolution
    println "thresholds ${sensor.interruptThresholds() as List}"                 // Read distance thresholds, () → int[] {low mm, high mm}
    ...
    sensor.onInterrupt({ int status -> println "interrupt, source $status" } as IntConsumer)  // Subscribe to GPIO1, (callback) → void
...
def moved = new I2CConnection(bus, 0x30)                                         // open I²C bus, device address, (bus, address=0x30) → I2CConnection
try {
    def sensor = new VL53L0XFull(moved)                                          // Create VL53L0X Full driver, (connection) → VL53L0XFull
