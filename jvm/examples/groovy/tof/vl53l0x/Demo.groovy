...
try {
    ...
    def first = sensor.readMeasurement()                                         // Read result block, () → Measurement
    if (first.ambientRateMcps < 0.5d) {
        sensor.setProfile(VL53L0XFull.Profile.LONG_RANGE)                        // Apply ranging profile, (profile) → void
        printf("dark scene (%.2f MCPS ambient): long range profile%n", first.ambientRateMcps)
    } else {
        sensor.setProfile(VL53L0XFull.Profile.DEFAULT)                           // Apply ranging profile, (profile) → void
        printf("bright scene (%.2f MCPS ambient): default profile%n", first.ambientRateMcps)
    }
    ...
    def events = new ArrayBlockingQueue<Integer>(MAX_EVENTS)
    sensor.onInterrupt({ int status ->                                           // Subscribe to GPIO1, (callback) → void
        try {
    ...
    for (int n = 0; n < MAX_EVENTS; n++) {
        Integer d = events.poll(Math.max(1L, deadline - System.currentTimeMillis()), TimeUnit.MILLISECONDS)
        if (d == null) break
        println "${d < 100 ? 'ENTER' : 'LEAVE'} $d mm"
    }
    ...
    printf("mean %.0f mm, min %d mm, max %d mm, signal %.2f MCPS%n",
            (double) samples.sum() / samples.size(), samples.min(), samples.max(), rate / samples.size())
