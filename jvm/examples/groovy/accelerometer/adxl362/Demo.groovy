...
try (SPIConnection connection = new SPIConnection(bus, dev, 0, 8_000_000)) {    // Create SPI connection, (bus, dev, mode=0, maxSpeedHz=8e6) → SPIConnection
    ...
    while (System.currentTimeMillis() - start < 60_000L) {                       // Loop until 60 s elapsed, () → boolean
        boolean nowAwake = chip.awake()                                         // Read AWAKE bit, () → boolean
        if (lastAwake == null || nowAwake != lastAwake) {
            double secs = (System.currentTimeMillis() - start) / 1000.0
            println String.format("%6.2fs  %s", secs, nowAwake ? "AWAKE" : "asleep")
            transitions++
            lastAwake = nowAwake
        }
        Thread.sleep(200)                                                        // Sleep 200 ms between polls, () → void
    }
