...
try {
    ...
    def t = imu.temperature()                                    // Read die temperature, () → double °C
    ...
    def ra = imu.accelRaw()                                      // Read raw accel values, () → int[]
                                                        // returns raw 16-bit signed accelerometer register values
    def rg = imu.gyroRaw()                                       // Read raw gyro values, () → int[]
                                                        // returns raw 16-bit signed gyroscope register values
    def rm = imu.magRaw()                                        // Read raw mag values, () → int[]
                                                        // returns raw 16-bit signed magnetometer register values

    def ready = imu.dataReady()                                  // Check data ready flag, () → boolean
    ...
    Thread.sleep(50)
    def count = imu.fifoCount()                                  // Read FIFO byte count, () → int
                                                        // reads FIFO_COUNTH/L: number of bytes available
    def data = imu.readFifo()                                    // Read FIFO data, () → byte[]
    ...
    def motion = imu.motionDetected()                            // Check motion detected, () → boolean
                                                        // reads WOM_INT bit from INT_STATUS register (clears on read)

    printf("t=%.2f  mag=%.1f,%.1f,%.1f  ready=%s  motion=%s%n", t, m[0], m[1], m[2], ready, motion)
} finally {
    conn.close()
    magConn.close()
}