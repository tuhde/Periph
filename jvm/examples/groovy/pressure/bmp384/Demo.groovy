...
try {
    ...
    while (System.currentTimeMillis() - start < 30_000) {
        ...
        if (now >= next) {
            ...
            def altitude = 44330.0 * (1.0 - Math.pow(p / SEA_LEVEL_HPA, 1.0 / 5.255))
            def elapsed = (now - start) / 1000.0
            printf("%.1fs  %.2f hPa  %.1f C  %.1f m%n", elapsed, p, t, altitude)
            rows++
            next += 500
        }
        Thread.sleep(50)
    }
    printf("Sampled %d rows over 30 s%n", rows)
} finally {
    connection.close()
}
