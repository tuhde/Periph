...
try {
    def strip = new WS2814Full(connection, pixelCount)    // construct driver, (connection, n) → WS2814Full

    strip.fill(255, 0, 0, 0)                                          // fill entire strip red and send, (r=0–255, g=0–255, b=0–255, w=0–255) → void
                                                                       // fills buffer in identity RGBW order and calls connection.write()
    Thread.sleep(500)
    ...
    strip.setPixel(3, 0, 0, 0, 255)                                   // set pixel 3 white W channel (buffer only), (index, r=0–255, g=0–255, b=0–255, w=0–255) → void
    strip.show()                                                       // transmit buffer to strip, () → void
    Thread.sleep(500)

    strip.setPixels([[255,0,0,0],[0,255,0,0],[0,0,255,0],[0,0,0,255]] as List<int[]>)  // set first 4 pixels (buffer only), (colors List<int[]>) → void
