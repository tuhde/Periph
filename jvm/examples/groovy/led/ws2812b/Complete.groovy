...
try {
    ...
    try {
        ...
        strip.setPixels([[255,0,0] as int[], [0,255,0] as int[], [0,0,255] as int[], [255,128,0] as int[]]) // set first 4 pixels (buffer only), (colors: List<int[]>) → void
        ...
        strip.fillHsv(0.0, 1.0, 1.0)                                // fill strip hue=0° (red) and send, (h=0.0–1.0, s=0.0–1.0, v=0.0–1.0) → void
                                                                     // converts HSV to RGB, then calls fill() which transmits immediately
        Thread.sleep(500)

        strip.fillHsv(0.33, 1.0, 1.0)                               // fill strip hue=120° (green) and send, (h=0.0–1.0, s=0.0–1.0, v=0.0–1.0) → void
        Thread.sleep(500)

        strip.fillHsv(0.67, 1.0, 1.0)                               // fill strip hue=240° (blue) and send, (h=0.0–1.0, s=0.0–1.0, v=0.0–1.0) → void
        Thread.sleep(500)
