...
def chip1
try {
    chip1 = new Tpic6b595Minimal(connection)                       // construct minimal driver, (connection, numDevices=1) → Tpic6b595Minimal
                                                                    // initialises every output to OFF (shadow zeroed, latched once)

    def p0 = chip1.pin(0)                                          // get pin proxy, (n=0) → Pin
...
def connection2 = SiPoConnection.hardware(0, 0, 17, 16, 15)      // open SiPo connection, (bus, device, rckLine, srclrLine, gLine) → SiPoConnection
try {
    def chip2 = new Tpic6b595Full(connection2, 2)                  // construct full driver, (connection, numDevices=2) → Tpic6b595Full
                                                                    // two cascaded devices — 16 outputs total (DRAIN0..DRAIN15)

    int[] bytes = [0x01, 0x80] as int[]
