...
def connection = SiPoConnection.hardware(0, 0, 17, -1, -1)     // open SiPo connection, (bus, device, rckLine, srclrLine, gLine) → SiPoConnection
try {
    def chip = new Tpic6b595Minimal(connection)                  // construct driver, (connection, numDevices=1) → Tpic6b595Minimal
                                                                  // initialises every output to OFF (shadow zeroed, latched once)

    def p0 = chip.pin(0)                                          // get pin proxy, (n=0) → Pin
    def p7 = chip.pin(7)                                          // get pin proxy, (n=7) → Pin
