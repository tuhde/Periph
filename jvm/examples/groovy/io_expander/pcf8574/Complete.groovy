...
try {
    ...
    chip.onInterrupt { int mask ->                     // subscribe to INT line, (callback, intPin=connection.intPin()) → void
                                                       // uses connection.intPin() if wired, else a 5 ms polling thread
        printf("changed pins: 0x%02X%n", mask)
    }
    ...
    printf("polled interrupt: 0x%02X%n", changed)

    def p5 = chip.pin(5)                               // get pin proxy, (n) → Pin
    p5.setInput()
    p5.watch({ pin ->                                  // subscribe to pin edges, (handler, trigger=CHANGE) → void
