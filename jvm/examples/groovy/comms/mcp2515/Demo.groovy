...
try {
    def can = new Mcp2515Full(connection, 125, 8)                                // create MCP2515 driver, (connection, bitrateKbps=125, oscMhz=8) → Mcp2515Full
    ...
    while ((System.currentTimeMillis() / 1000L) - startSec < 5) {
        ...
        if (frame != null) {
            received++
            String hex = frame.data.collect { String.format("%02X ", it & 0xFF) }.join()
            printf("rx id=0x%X dlc=%d ext=%s rtr=%s data=%s%n",
                    frame.id, frame.data.length, frame.extended, frame.rtr, hex)
        }

        Thread.sleep(1000)
    }

    printf("done — sent=%d received=%d%n", sent, received)
} finally {
    connection.close()
}
