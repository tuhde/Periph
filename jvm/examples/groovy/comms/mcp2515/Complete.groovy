...
try {
    def can = new Mcp2515Full(connection, 250, 8)                                // create MCP2515 full driver, (connection, bitrateKbps=250, oscMhz=8) → Mcp2515Full
    ...
    can.sendBuffered(0x123, [0x01, 0x02] as byte[], false, 0)                   // send via TXB0, (id=0x123, data=2 B, extended=false, buf=0) → int
                                                                                 // explicit TX buffer selection
    can.sendBuffered(0x456, [0x03] as byte[], false, 1)                         // send via TXB1, (id=0x456, data=1 B, extended=false, buf=1) → int
    can.sendBuffered(0x1FFFFFFF, [0x04] as byte[], true, 2)                    // send via TXB2, (id=0x1FFFFFFF, data=1 B, extended=true, buf=2) → int
                                                                                 // extended 29-bit identifier

    Mcp2515Minimal.CanFrame frame = can.recv(500)                                // receive one frame, (timeoutMs=500) → CanFrame | null
    if (frame != null) {
        printf("rx id=0x%X ext=%s%n", frame.id, frame.extended)
    }

    Mcp2515Full.Errors errs = can.readErrors()                                  // read error counters, () → Errors
    printf("tec=%d rec=%d eflg=0x%02X%n", errs.tec, errs.rec, errs.eflg)
