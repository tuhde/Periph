...
final float TEMP_COEFF = 0.05f
final float TEMP_REFERENCE = 1.25f
final float CHANGE_THRESHOLD = 0.001f

def connection = new SPIConnection(0, 0, 3, 5_000_000)                                               // Open SPI bus 0, CS 0, Mode 3, 5 MHz, (bus, device, mode=3, maxSpeedHz=5_000_000) → SPIConnection
try {
    def adc = new Ad7705Full(connection, 2.5f, Ad7705Full.MCLK_2_4576MHZ)                           // Create AD7705 driver, (connection, vref=2.5 V, mclk_hz=2_457_600 Hz) → Ad7705Full
    ...
    while (true) {
        ...
        if (lastPressure == null || Math.abs(pressure - lastPressure) > CHANGE_THRESHOLD) {
            printf("→ pressure=%.4f V (raw %.4f V, temp %.4f V)%n", pressure, pressureRaw, temp)
