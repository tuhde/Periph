...
try {
    ...
    def off2 = adc.getOffsetCalibration(2)                                // Read offset calibration, (channel=2) → int 24-bit
    def gain2 = adc.getGainCalibration(2)                                 // Read gain calibration, (channel=2) → int 24-bit
    println("ch2 offset=${off2} gain=${gain2}")

    def raw1 = adc.readRaw(1)                                              // Read raw 16-bit code, (channel=1) → int 16-bit
                                                                              // blocks until DRDY, returns raw Data Register code
    def v1 = adc.readVoltage(1)                                            // Read voltage, (channel=1) → float V
                                                                              // converts raw code to volts using channel's current gain/bipolar setting
    def v2 = adc.readVoltage(2)                                            // Read voltage, (channel=2) → float V
    def v3 = adc.readVoltage(3)                                            // Read voltage, (channel=3) → float V
    println("ch1 raw=${raw1} ch1 v=${String.format('%.4f', v1)} ch2 v=${String.format('%.4f', v2)} ch3 v=${String.format('%.4f', v3)}")
