...
public class Complete {
    public static void main(String[] args) throws Exception {
        ...
        try (var connection = new I2CConnection(bus, DRV8830Full.DEFAULT_ADDRESS)) {     // open I²C bus, device address, (bus, address=0x60) → I2CConnection
            ...
            motor.drive(2.5);                                                               // Drive at regulated voltage, (voltage V, + = forward) → void
            ...
            System.out.printf("commanded %.2f V %s%n", out.voltage(), out.direction());

            motor.drive(-1.5);                                                              // Drive at regulated voltage, (voltage V, - = reverse) → void
            ...
            var fault = motor.readFault();                                                  // Read fault status, () → Fault(fault, ocp, uvlo, ots, ilimit)
            ...
            motor.onInterrupt(f -> System.out.println("fault interrupt " + f));             // Subscribe to FAULTn, (callback) → void
                                                                                            // falls back to a 5 ms polling thread when no intPin is wired
            var status = motor.pollInterrupt();                                             // Poll fault status, () → Fault
                                                                                            // same as readFault(); never clears implicitly
            motor.offInterrupt();                                                           // Unsubscribe, () → void
            System.out.println("poll " + status);
        }
    }
}
