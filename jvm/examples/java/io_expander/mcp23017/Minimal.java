...
public class Minimal {
    public static void main(String[] args) throws Exception {
        try (var connection = new I2CConnection(1, 0x20)) {           // open I²C bus 1, device 0x20, (bus, address) → I2CConnection
            var chip = new Mcp23017Minimal(connection, 0x20);            // construct driver, (connection, addr=0x20) → Mcp23017Minimal

            var p0 = chip.pin(0);                                      // get pin proxy, (n) → Pin
            p0.setOutput();                                            // set GPA0 as output, () → void

            var p8 = chip.pin(8);                                      // get pin proxy, (n) → Pin
