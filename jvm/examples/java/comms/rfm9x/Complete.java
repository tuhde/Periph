...
public class Complete {
    public static void main(String[] args) throws Exception {
        try (var connection = new SPIConnection(0, 0)) {                    // open SPI bus 0, CS 0, (busNum, deviceNum, mode=0, speed=1 MHz) → SPIConnection
            var radio = new Rfm95Full(connection, 868_000_000L);             // create RFM95W full driver, (connection, frequencyHz=868e6) → Rfm95Full

            int ver = radio.version();                                        // read silicon version, () → int
            System.out.printf("version: 0x%02X%n", ver);                     // expect 0x12 (SX1276)
            ...
            radio.send("hello world".getBytes());                             // send packet, (data=byte[] ≤255 B) → void
            ...
            if (pkt != null) {
                ...
                System.out.printf("rx %d B  rssi=%.1f  snr=%.1f%n", pkt.length, rssi, snr);
            }
