...
public class Minimal {
    public static void main(String[] args) throws Exception {
        try (var connection = new SPIConnection(0, 0)) {                    // open SPI bus 0, CS 0, (busNum, deviceNum, mode=0, speed=1 MHz) → SPIConnection
            var can = new Mcp2515Minimal(connection);                          // create MCP2515 driver, (connection, bitrateKbps=125, oscMhz=8) → Mcp2515Minimal
            ...
            if (frame != null) {
                System.out.printf("rx id=0x%X dlc=%d ext=%s rtr=%s data=%s%n",
                        frame.id, frame.data.length, frame.extended, frame.rtr,
                        java.util.Arrays.toString(frame.data));
            }
        }
    }
}
