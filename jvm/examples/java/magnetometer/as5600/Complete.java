...
public class Complete {
    public static void main(String[] args) throws Exception {
        try (var connection = new I2CConnection(1, 0x36)) {            // open I²C bus 1, device 0x36, (bus, address) → I2CConnection

            var as5600 = new As5600Full(connection);                        // construct driver, (connection) → As5600Full
            ...
            System.out.printf("angle=%.2f°  raw=%d%n", angle, raw);
            ...
            double rawDeg = as5600.rawAngleDegrees();                // read raw angle in degrees, () → double deg
                                                                     // rawAngle × 360 / 4096
            System.out.printf("raw_angle=%d  raw_deg=%.2f°%n", rawAngle, rawDeg);
            ...
            System.out.printf("agc=%d  magnitude=%d  status=0x%02X%n", agc, mag, status);

            System.out.printf("magnet_detected=%s  too_strong=%s  too_weak=%s%n",
            ...
            System.out.printf("zpos=%d  mpos=%d  mang=%d%n", zpos, mpos, mang);
            ...
            System.out.printf("burn_count=%d (remaining=%d)%n", bc, 3 - bc);
