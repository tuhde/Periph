...
public class Complete {
    public static void main(String[] args) throws Exception {
        try (var connection = new I2CConnection(1, 0x77)) {        // open I²C bus 1, device 0x77, (bus, address=0x77) → I2CConnection
            var sensor = new Bmp085Full(connection);                      // construct driver, verifies chip ID and loads calibration, (connection) → Bmp085Full

            int id = sensor.chipId();                                    // read chip ID register 0xD0, () → int
                                                                          // returns 0x55 for BMP085; useful for confirming the device is present
            ...
            sensor.setOversampling(Bmp085Full.OSS_HIGH_RES);            // set oversampling to high-resolution mode, (oss=0–3) → void
            ...
            System.out.printf("pressure: %.2f Pa%n", p);

            double alt = sensor.altitude();                              // compute altitude using default sea-level pressure 101325.0 Pa, () → double m
                                                                          // applies barometric formula: 44330 × (1 − (p/101325)^(1/5.255))
            System.out.printf("altitude: %.1f m%n", alt);

            double altRef = sensor.altitude(101300.0);                   // compute altitude with custom sea-level pressure, (seaLevelPa=101325.0 Pa) → double m
                                                                          // use local QNH for accurate terrain altitude
            System.out.printf("altitude (QNH 101300.0): %.1f m%n", altRef);

            double slp = sensor.seaLevelPressure(50.0);                 // back-calculate sea-level pressure at known altitude, (altitudeM=0.0 m) → double Pa
                                                                          // reduces station pressure to MSL using the barometric formula inverse
            System.out.printf("sea-level pressure at 50 m: %.2f Pa%n", slp);
            ...
            sensor.setOversampling(Bmp085Full.OSS_ULP);                 // restore ultra-low-power mode, (oss=0–3) → void
