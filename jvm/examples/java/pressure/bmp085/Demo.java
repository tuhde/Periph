...
public class Demo {
    ...
    public static void main(String[] args) throws Exception {
        try (var connection = new I2CConnection(1, 0x77)) {       // open I²C bus 1, device 0x77 (fixed address), (bus, address=0x77) → I2CConnection
            var sensor = new Bmp085Full(connection);                     // construct driver, verifies chip ID, reads calibration, (connection) → Bmp085Full
            ...
            sensor.setOversampling(Bmp085Full.OSS_ULP);                // set ultra-low-power mode, (oss=0–3) → void
            ...
            double firstPressure = sensor.pressure();                   // read pressure for reference, () → double Pa
            double seaLevel      = sensor.seaLevelPressure(0.0);       // derive sea-level pressure at altitude = 0, (altitudeM=0.0 m) → double Pa
            double firstTemp     = sensor.temperature();                // read temperature for reference, () → double °C
            double firstAlt      = sensor.altitude(seaLevel);          // compute altitude (should be ~0), (seaLevelPa) → double m

            System.out.printf("Reference: temperature=%.2f °C  pressure=%.2f Pa  altitude=%.1f m  (sea-level ref=%.2f Pa)%n",
            ...
            for (int i = 0; i < SAMPLES; i++) {
                ...
                double alt = sensor.altitude(seaLevel);                 // compute altitude relative to reference, (seaLevelPa) → double m
                ...
                System.out.printf("[%2d] temperature=%.2f °C  pressure=%.2f Pa  altitude=%.1f m  moved %s %.0f cm%n",
            ...
            System.out.printf("Pressure:    min=%.2f Pa  max=%.2f Pa  mean=%.2f Pa%n",
