...
public class Complete {
    public static void main(String[] args) throws Exception {
        try (var connection = new I2CConnection(1, 0x46)) {       // open I²C bus 1, device 0x46, (bus, address=0x46) → I2CConnection
            ...
            int cid = bmp.chipId();                                    // read chip ID, () → int
            System.out.printf("chip_id=0x%02x (expect 0x50)%n", cid);
            ...
            int n = bmp.fifoCount();                                   // read FIFO frame count, () → int
            bmp.enableDrdyInterrupt(true);                             // enable data-ready interrupt, (enable) → void
            boolean drdy = bmp.dataReady();                            // check data ready, () → boolean
            double[] forced = bmp.forced();                            // trigger FORCED measurement, () → double[] {Pa, °C}
            double[] both = bmp.both();                                // read both atomically, () → double[] {Pa, °C}
            double alt = bmp.altitude();                               // compute altitude, (seaLevelPa=101325.0) → double
            int st = bmp.status();                                     // read STATUS, () → int
            int ist = bmp.interruptStatus();                           // read INT_STATUS, () → int
            int[] eff = bmp.effectiveOsr();                             // read effective OSR, () → int[] {osr_p, osr_t}
            ...
            System.out.printf("P=%.1f Pa  T=%.2f C  alt=%.1f m  frames=%d  drdy=%s  eff=(%d,%d)  status=0x%02x  isr=0x%02x%n",
                both[0], both[1], alt, n, drdy, eff[0], eff[1], st, ist);
            System.out.printf("forced: P=%.1f Pa  T=%.2f C%n", forced[0], forced[1]);
        }
    }
}