...
public class Minimal {
    public static void main(String[] args) throws Exception {
        try (var connection = new I2CConnection(1, 0x5C)) {       // open I²C bus 1, device 0x5C, (bus, address=0x5C) → I2CConnection
            var sensor = new Lps28dfwMinimal(connection);               // construct driver, verifies chip ID and applies defaults, (connection) → Lps28dfwMinimal

            while (true) {
                double t = sensor.readTemperature();                   // read temperature, () → double °C
                double p = sensor.readPressure();                      // read pressure, () → double hPa
