...
public class Complete {
    public static void main(String[] args) throws Exception {
        try (var connection = new I2CConnection(1, 0x5C)) {    // open I²C bus 1, device 0x5C, (bus, address=0x5C) → I2CConnection
            var sensor = new Lps33hwFull(connection);                     // construct driver, verifies chip ID, (connection) → Lps33hwFull
            ...
            double[] oneShot = sensor.oneShot();                         // trigger one-shot measurement, () → double[2] (Pa, °C)
            ...
            System.out.printf("status=0x%02X  int_source=0x%02X  fifo_status=0x%02X%n", st, intsrc, fst);
            System.out.printf("one_shot: %.1f Pa / %.2f C, then: %.1f Pa / %.2f C%n",
