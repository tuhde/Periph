...
public class Demo {
    static final float FILTER_DP_THRESHOLD = 0.001f;

    public static void main(String[] args) throws Exception {
        try (var connection = new SPIConnection(0, 0, 3, 5_000_000)) {    // open SPI bus 0, device 0, Mode 3, 5 MHz
            var adc = new Ad7706Full(connection, 2.5f, Ad7706Minimal.MCLK_2_4576MHZ);    // construct and initialise the AD7706, (connection, vref=2.5 V, mclk_hz=2_457_600 Hz) → Ad7706Full
            ...
            Float lastFilterDp = null;
            while (true) {
                float inlet  = adc.readVoltage(1);                        // Read voltage, (channel=1) → float V
                float outlet = adc.readVoltage(2);                        // Read voltage, (channel=2) → float V
                float duct   = adc.readVoltage(3);                        // Read voltage, (channel=3) → float V
                float filterDp = inlet - outlet;
                if (lastFilterDp == null || Math.abs(filterDp - lastFilterDp) > FILTER_DP_THRESHOLD) {
                    System.out.printf("→ filter_dp=%.4f V, duct_pressure=%.4f V%n", filterDp, duct);
