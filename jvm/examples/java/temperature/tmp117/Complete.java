...
public class Complete {
    public static void main(String[] args) throws Exception {
        ...
        try (var connection = new I2CConnection(bus, TMP117Full.DEFAULT_ADDRESS)) {       // open I²C bus, device address, (bus, address=0x48) → I2CConnection
            ...
            System.out.printf("temperature %.4f °C%n", sensor.readTemperature());         // Read temperature, () → double °C
            ...
            System.out.println("config " + sensor.getConfig());                           // Read conversion config, () → Config(mode, averaging, cycleSeconds s)
            ...
            System.out.println("shutdown " + sensor.isShutdown());                        // Check Shutdown mode, () → boolean
                                                                                          // reads MOD[1:0] == 01
            ...
            System.out.printf("one-shot %.4f °C%n", sensor.readTemperature());            // Read temperature, () → double °C
                                                                                          // the one-shot result
            ...
            sensor.setLowLimit(10.0);                                                     // Set TLOW_LIMIT, (celsius °C) → void
                                                                                          // rounded to the nearest 0.0078125 °C step
            System.out.println("high " + sensor.getHighLimit());                          // Read THIGH_LIMIT, () → double °C
                                                                                          // same format as TEMP_RESULT
            System.out.println("low " + sensor.getLowLimit());                            // Read TLOW_LIMIT, () → double °C
                                                                                          // same format as TEMP_RESULT

            sensor.setTemperatureOffset(0.25);                                            // Set calibration offset, (celsius °C) → void
                                                                                          // added to every result after linearization
            System.out.println("offset " + sensor.getTemperatureOffset());                // Read calibration offset, () → double °C
                                                                                          // decodes TEMP_OFFSET
            ...
            System.out.println("eeprom busy " + sensor.isEepromBusy());                   // Check EEPROM busy, () → boolean
                                                                                          // reads EEPROM_UL.EEPROM_Busy
            sensor.lockEeprom();                                                          // Lock EEPROM, () → void
                                                                                          // EUN=0: writes are volatile again
            System.out.printf("eeprom1 0x%04X%n", sensor.readEepromScratch(1));           // Read EEPROM scratch, (slot 1|2|3) → int
            ...
            System.out.printf("eeprom2 0x%04X%n", sensor.readEepromScratch(2));           // Read EEPROM scratch, (slot 1|2|3) → int
                                                                                          // reads back EEPROM2
            ...
            System.out.println("above high " + ((status & TMP117Full.SOURCE_HIGH) != 0)
                    + " below low " + ((status & TMP117Full.SOURCE_LOW) != 0));

            sensor.onInterrupt(s -> System.out.println("alert, status mask " + s));       // Subscribe to ALERT, (callback) → void
