...
fun main() {
    ...
    I2CConnection(bus, 0x0C).use { magConnection ->                  // AK8963, same bus, reached via I²C bypass
        ...
        val t = imu.temperature()                                    // Read die temperature, () → double °C
        ...
        val rm = imu.magRaw()                                        // Read raw mag values, () → IntArray
                                                        // returns raw 16-bit signed magnetometer register values

        val ready = imu.dataReady()                                  // Check data ready flag, () → boolean
        ...
        Thread.sleep(50)
        val count = imu.fifoCount()                                  // Read FIFO byte count, () → int
        ...
        val motion = imu.motionDetected()                            // Check motion detected, () → boolean
                                                        // reads WOM_INT bit from INT_STATUS register (clears on read)

        println("t=%.2f  mag=%.1f,%.1f,%.1f  ready=%s  motion=%s".format(t, m[0], m[1], m[2], ready, motion))
    }
    }
}