///usr/bin/env jbang "$0" "$@" ; exit $?
//JAVA 22+
//JAVA_OPTIONS --enable-native-access=ALL-UNNAMED
//DEPS it.uhde:periph-connection:1.1.0
//DEPS it.uhde:periph-kotlin:1.1.0

import it.uhde.periph.connection.I2CConnection
import it.uhde.periph.chips.environmental.Bme280Minimal

fun main() {
    val bus  = System.getenv("I2C_BUS")?.toIntOrNull() ?: 1
    val addr = System.getenv("I2C_ADDR")?.removePrefix("0x")?.toInt(16) ?: 0x76
    I2CConnection(bus, addr).use { connection ->                  // open I²C bus, (bus, address=0x76) → I2CConnection
        val sensor = Bme280Minimal(connection)                   // construct driver, verifies chip ID and loads calibration, (connection) → Bme280Minimal

        for (i in 0 until 5) {
            val t = sensor.temperature()                        // read temperature, () → Double °C
            val p = sensor.pressure()                           // read pressure, () → Double hPa
            val h = sensor.humidity()                           // read humidity, () → Double %RH
            println("%.1f °C, %.1f hPa, %.1f %%RH".format(t, p, h))
            Thread.sleep(1000)
        }
    }
}
