...
fun main() {
    ...
    I2CConnection(bus, DS3231Minimal.DEFAULT_ADDRESS).use { connection ->
        ...
        val alarm1 = rtc.getAlarm1()                                                                 // Read Alarm 1 config, () → Alarm1
        ...
        val alarm2 = rtc.getAlarm2()                                                                 // Read Alarm 2 config, () → Alarm2
                                                                                                           // decodes mask bits back into match_mode
        ...
        rtc.enableInterrupt(DS3231Full.SOURCE_ALARM2)                                                // Enable an alarm interrupt, (source) → None
                                                                                                           // sets A2IE
        val status = rtc.pollInterrupt()                                                             // Poll and clear alarm flags, () → Int
        ...
        val has32k = rtc.is32kHzEnabled()                                                            // Query 32kHz output, () → Boolean
        ...
        val aging = rtc.getAgingOffset()                                                             // Read aging offset trim, () → Int (raw signed byte)
        rtc.setAgingOffset(0)                                                                        // Set aging offset trim, (offset=0) → None
                                                                                                           // restores the neutral (power-on) trim value

        println("datetime=$dt temp=%.2fC alarm1=$alarm1 alarm2=$alarm2 status=0x%02X 32kHz=$has32k osf=$stopped aging=$aging".format(tempC, status))
    }
}
