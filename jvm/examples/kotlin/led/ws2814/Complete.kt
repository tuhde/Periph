...
fun main() {
    ...
    NeoPixelConnection(spiBus, spiDevice).use { connection ->  // open SPI bus, (busNum, deviceNum) → NeoPixelConnection
        val strip = WS2814Full(connection, pixelCount)               // construct driver, (connection, n) → WS2814Full

        strip.fill(255, 0, 0, 0)                                     // fill entire strip red and send, (r=0–255, g=0–255, b=0–255, w=0–255) → Unit
                                                                     // inherited from WS2814Minimal; fills buffer in identity RGBW order and calls connection.write()
        Thread.sleep(500)
        ...
        strip.show()                                                  // transmit buffer to strip, () → Unit
                                                                     // sends brightness-scaled RGBW bytes (identity order)
        Thread.sleep(500)
