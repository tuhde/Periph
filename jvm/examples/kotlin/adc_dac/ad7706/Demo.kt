...
val FILTER_DP_THRESHOLD = 0.001f
...
fun main() {
    SPIConnection(0, 0, 3, 5_000_000).use { connection ->                // open SPI bus 0, device 0, Mode 3, 5 MHz
        ...
        var lastFilterDp: Float? = null
        while (true) {
            val inlet  = adc.readVoltage(1)                                  // Read voltage, (channel=1) → float V
            val outlet = adc.readVoltage(2)                                  // Read voltage, (channel=2) → float V
            val duct   = adc.readVoltage(3)                                  // Read voltage, (channel=3) → float V
            val filterDp = inlet - outlet
            if (lastFilterDp == null || abs(filterDp - lastFilterDp!!) > FILTER_DP_THRESHOLD) {
                println("→ filter_dp=%.4f V, duct_pressure=%.4f V".format(filterDp, duct))
