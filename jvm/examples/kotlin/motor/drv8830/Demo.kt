...
fun checkFault(motor: DRV8830Full) {
    ...
    val f = motor.readFault()                                               // Read fault status, () → Fault
    if (f.fault) {
...
fun run(motor: DRV8830Full, voltage: Double, seconds: Int) {
    ...
    motor.drive(voltage)                                                    // Drive at regulated voltage, (voltage V, signed) → Unit
    checkFault(motor)
    repeat(seconds) {
        Thread.sleep(1000)
        val out = motor.readOutput()                                        // Read back CONTROL, () → Output(voltage V, direction)
        println("%-7s %.2f V".format(out.direction, out.voltage))
    }
}
