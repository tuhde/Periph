...
/**
 * BMP085 — digital barometric pressure and temperature sensor (minimal driver).
 *
 ...
 */
open class Bmp085Minimal(protected val connection: Connection) {
    ...
    init {
        ...
        if (id[0].toInt() and 0xFF != CHIP_ID) {
            throw IOException(
                "BMP085 not found: expected chip ID 0x55, got 0x${(id[0].toInt() and 0xFF).toString(16)}"
            )
        }
        readCalibration()
    }
    ...
    protected fun checkCalibration() {
        ...
        for (i in raw16.indices) {
            if (raw16[i] == 0x0000 || raw16[i] == 0xFFFF) {
                throw IOException(
                    "BMP085 calibration invalid: ${names[i]} = 0x${raw16[i].toString(16)}"
                )
            }
        }
    }
     ...
     * measurement. Runs Bosch integer compensation and returns the result in pascals.
     *
