...
class Bmp180Test {
    ...
    fun fullApi() {
        ...
        // Unlike Python/C++/JS/Rust, the Kotlin constructor verifies the
        ...
        connection.setRegister(Bmp180Minimal.REG_OUT_MSB, 0x6C, 0xFA)
        assertEquals(830.599010429, sensor.seaLevelPressure(100.0), 0.5)

        // reset(): writes soft-reset command, then re-reads calibration coefficients.
        preloadCalibration(connection)
        sensor.reset()
        val sawSoftReset = connection.writes().any { it.size == 2 && (it[0].toInt() and 0xFF) == Bmp180Minimal.REG_SOFT_RST && (it[1].toInt() and 0xFF) == 0xB6 }
        assertTrue(sawSoftReset, "reset should write the soft-reset command")
        val calReads = connection.writes().count { it.size == 1 && (it[0].toInt() and 0xFF) == Bmp180Minimal.REG_CAL_START }
        ...
        assertThrows(IOException::class.java) { Bmp180Full(badConnection) }
    }
}
