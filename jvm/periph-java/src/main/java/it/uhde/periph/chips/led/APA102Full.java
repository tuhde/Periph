 ...
 */
public class APA102Full extends APA102Minimal {
     ...
     */
    public APA102Full(Connection connection, int n) {
        super(connection, n);
    }
     ...
     * @param pixelBrightness  per-pixel hardware brightness 0–31
     */
    public void setPixel(int index, int r, int g, int b, int pixelBrightness) {
        if (n == 0) return;
    ...
    /**
     * Write one pixel into the buffer with default hardware brightness 31.
     */
    public void setPixel(int index, int r, int g, int b) {
        setPixel(index, r, g, b, 31);
    }
    ...
    public void show() throws IOException {
        ...
        if (brightness == 255) {
            ...
            for (int i = 0; i < n; i++) {
                int base = i * 4;
                frame[4 + base]     = buf[base];                                                // hardware brightness
                frame[4 + base + 1] = (byte) ((buf[base + 1] & 0xFF) * brightness / 255);     // blue
                frame[4 + base + 2] = (byte) ((buf[base + 2] & 0xFF) * brightness / 255);     // green
                frame[4 + base + 3] = (byte) ((buf[base + 3] & 0xFF) * brightness / 255);     // red
            }
        }
