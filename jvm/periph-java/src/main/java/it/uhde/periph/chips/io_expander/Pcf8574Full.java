...
public class Pcf8574Full extends Pcf8574Minimal {
    ...
    /**
     * Fixed {@link EdgeTrigger}-agnostic edge handler instance, reused across
     * onEdge()/offEdge() calls. A method reference like {@code this::handleEdge}
     * is not guaranteed by the JLS to evaluate to the same object each time, so
     * offEdge() could silently fail to remove the handler onEdge() registered.
     */
     ...
     */
    public Pcf8574Full(Connection connection) throws IOException {
        super(connection);
        prev = readPort();
    }
     ...
     * @return Full Pin proxy with {@link Pin#watch watch}/{@link Pin#unwatch unwatch} support
     */
    @Override
    ...
    public static class Pin extends Pcf8574Minimal.Pin {
