...
public class VL53L0XFull extends VL53L0XMinimal {
     ...
     */
    public VL53L0XFull(Connection connection) throws IOException {
        super(connection);
    }
    ...
    public synchronized void startContinuous(int periodMs) throws IOException {
        ...
        if (periodMs > 0) {
            ...
            wr32(REG_SYSTEM_INTERMEASUREMENT, osc != 0 ? (long) periodMs * osc : periodMs);
    ...
    public synchronized Measurement readMeasurement() throws IOException {
        readResult();
        return new Measurement(resultWord(10), rangeStatus, resultWord(6) / 128.0, resultWord(8) / 128.0,
                resultWord(2) / 256.0);
    }
    ...
    public synchronized double signalRateLimit() throws IOException {
        return rd16(REG_FINAL_MIN_COUNT_RATE_RTN) / 128.0;
    }
    ...
    public synchronized void setVcselPulsePeriod(VcselPeriodType type, int pclks) throws IOException {
        ...
        if (type == VcselPeriodType.PRE_RANGE) {
            switch (pclks) {
                case 12 -> preHigh = 0x18;
                case 14 -> preHigh = 0x30;
                case 16 -> preHigh = 0x40;
                case 18 -> preHigh = 0x50;
                default -> throw new IllegalArgumentException("pre-range VCSEL period must be 12, 14, 16 or 18");
            }
        } else {
            // VALID_PHASE_HIGH, VALID_PHASE_LOW, VCSEL_WIDTH, PHASECAL_CONFIG_TIMEOUT, page-1 PHASECAL_LIM.
            fin = switch (pclks) {
                case 8 -> new int[]{0x10, 0x08, 0x02, 0x0C, 0x30};
                case 10 -> new int[]{0x28, 0x08, 0x03, 0x09, 0x20};
                case 12 -> new int[]{0x38, 0x08, 0x03, 0x08, 0x20};
                case 14 -> new int[]{0x48, 0x08, 0x03, 0x07, 0x20};
                default -> throw new IllegalArgumentException("final-range VCSEL period must be 8, 10, 12 or 14");
            };
        }
    ...
    public synchronized void setInterruptThresholds(int lowMm, int highMm) throws IOException {
        ...
        wr16(REG_SYSTEM_THRESH_LOW, (lowMm / 2) & 0x0FFF);
        wr16(REG_SYSTEM_THRESH_HIGH, (highMm / 2) & 0x0FFF);
    }
    ...
    public synchronized int[] interruptThresholds() throws IOException {
        return new int[]{(rd16(REG_SYSTEM_THRESH_LOW) & 0x0FFF) * 2, (rd16(REG_SYSTEM_THRESH_HIGH) & 0x0FFF) * 2};
    }
