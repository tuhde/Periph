 ...
 */
public class Neo6Full extends Neo6Minimal {
    ...
    protected void handleExtra(String sentenceId, String[] fields) {
        switch (sentenceId) {
            case "GGA":
                if (fields.length > 1 && !fields[1].isEmpty()) utcTime = fields[1];
                if (fields.length > 8 && !fields[8].isEmpty()) {
                    try {
                        hdop = Double.parseDouble(fields[8]);
    ...
    private void parseRmc(String[] fields) {
        ...
        if (!fields[1].isEmpty()) utcTime = fields[1];
        try {
            if (!fields[7].isEmpty()) speed = Double.parseDouble(fields[7]) * 0.514444;
            if (!fields[8].isEmpty()) course = Double.parseDouble(fields[8]);
        } catch (NumberFormatException ignored) { }
        if (!fields[9].isEmpty()) utcDate = fields[9];
    }

    private void parseVtg(String[] fields) {
        try {
            if (fields.length > 1 && !fields[1].isEmpty()) course = Double.parseDouble(fields[1]);
            if (fields.length > 7 && !fields[7].isEmpty()) speed = Double.parseDouble(fields[7]) / 3.6;
        } catch (NumberFormatException ignored) { }
    }
    ...
    public Double course() { return course; }
    ...
    public String utcDate() { return utcDate; }
    ...
    private byte[] readUbxResponse(int wantClass, int wantId) throws IOException {
        final int maxFrames = 400;
        final int maxIdle = 4000;
        int idle = 0;
        int frames = 0;
        while (frames < maxFrames) {
            ...
            if (b == null) {
                idle++;
                if (idle > maxIdle) throw new IOException("UBX response timeout");
                continue;
            }
            idle = 0;
            ...
            byte[] header = new byte[]{(byte) (int) cls, (byte) (int) mid, (byte) (int) lenLo, (byte) (int) lenHi};
            ...
            if (cls == CLASS_ACK && mid == ID_ACK_NAK) {
                throw new IOException(String.format(
                        "UBX NAK for class 0x%02X id 0x%02X", wantClass, wantId));
            }
    ...
    public void saveConfig() throws IOException {
        byte[] payload = new byte[]{
                0, 0, 0, 0,               // clearMask = 0
                (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, // saveMask = all
                0, 0, 0, 0,               // loadMask = 0
