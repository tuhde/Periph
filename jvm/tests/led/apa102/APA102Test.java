...
public class APA102Test {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try (var connection = new SPIConnection(busNum, deviceNum, 0, 1_000_000)) {

            var strip = new APA102Full(connection, pixels);
            ...
            strip.setPixel(1, 0, 255, 0, 31);
            strip.setPixel(pixels - 1, 0, 0, 255, 31);
            strip.show();
            ...
            // --- setPixels with per-pixel hardware brightness ---
            int[][] colorsBright = new int[pixels][4];
            for (int i = 0; i < pixels; i++) {
                colorsBright[i][0] = 255;
                colorsBright[i][1] = 0;
                colorsBright[i][2] = 0;
                colorsBright[i][3] = 31 - (i * 30 / pixels);
            }
            strip.setPixels(colorsBright);
            strip.show();
            checkTrue("setPixels with hardware brightness + show accepted", true);

            // --- rotate ---
            strip.fill(255, 0, 0);
            strip.setPixel(0, 0, 255, 0, 31);
            strip.rotate(1);
            strip.show();
            ...
            strip.setPixel(pixels + 10, 255, 0, 0, 31);
