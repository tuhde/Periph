...
public class WS2814Test {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try (var connection = new NeoPixelConnection(busNum, deviceNum)) {
            ...
            var strip = new WS2814Full(connection, pixels);
            ...
            checkTrue("fill(white W) accepted", true);
            ...
            // --- setPixel + show ---
            strip.setPixel(0, 255, 0, 0, 0);
            strip.setPixel(1, 0, 255, 0, 0);
            strip.setPixel(2, 0, 0, 255, 0);
            strip.setPixel(3, 0, 0, 0, 255);
            strip.show();
            ...
            for (int i = 0; i < pixels; i++) {
                ...
                colors[i][3] = i % 2 == 0 ? 128 : 0;
            }
            strip.setPixels(colors);
            strip.show();
            checkTrue("setPixels + show accepted", true);

            // --- setPixels with 3-element arrays (w defaults to 0) ---
            int[][] rgb3 = new int[2][3];
            rgb3[0] = new int[]{255, 0, 0};
            rgb3[1] = new int[]{0, 0, 0};
            strip.setPixels(rgb3);
            strip.show();
            checkTrue("setPixels with [r,g,b] (no w) accepted", true);

            // --- rotate ---
            strip.fill(255, 0, 0, 0);
            strip.setPixel(0, 0, 255, 0, 0);
            strip.rotate(1);
            strip.show();
            ...
            // --- clamping ---
            strip.fill(300, -10, 256, 500);
            checkTrue("fill with out-of-range values clamped", true);
            strip.setPixel(pixels + 10, 255, 0, 0, 0);
