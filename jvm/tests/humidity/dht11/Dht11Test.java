...
public class Dht11Test {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Dht11Minimal d1 = new Dht11Minimal(new MockConnection(new byte[]{0x35, 0x00, 0x18, 0x04, 0x51}));
        double[] r1 = d1.read();
        ...
        Dht11Minimal d2 = new Dht11Minimal(new MockConnection(new byte[]{0x20, 0x00, 0x0A, (byte)0x81, (byte)0xAB}));
        double[] r2 = d2.read();
        ...
        Dht11Minimal d3 = new Dht11Minimal(new MockConnection(new byte[]{0x35, 0x00, 0x18, 0x04, 0x00}));
        ...
        Dht11Full d4 = new Dht11Full(new MockConnection(new byte[]{0x35, 0x00, 0x18, 0x04, 0x51}), 3);
        ...
        FlakyConnection flaky = new FlakyConnection();
        ...
        Dht11Full d6 = new Dht11Full(new AlwaysBadConnection(), 2);
        boolean exhausted = false;
        try { d6.readRetry(2); } catch (Dht11Minimal.Dht11Exception e) { exhausted = true; }
        checkTrue("read_retry_exhausted", exhausted);

        // Test 7: readRaw
        Dht11Full d7 = new Dht11Full(new MockConnection(new byte[]{0x35, 0x00, 0x18, 0x04, 0x51}), 3);
        byte[] raw = d7.readRaw();
    ...
    static class MockConnection extends DHTxxConnection {
        private final byte[] frame;
        MockConnection(byte[] frame) {
            super("/dev/null", 0);
            this.frame = frame;
        }
        @Override
    ...
    static class FlakyConnection extends DHTxxConnection {
        int attempts = 0;
        FlakyConnection() { super("/dev/null", 0); }
        @Override
        public byte[] read() {
            attempts++;
    ...
    static class AlwaysBadConnection extends DHTxxConnection {
        AlwaysBadConnection() { super("/dev/null", 0); }
        @Override
        public byte[] read() {
