#!/usr/bin/env bash
# Usage:
#   ./test_linux_groovy.sh [--level unit|hil|conformance] <category>/<chip>
#
# Runs a JVM (jbang) hardware test on the Pi via Linux FFM (no Pi4J), for the
# Groovy driver variant. Replaces `test.sh --lang groovy`.
...
detect_level() {
    ...
    elif [ -f "$SCRIPT_DIR/$MODULE/src/test/groovy/it/uhde/periph/chips/$CATEGORY/${CHIP_PASCAL}${TEST_SUFFIX}.groovy" ]; then
        echo "unit"
    else
...
find_test_file() {
    find "$SCRIPT_DIR/tests/$CATEGORY/$CHIP/" -name "*Test.groovy" 2>/dev/null | head -1
}
...
run_hil() {
    ...
    if [ -z "$test_file" ]; then
        echo "ERROR: no *Test.groovy file found under tests/$CATEGORY/$CHIP/" >&2
        exit 1
    fi
    if [ "$TRANSPORT" = "i2c" ]; then
        echo "=== [hil] Running $TARGET on JVM/groovy (I2C bus $I2C_BUS, addr $I2C_ADDR) ==="
        I2C_BUS="$I2C_BUS" I2C_ADDR="$I2C_ADDR" jbang "$test_file"
    else
        echo "=== [hil] Running $TARGET on JVM/groovy (SPI bus $SPI_BUS, device $SPI_DEVICE, pixels $PIXEL_COUNT) ==="
...
run_conformance() {
    ...
    I2C_BUS="$I2C_BUS" I2C_ADDR="${I2C_ADDR:-}" python3 "$checker" --lang jvm-groovy --jbang-test "$test_file"
}
