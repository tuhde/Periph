#!/usr/bin/env bash
# Usage:
#   ./test_linux_kotlin.sh [--level unit|hil|conformance] <category>/<chip>
#
# Runs a JVM (jbang) hardware test on the Pi via Linux FFM (no Pi4J), for the
# Kotlin driver variant. Replaces `test.sh --lang kotlin`.
...
detect_level() {
    ...
    elif [ -f "$SCRIPT_DIR/$MODULE/src/test/kotlin/it/uhde/periph/chips/$CATEGORY/${CHIP_PASCAL}${TEST_SUFFIX}.kt" ]; then
        echo "unit"
    else
...
find_test_file() {
    find "$SCRIPT_DIR/tests/$CATEGORY/$CHIP/" -name "*Test.kt" 2>/dev/null | head -1
}
...
run_hil() {
    ...
    if [ -z "$test_file" ]; then
        echo "ERROR: no *Test.kt file found under tests/$CATEGORY/$CHIP/" >&2
        exit 1
    fi
    if [ "$TRANSPORT" = "i2c" ]; then
        echo "=== [hil] Running $TARGET on JVM/kotlin (I2C bus $I2C_BUS, addr $I2C_ADDR) ==="
        I2C_BUS="$I2C_BUS" I2C_ADDR="$I2C_ADDR" jbang "$test_file"
    else
        echo "=== [hil] Running $TARGET on JVM/kotlin (SPI bus $SPI_BUS, device $SPI_DEVICE, pixels $PIXEL_COUNT) ==="
