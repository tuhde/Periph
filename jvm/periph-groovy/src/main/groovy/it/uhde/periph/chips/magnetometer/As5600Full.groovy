 ...
 * {@code PM_NOM}, {@code PM_LPM1}, {@code PM_LPM2}, {@code PM_LPM3}
 *
 * <h2>Hysteresis constants</h2>
 * {@code HYST_OFF}, {@code HYST_1LSB}, {@code HYST_2LSB}, {@code HYST_3LSB}
 *
 * <h2>Output stage constants</h2>
 * {@code OUTS_ANALOG}, {@code OUTS_ANALOG_REDUCED}, {@code OUTS_PWM}
 *
 * <h2>PWM frequency constants</h2>
 * {@code PWMF_115HZ}, {@code PWMF_230HZ}, {@code PWMF_460HZ}, {@code PWMF_920HZ}
 *
 * <h2>Slow filter constants</h2>
 * {@code SF_16X}, {@code SF_8X}, {@code SF_4X}, {@code SF_2X}
 *
 * <h2>Burn commands</h2>
 * {@code BURN_ANGLE}, {@code BURN_SETTING}
 */
@CompileStatic
class As5600Full extends As5600Minimal {
    ...
    static final int PM_NOM  = 0
    /** Power mode: low power 1 (3.4 mA, 5 ms poll). */
    static final int PM_LPM1 = 1
    /** Power mode: low power 2 (1.8 mA, 20 ms poll). */
    static final int PM_LPM2 = 2
    /** Power mode: low power 3 (1.5 mA, 100 ms poll). */
    static final int PM_LPM3 = 3

    // --- Hysteresis constants ---
    /** Hysteresis: off. */
    static final int HYST_OFF   = 0
    /** Hysteresis: 1 LSB. */
    static final int HYST_1LSB  = 1
    /** Hysteresis: 2 LSBs. */
    static final int HYST_2LSB  = 2
    /** Hysteresis: 3 LSBs. */
    static final int HYST_3LSB  = 3

    // --- Output stage constants ---
    /** Output: analog 0–VDD. */
    static final int OUTS_ANALOG          = 0
    /** Output: analog 10–90% VDD. */
    static final int OUTS_ANALOG_REDUCED  = 1
    /** Output: digital PWM. */
    static final int OUTS_PWM             = 2

    // --- PWM frequency constants ---
    /** PWM frequency: 115 Hz. */
    static final int PWMF_115HZ  = 0
    /** PWM frequency: 230 Hz. */
    static final int PWMF_230HZ  = 1
    /** PWM frequency: 460 Hz. */
    static final int PWMF_460HZ  = 2
    /** PWM frequency: 920 Hz. */
    static final int PWMF_920HZ  = 3

    // --- Slow filter constants ---
    /** Slow filter: 16× (2.2 ms settle). */
    static final int SF_16X = 0
    /** Slow filter: 8× (1.1 ms settle). */
    static final int SF_8X  = 1
    /** Slow filter: 4× (0.55 ms settle). */
    static final int SF_4X  = 2
    /** Slow filter: 2× (0.286 ms settle). */
    static final int SF_2X  = 3

    // --- Burn commands ---
    /** Burn command: permanently store ZPOS+MPOS to OTP. */
    static final int BURN_ANGLE   = 0x80
    /** Burn command: permanently store MANG+CONF to OTP. */
    static final int BURN_SETTING = 0x40
     ...
     */
    double rawAngleDegrees() {
        rawAngle() * 360.0d / 4096.0d
    }
     ...
     */
    void configure(int pm, int hyst, int outs, int pwmf, int sf, int fth, boolean wd) {
     ...
     */
    void setZeroPosition(int pos) { writeReg12(REG_ZPOS_H, REG_ZPOS_L, pos) }
     ...
     */
    void setMaxPosition(int pos) { writeReg12(REG_MPOS_H, REG_MPOS_L, pos) }
     ...
     */
    void setMaxAngle(int span) { writeReg12(REG_MANG_H, REG_MANG_L, span) }
     ...
     */
    int zeroPosition() { readReg12(REG_ZPOS_H) }
     ...
     */
    int maxPosition() { readReg12(REG_MPOS_H) }
     ...
     */
    int maxAngle() { readReg12(REG_MANG_H) }
     ...
     */
    int burnCount() { readReg8(REG_ZMCO) & 0x03 }
     ...
     *
     * @throws RuntimeException if MD=0 or ZMCO≥3
     */
    void burnAngle() {
        int status = readReg8(REG_STATUS)
        if ((status & STATUS_MD) == 0) {
            throw new RuntimeException("AS5600: burn_angle requires magnet detected (MD=1)")
        }
        if (burnCount() >= 3) {
            throw new RuntimeException("AS5600: burn_angle failed, ZMCO=3 (no remaining writes)")
        }
     ...
     *
     * @throws RuntimeException if ZMCO≠0
     */
    void burnSetting() {
        if (burnCount() != 0) {
            throw new RuntimeException("AS5600: burn_setting requires ZMCO=0")
        }
