 ...
 */
@CompileStatic
class WS2814Full extends WS2814Minimal {
    ...
    void setPixel(int index, int r, int g, int b, int w = 0) {
        ...
        buf[index * 4]     = (byte) Math.max(0, Math.min(255, r))
        buf[index * 4 + 1] = (byte) Math.max(0, Math.min(255, g))
    ...
    void setPixels(List<int[]> colors) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            buf[i * 4]     = (byte) Math.max(0, Math.min(255, colors[i][0])) // r
            buf[i * 4 + 1] = (byte) Math.max(0, Math.min(255, colors[i][1])) // g
