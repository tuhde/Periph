...
class TMP117Full extends TMP117Minimal {
     ...
     */
    static record Config(Mode mode, int averaging, double cycleSeconds) {
    }
    ...
    private static final double[] CYCLES = [0.0155d, 0.125d, 0.25d, 0.5d, 1.0d, 4.0d, 8.0d, 16.0d] as double[]
    // Averaging counts, indexed by AVG[1:0].
    private static final int[] AVERAGINGS = [0, 8, 32, 64] as int[]
    ...
    private static int encodeTemperature(double celsius) {
        ...
        long value = steps >= 0 ? (long) (steps + 0.5d) : -((long) (-steps + 0.5d))
        value = Math.max(-32768L, Math.min(32767L, value))
     ...
     */
    void configure(Mode mode, int averaging, double cycleSeconds) {
        int avg = -1
     ...
     * @throws IOException on bus error
     */
    void setHighLimit(double celsius) {
     ...
     * @throws IOException on bus error
     */
    void setLowLimit(double celsius) {
     ...
     * @throws IOException on bus error
     */
    void setTemperatureOffset(double celsius) {
     ...
     */
    int readEepromScratch(int slot) {
        int reg
        switch (slot) {
     ...
     */
    void writeEepromScratch(int slot, int value) {
     ...
     * @throws IOException on bus error
     */
    void configureAlert(AlertMode mode, AlertPolarity polarity, AlertPinFunction pinFunction) {
