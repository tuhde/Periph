...
class Mfrc522Minimal {
    ...
    protected void flushFifo() throws IOException { writeReg(REG_FIFO_LEVEL, FIFO_FLUSH) }
    ...
    protected boolean cardCommand(int command, int waitIrq, int[] sendData) throws IOException {
        ...
        flushFifo()
        if (sendData != null && sendData.length > 0) writeFifo(sendData)
    ...
    byte[] readUid() {
        ...
        try { haltCard() } catch (IOException e) { throw new RuntimeException(e) }
