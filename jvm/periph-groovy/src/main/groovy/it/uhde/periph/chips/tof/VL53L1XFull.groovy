...
class VL53L1XFull extends VL53L1XMinimal {
    ...
    private static final int[][] BUDGETS = [
        [15, 0x001D, 0x0027, 0, 0] as int[],
        [20, 0x0051, 0x006E, 0x001E, 0x0022] as int[],
        [33, 0x00D6, 0x006E, 0x0060, 0x006E] as int[],
        [50, 0x01AE, 0x01E8, 0x00AD, 0x00C6] as int[],
        [100, 0x02E1, 0x0388, 0x01CC, 0x01EA] as int[],
        [200, 0x03E1, 0x0496, 0x02D9, 0x02F8] as int[],
        [500, 0x0591, 0x05C1, 0x048F, 0x04A4] as int[],
    ] as int[][]
     ...
     */
    VL53L1XFull(Connection connection) throws IOException {
        super(connection)
    }
    ...
    public synchronized void startContinuous(int periodMs) throws IOException {
        ...
        int period = Math.max(Math.max(periodMs, timingBudget().intdiv(1000) as int), 1)
    ...
    public synchronized Measurement readMeasurement() throws IOException {
        readResult()
        return new Measurement(resultWord(13), rangeStatus, resultWord(15) / 128.0d, resultWord(7) / 128.0d,
                resultWord(3) / 256.0d)
    }
    ...
    public synchronized void setInterMeasurement(int periodMs) throws IOException {
        ...
        write32(REG_INTERMEASUREMENT_PERIOD, (clockPll * periodMs * 1075).intdiv(1000) as long)
    }
    ...
    public synchronized int interMeasurement() throws IOException {
        ...
        return ((read32(REG_INTERMEASUREMENT_PERIOD) * 1000).intdiv(clockPll * 1075)) as int
    }
    ...
    public synchronized void setSignalRateLimit(double limitMcps) throws IOException {
        if (limitMcps < 0 || limitMcps > 511.99d) {
            throw new IllegalArgumentException("signal rate limit must be 0 to 511.99 MCPS")
        }
        write16(REG_MIN_COUNT_RATE_RTN_LIMIT, (int) (limitMcps * 128 + 0.5d))
    }
    ...
    public synchronized double signalRateLimit() throws IOException {
        return read16(REG_MIN_COUNT_RATE_RTN_LIMIT) / 128.0d
    }
    ...
    public synchronized void setOffset(double offsetMm) throws IOException {
        if (offsetMm < -1024.0d || offsetMm > 1023.75d) {
            throw new IllegalArgumentException("offset must be -1024.0 to 1023.75 mm")
        }
        double q = offsetMm * 4
        int raw = q >= 0 ? (int) (q + 0.5d) : -((int) (-q + 0.5d))
    ...
    public synchronized void setCrosstalkCompensation(double rateMcps) throws IOException {
        ...
        write16(REG_XTALK_PLANE_OFFSET, Math.min((int) (rateMcps * 512000 + 0.5d), 0xFFFF))
    }
    ...
    public synchronized double crosstalkCompensation() throws IOException {
        return read16(REG_XTALK_PLANE_OFFSET) / 512000.0d
    }
    ...
    public synchronized double calibrateCrosstalk(int targetMm) throws IOException {
        ...
        for (byte[] r : collect()) {
            distance += word(r, 13)
            signal += word(r, 15) / 128.0d
            spads += word(r, 3) / 256.0d
        }
        double n = CALIBRATION_SAMPLES
        double meanDistance = distance / n
        double meanSignal = signal / n
        double meanSpads = spads / n
        double rate = meanSpads > 0 ? meanSignal * (1 - meanDistance / targetMm) / meanSpads : 0.0d
        rate = Math.min(Math.max(rate, 0.0d), 0.127d)
    ...
    public synchronized int[] interruptThresholds() throws IOException {
        return ([read16(REG_THRESH_LOW), read16(REG_THRESH_HIGH)] as int[])
    }
