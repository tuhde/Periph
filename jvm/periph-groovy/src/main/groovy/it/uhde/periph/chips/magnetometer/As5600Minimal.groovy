 ...
 * If MD=0, a {@link RuntimeException} is thrown — output data is invalid without a magnet.
 */
@CompileStatic
class As5600Minimal {
    ...
    boolean isMagnetDetected() {
        (readReg8(REG_STATUS) & STATUS_MD) != 0
    }
     ...
     */
    boolean isMagnetTooStrong() {
        (readReg8(REG_STATUS) & STATUS_MH) != 0
    }
     ...
     */
    boolean isMagnetTooWeak() {
        (readReg8(REG_STATUS) & STATUS_ML) != 0
    }
