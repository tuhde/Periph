 ...
 *
 * <p>Groovy keeps its own copy rather than extending the Java one (periph-groovy must not depend
 * on periph-java).
 */
@CompileStatic
abstract class VL53Base {
    ...
    /** Write one register. @param reg register index @param value byte value */
    protected void write8(int reg, int value) throws IOException {
        writeBlock(reg, [(byte) value] as byte[])
    }

    /** Read one register. @param reg register index @return unsigned byte value */
    protected int read8(int reg) throws IOException {
        return readBlock(reg, 1)[0] & 0xFF
    }

    /** Write a big-endian 16-bit register. @param reg register index @param value 16-bit value */
    protected void write16(int reg, int value) throws IOException {
        writeBlock(reg, [(byte) (value >> 8), (byte) value] as byte[])
    }

    /** Read a big-endian 16-bit register. @param reg register index @return unsigned value */
    protected int read16(int reg) throws IOException {
        byte[] b = readBlock(reg, 2)
        return ((b[0] & 0xFF) << 8) | (b[1] & 0xFF)
    }

    /** Write a big-endian 32-bit register. @param reg register index @param value 32-bit value */
    protected void write32(int reg, long value) throws IOException {
        writeBlock(reg, [(byte) (value >> 24), (byte) (value >> 16), (byte) (value >> 8), (byte) value] as byte[])
    }

    /** Read a big-endian 32-bit register. @param reg register index @return unsigned value */
    ...
    /**
     * Poll {@code condition} until it returns true.
     *
     ...
     */
    protected void waitUntil(Closure<Boolean> condition, String what) throws IOException {
    ...
    private void startPolling() {
        ...
        } as Runnable, chipName.toLowerCase() + "-poll")
