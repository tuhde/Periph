...
class Adxl345Full extends Adxl345Minimal {
    ...
    void setRange(int rangeG) throws IOException {
        int code
        switch (rangeG) {
            case 2:  code = 0; break
            case 4:  code = 1; break
    ...
    void setLowPower(boolean enabled) throws IOException {
        int bw = readReg(REG_BW_RATE)
        if (enabled) bw |= 0x10
        else         bw &= ~0x10
        writeReg(REG_BW_RATE, bw)
    ...
    void setOffset(double x, double y, double z) throws IOException {
    ...
    void calibrateOffset(double targetX, double targetY, double targetZ, int samples) throws IOException {
    ...
    void setTapDetection(double thresholdG, double durationMs, int axes = 0x07, boolean suppress = false) throws IOException {
    ...
    void setDoubleTap(double latencyMs, double windowMs) throws IOException {
    ...
    void setActivity(double thresholdG, int axes = 0x70, boolean acCoupled = true) throws IOException {
    ...
    void setInactivity(double thresholdG, double timeSec, int axes = 0x07, boolean acCoupled = false) throws IOException {
    ...
    void setFreeFall(double thresholdG, double timeMs) throws IOException {
    ...
    void setInterrupt(int source, boolean enabled, int pin = 1) throws IOException {
    ...
    int readInterruptSource() throws IOException { readReg(REG_INT_SOURCE) }

    /** Configure the FIFO. */
    void setFifoMode(int mode, int samples = 16) throws IOException {
    ...
    /** Drain the FIFO, returning up to {@code maxSamples} (x, y, z) samples in *g*. */
    double[][] readFifo(int maxSamples) throws IOException {
        int n = fifoCount()
    ...
    void setSleep(boolean enabled, int wakeupHz = 8) throws IOException {
    ...
    void setLinkMode(boolean enabled) throws IOException {
        int pwr = readReg(REG_POWER_CTL)
        if (enabled) pwr |= 0x40
        else         pwr &= ~0x40
        writeReg(REG_POWER_CTL, pwr)
    ...
    void setAutoSleep(boolean enabled) throws IOException {
        int pwr = readReg(REG_POWER_CTL)
        if (enabled) pwr |= 0x20
        else         pwr &= ~0x20
        writeReg(REG_POWER_CTL, pwr)
    ...
    void selfTest(boolean enabled) throws IOException {
        int df = readReg(REG_DATA_FORMAT)
        if (enabled) df |= 0x80
        else         df &= ~0x80
        writeReg(REG_DATA_FORMAT, df)
