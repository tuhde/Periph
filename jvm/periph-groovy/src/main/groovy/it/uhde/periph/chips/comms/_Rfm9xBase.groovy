...
abstract class _Rfm9xBase {
    ...
    void setTxPower(int powerDbm, boolean usePaBoost) {
        int p = powerDbm
        if (usePaBoost) {
            if (p > 17) {
                if (p > 20) p = 20
    ...
    void configure(int sf, float bandwidthKhz, int codingRate, boolean crc) {
    ...
    void standby() { writeReg(REG_OP_MODE, MODE_LONG_RANGE | bandFlag() | MODE_STANDBY) }
    void sleep()   { writeReg(REG_OP_MODE, MODE_LONG_RANGE | bandFlag() | MODE_SLEEP) }

    int version() { return readReg(REG_VERSION) }
    ...
    void stopReceive() { standby() }

    float rssi() { return -137.0f + readReg(REG_RSSI) }
    float lastPacketRssi() { return -137.0f + readReg(REG_PKT_RSSI) }
    float lastPacketSnr() {
