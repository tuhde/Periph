...
class Decoder(srd.Decoder):
    ...
    id = 'ad7705'
    name = 'AD7705'
    longname = 'Analog Devices AD7705 2-channel 16-bit sigma-delta ADC'
    desc = 'Decode AD7705 register-access SPI transactions.'
    license = 'gplv2+'
    inputs = ['spi']
    outputs = ['ad7705']
    tags = ['IC', 'ADC']
    ...
    def reset(self):
        ...
        self.last_gain     = {0: 1, 1: 1}
        self.last_bipolar  = {0: True, 1: True}
    ...
    def _finish_transaction(self):
        ...
        if self.reg_ptr == 1 and len(self.databuf) == 1:
            ...
            if self.channel is not None and self.channel < 2:
                self.last_gain[self.channel]    = 1 << gain_bits
                self.last_bipolar[self.channel] = (bu == 0)
            ...
            ch = self.channel if self.channel is not None and self.channel < 2 else 0
            gain = self.last_gain.get(ch, 1)
            bipolar = self.last_bipolar.get(ch, True)
            # vref unknown to the decoder; report raw code only.
            msg = (
                f'Data {CH_NAMES.get(ch, "?")}: code=0x{code:04X} ({code})',
                f'Data {CH_NAMES.get(ch, "?")}: 0x{code:04X}',
                f'D=0x{code:04X}',
            )
