...
READING_MAP = {25: ('DIFF', 10), 26: ('SUPPLY_DIFF', 40), 27: ('DIFF', 40)}
...
class Decoder(srd.Decoder):
    ...
    id = 'hx710b'
    name = 'HX710B'
    longname = 'HX710B 24-bit ADC'
    desc = ('Decode HX710B 2-wire bit-bang protocol (DOUT + PD_SCK) into '
            'signed 24-bit ADC conversions with reading type and rate.')
    license = 'gplv2+'
    inputs = ['logic']
    outputs = ['hx710b']
    tags = ['IC', 'ADC']
    ...
    def decode(self):
        if not self.samplerate:
            raise SamplerateError('HX710B decoder requires a sample rate.')
        ...
        while True:
            ...
            if pulse_count in READING_MAP:
                signed         = raw - 0x1000000 if raw >= 0x800000 else raw
                reading, rate  = current_reading
                next_reading, next_rate = READING_MAP[pulse_count]
                current_reading = (next_reading, next_rate)

                self.put(conv_ss, last_es, self.out_ann,
                         [ANN_CONV, [
                             '%s-%dHz: %d (0x%06X)  Next: %s-%dHz' % (
                                 reading, rate, signed, raw & 0xFFFFFF, next_reading, next_rate),
                             '%s-%dHz: %d  Next: %s-%dHz' % (
                                 reading, rate, signed, next_reading, next_rate),
                             '%s: %d' % (reading, signed),
                         ]])
