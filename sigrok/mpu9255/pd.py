...
class Decoder(srd.Decoder):
    ...
    id = 'mpu9255'
    name = 'MPU-9255'
    longname = 'MPU-9255 9-axis MotionTracking device'
    desc = 'Decode MPU-9255 I2C register transactions.'
    license = 'gplv2+'
    inputs = ['i2c']
    outputs = ['mpu9255']
    tags = ['IC', 'IMU', 'Sensor']
    ...
    def decode(self, ss, es, data):
        ...
        if ptype in ('START', 'START REPEAT'):
            ...
            if pdata not in MPU9255_ADDRS and pdata != AK8963_ADDR:
                self.state = 'IDLE'
                return
            self.addr = pdata
    ...
    def _finish_transaction(self):
        if self.state in ('GET_DATA_WRITE', 'GET_DATA_READ') and self.reg_ptr is not None:
            reg = self.reg_ptr
            name = AK8963_REGS.get(reg, MPU9255_REGS.get(reg, 'UNKNOWN'))
            if self.is_read:
    ...
    def _annotate_read(self, name, reg):
        ...
        if n >= 1:
            if name in ('ACCEL_XOUT_H', 'GYRO_XOUT_H') and n >= 6:
                ...
                val = self.databuf[0]
                if val != 0x73:
                    self.put(self.ss_block, self.es, self.out_ann, [ANN_WARNING,
    ...
    def _format_write(self, name, reg, val):
        ...
        elif name == 'INT_ENABLE':
            bits = []
            if val & 0x40: bits.append('WOM_EN')
            if val & 0x10: bits.append('FIFO_OFLOW_EN')
            if val & 0x08: bits.append('I2C_MST_INT_EN')
            if val & 0x01: bits.append('DATA_RDY_EN')
            return f'{name}: {", ".join(bits) or "none"}'
        elif name == 'MOT_DETECT_CTRL':
            intel = 'on' if val & 0x80 else 'off'
            mode = 'compare-to-prev' if val & 0x40 else 'fixed-threshold'
            return f'{name}: ACCEL_INTEL={intel}, MODE={mode}'
        elif name == 'LP_ACCEL_ODR':
            sel = val & 0x0F
            rate = LPOSC_TABLE[sel] if sel < len(LPOSC_TABLE) else '?'
            return f'{name}: LPOSC={sel} ({rate} Hz)'
        elif name == 'WOM_THR':
            mg = val * 4
            return f'{name}: {val} LSB (~{mg} mg)'
        elif name == 'PWR_MGMT_2':
            disabled = []
            if val & 0x07: disabled.append('gyro')
            if val & 0x38: disabled.append('accel')
            return f'{name}: DIS={", ".join(disabled) or "none"}'
