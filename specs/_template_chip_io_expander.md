...
### UIFlow 1
- [ ] Manifest `python/uiflow1/io_expander/<chip>/<chip>.json` — `Periph` category, `#a6bbcf` color
...
### UIFlow 2
- [ ] Wrapper class `python/uiflow2/io_expander/<chip>/<Chip>.py` — YAML docstrings per `python/uiflow2/UIFLOW2_BLOCKS.md`, `Periph` category, `#a6bbcf` color; one method for `init`, one method per other `Full`-class method wrapped (pin mode, read pin, write pin, ...), with a return annotation only on methods that return a value
