...
If this chip is register-addressed (I²C or SPI), accept `RegisterConnection` (not the bare
`Connection`) and call `connection.read(reg, length)` / `connection.write(reg, data)`
directly — no chip-local `_read_reg`/`_write_reg` and no `bus_type` branch. See
...
- [ ] Blocks `python/uiflow1/io_expander/<chip>/<chip>_*.py` — one execute block for `init`, one value/execute block per other `Full`-class method wrapped (pin mode, read pin, write pin, ...)
- [ ] Generated `python/uiflow1/io_expander/<chip>/<chip>.m5b` — run `python/uiflow1/generate.sh`, commit the output

### UIFlow 2
- [ ] Wrapper class `python/uiflow2/io_expander/<chip>/<Chip>.py` — YAML docstrings per `python/uiflow2/UIFLOW2_BLOCKS.md`, `Periph` category, `#C084FC` color; one method for `init`, one method per other `Full`-class method wrapped (pin mode, read pin, write pin, ...), with a return annotation only on methods that return a value
- [ ] Exported `python/uiflow2/io_expander/<chip>/<Chip>.m5b2` — built by hand in the UiFlow 2 web IDE's Block Designer (no generator — see `python/uiflow2/UIFLOW2_BLOCKS.md` § Workflow), commit the output alongside the wrapper class
