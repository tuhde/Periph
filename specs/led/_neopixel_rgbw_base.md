...
2. Construct `<Chip>Minimal(connection, n)` where `n` is the number of pixels. The constructor allocates an internal buffer of `n × 4` zero-bytes (all pixels off).
...
**Sensible defaults:** Buffer initialized to all zeros. No brightness scaling. `fill()` always calls the connection write — there is no separate show step. `w` defaults to `0` to allow RGB-only usage.
...
```
# User (r, g, b, w) → wire order (applied internally before connection.write())
...
- **Channel reorder:** apply `channel_order` mapping before handing bytes to the connection; the connection is order-agnostic. If `channel_order = (0, 1, 2, 3)` (identity), no reorder is needed — copy bytes directly.
...
- **Buffer ownership:** the chip driver owns the internal buffer in user (R, G, B, W) order. The connection receives the wire-ordered copy. Storing user values allows non-destructive brightness scaling.
...
- **Unit test mock:** NeoPixel chips are write-only (no registers, no reads), so no dedicated `neopixel_mock` is needed — reuse each language's existing generic write-recording mock: Python/C++/Node.js/JVM reuse `I2CConnectionMock`/`i2c_mock`/`MockConnection` (only the recorded `.writes` list is used); Go uses a small inline `mockConnection` implementing the `Connection` interface, colocated with the test per Go convention; Rust mocks at the `embedded-hal` SPI level with `embedded-hal-mock` and must reproduce `NeoPixelConnection`'s NZR bit-encoding (0→`0b100`, 1→`0b110` triplets, MSB-first, plus trailing reset zero-bytes) to compute the expected encoded SPI bytes, since the connection — not the chip driver — does that encoding. See `python/tests/led/sk6812rgbw_test_unit.py` / `rust/periph/src/chips/led/sk6812rgbw.rs` (`mod tests`) as the reference pattern for chips built on this base.
