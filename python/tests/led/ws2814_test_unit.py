...
# sk6812rgbw_test_unit.py) - the generic I2CConnectionMock works unchanged;
...
N = 3
RESET_BYTES = bytes(90)

connection = I2CConnectionMock()
sensor = WS2814Full(connection, N)
check_true('init', True)

# fill(): RGBW wire order (identity), all N pixels, transmits immediately,
# with the 90-byte extended reset tail appended.
sensor.fill(0x11, 0x22, 0x33, 0x44)
check_true('fill_transmits', len(connection.writes) == 1)
check_true('fill_length', len(connection.writes[-1]) == N * 4 + 90)
check_true('fill_rgbw_order', connection.writes[-1][0:4] == bytes([0x11, 0x22, 0x33, 0x44]))
check_true('fill_all_pixels', connection.writes[-1][:N * 4] == bytes([0x11, 0x22, 0x33, 0x44]) * N)
check_true('fill_reset_tail', connection.writes[-1][N * 4:] == RESET_BYTES)

# fill() white channel defaults to 0.
sensor.fill(0x10, 0x20, 0x30)
check_true('fill_white_defaults_zero', connection.writes[-1][0:4] == bytes([0x10, 0x20, 0x30, 0x00]))

# fill() clamps out-of-range channels.
sensor.fill(-10, 300, 128, 999)
check_true('fill_clamps', connection.writes[-1][0:4] == bytes([0, 255, 128, 255]))
...
sensor.show()
check_true('set_pixel_then_show', connection.writes[-1][4:8] == bytes([0xAA, 0xBB, 0xCC, 0xDD]))
check_true('set_pixel_other_pixels_unchanged', connection.writes[-1][0:4] == bytes(4))

sensor.set_pixel(0, 0x01, 0x02, 0x03)
sensor.show()
check_true('set_pixel_white_defaults_zero', connection.writes[-1][0:4] == bytes([0x01, 0x02, 0x03, 0x00]))

# set_pixel() clamps index to [0, n-1].
sensor.set_pixel(99, 0x05, 0x06, 0x07, 0x08)
sensor.show()
check_true('set_pixel_clamps_index', connection.writes[-1][(N - 1) * 4:(N - 1) * 4 + 4] == bytes([0x05, 0x06, 0x07, 0x08]))
...
sensor.show()
scaled = tuple(v * 128 // 255 for v in (200, 100, 50, 40))  # stored as R,G,B,W (identity)
...
sensor.show()
check_true('rotate_shifts_left', connection.writes[-1][0] == 2 and connection.writes[-1][2 * 4] == 1)

# fill_hsv(): pure red (h=0, s=1, v=1) -> RGB (255, 0, 0), white=0.
sensor.fill_hsv(0.0, 1.0, 1.0)
check_true('fill_hsv_red', connection.writes[-1][0:4] == bytes([255, 0, 0, 0]))
