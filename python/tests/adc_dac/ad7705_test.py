...
spi = SPI(cfg.SPI_ID, baudrate=cfg.FREQ, polarity=1, phase=1, sck=Pin(cfg.SCK), mosi=Pin(cfg.MOSI), miso=Pin(cfg.MISO))
cs = Pin(cfg.CS, Pin.OUT, value=1)
conn = SPIConnection(spi, cs)

adc_min = AD7705Minimal(conn, cfg.VREF, cfg.MCLK_HZ)
adc = AD7705Full(conn, cfg.VREF, cfg.MCLK_HZ, reset_pin=None)
