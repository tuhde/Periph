...
from periph.chips.adc_dac.ad7705 import AD7705Full, AD7705Minimal
from periph.connection.spi_linux import SPIConnection

SPI_BUS  = int(os.environ.get('LINUX_SPI_BUS', '0'))
SPI_DEV  = int(os.environ.get('LINUX_SPI_DEV', '0'))
VREF     = float(os.environ.get('VREF', '2.5'))
MCLK_HZ  = int(os.environ.get('MCLK_HZ', '2457600'))
...
conn = SPIConnection(SPI_BUS, SPI_DEV, mode=3, max_speed_hz=1_000_000)

adc_min = AD7705Minimal(conn, VREF, MCLK_HZ)
adc = AD7705Full(conn, VREF, MCLK_HZ, reset_pin=None)
...
check_true('Minimal.read_voltage in [-VREF, +VREF]', -VREF <= v <= VREF)
