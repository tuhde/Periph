...
from periph.chips.imu.mpu9250 import MPU9250Full, MPU9250Minimal
...
# so it needs its own connection - separate from the MPU-9250's own,
# mirroring how the real driver is wired (see MPU9250Full's docstring).
connection = I2CConnectionMock()
connection.set_register(MPU9250Full._REG_WHO_AM_I, 0x71)
mag_connection = I2CConnectionMock()

sensor = MPU9250Full(connection, mag_connection)
...
expected_init_writes = [
    bytes([MPU9250Full._REG_PWR_MGMT_1, 0x80]),
    bytes([MPU9250Full._REG_PWR_MGMT_1, 0x01]),
...
connection.set_register(MPU9250Full._REG_ACCEL_XOUT_H, *s16(16384), *s16(-8192), *s16(4096))
...
connection.set_register(MPU9250Full._REG_GYRO_XOUT_H, *s16(131), *s16(-131), *s16(262))
...
check_true('configure_gyro_writes', connection.writes[-1] == bytes([MPU9250Full._REG_GYRO_CONFIG, 2 << 3]))
# Sensitivity for FS_SEL=2 is 32.8 LSB/(deg/s); raw=328 -> 10 dps.
connection.set_register(MPU9250Full._REG_GYRO_XOUT_H, *s16(328), *s16(0), *s16(0))
gx2, _, _ = sensor.gyro()
check_true('configure_gyro_changes_sensitivity', abs(gx2 - math.radians(10)) < 1e-9)

sensor.configure_accel(1)
check_true('configure_accel_writes', connection.writes[-1] == bytes([MPU9250Full._REG_ACCEL_CONFIG, 1 << 3]))
# Sensitivity for AFS_SEL=1 is 8192 LSB/g; raw=8192 -> 1g.
connection.set_register(MPU9250Full._REG_ACCEL_XOUT_H, *s16(8192), *s16(0), *s16(0))
...
check_true('configure_dlpf', connection.writes[-2:] == [
    bytes([MPU9250Full._REG_CONFIG, 5]),
    bytes([MPU9250Full._REG_ACCEL_CONFIG2, 2]),
])

sensor.configure_sample_rate(9)
check_true('configure_sample_rate', connection.writes[-1] == bytes([MPU9250Full._REG_SMPLRT_DIV, 9]))

# temperature(): raw=340 -> 340/333.87 + 21.0.
connection.set_register(MPU9250Full._REG_TEMP_OUT_H, *s16(340))
check_true('temperature', abs(sensor.temperature() - (340 / 333.87 + 21.0)) < 1e-9)

# accel_raw() / gyro_raw()
connection.set_register(MPU9250Full._REG_ACCEL_XOUT_H, *s16(100), *s16(-200), *s16(300))
check_true('accel_raw', sensor.accel_raw() == (100, -200, 300))
connection.set_register(MPU9250Full._REG_GYRO_XOUT_H, *s16(-50), *s16(60), *s16(-70))
check_true('gyro_raw', sensor.gyro_raw() == (-50, 60, -70))

# data_ready()
connection.set_register(MPU9250Full._REG_INT_STATUS, 0x01)
check_true('data_ready_true', sensor.data_ready() is True)
connection.set_register(MPU9250Full._REG_INT_STATUS, 0x00)
...
check_true('set_sleep_true', connection.writes[-1] == bytes([MPU9250Full._REG_PWR_MGMT_1, 0x41]))
sensor.set_sleep(False)
check_true('set_sleep_false', connection.writes[-1] == bytes([MPU9250Full._REG_PWR_MGMT_1, 0x01]))

# fifo_count()
connection.set_register(MPU9250Full._REG_FIFO_COUNTH, 0x03, 0x45)
check_true('fifo_count', sensor.fifo_count() == ((0x03 & 0x1F) << 8) | 0x45)

# read_fifo()
connection.set_register(MPU9250Full._REG_FIFO_COUNTH, 0x00, 0x02)
connection.set_register(MPU9250Full._REG_FIFO_R_W, 0xAA, 0xBB)
check_true('read_fifo', sensor.read_fifo() == bytes([0xAA, 0xBB]))

connection.set_register(MPU9250Full._REG_FIFO_COUNTH, 0x00, 0x00)
...
fifo_en_write = bytes([MPU9250Full._REG_FIFO_EN, (1 << 3) | (1 << 4)])
user_ctrl_read = bytes([MPU9250Full._REG_USER_CTRL])
user_ctrl_write = bytes([MPU9250Full._REG_USER_CTRL, 0x40])  # USER_CTRL was 0, OR 0x40
...
sensor.reset_fifo()
check_true('reset_fifo', connection.writes[-1] == bytes([MPU9250Full._REG_USER_CTRL, 0x44]))
...
mag_connection.set_register(MPU9250Full._AK8963_REG_ASAX, 200)
mag_connection.set_register(MPU9250Full._AK8963_REG_ASAY, 100)
mag_connection.set_register(MPU9250Full._AK8963_REG_ASAZ, 50)
sensor.enable_mag()
check_true('enable_mag_int_pin_cfg_write', connection.writes[-1] == bytes([MPU9250Full._REG_INT_PIN_CFG, 0x22]))
expected_mag_init_writes = [
    bytes([MPU9250Full._AK8963_REG_CNTL1, 0x00]),
    bytes([MPU9250Full._AK8963_REG_CNTL1, 0x0F]),
    bytes([MPU9250Full._AK8963_REG_ASAX]),
    bytes([MPU9250Full._AK8963_REG_ASAY]),
    bytes([MPU9250Full._AK8963_REG_ASAZ]),
    bytes([MPU9250Full._AK8963_REG_CNTL1, 0x00]),
    bytes([MPU9250Full._AK8963_REG_CNTL1, 0x16]),  # 16-bit | mode=6
]
...
mag_connection.set_register(MPU9250Full._AK8963_REG_HXL,
...
unenabled_connection.set_register(MPU9250Full._REG_WHO_AM_I, 0x71)
unenabled_sensor = MPU9250Full(unenabled_connection, I2CConnectionMock())
try:
...
bad_connection.set_register(MPU9250Minimal._REG_WHO_AM_I, 0x00)
try:
    MPU9250Minimal(bad_connection)
