...
class DHTxxError(Exception):
    ...
    The connection distinguishes two failure modes:
...
class DHTxxConnection:
    """DHTxx single-wire connection for CircuitPython (wraps a digitalio.DigitalInOut).
    ...
    The connection switches the pin's direction as needed to drive the start
    ...
    Args:
        data_pin: digitalio.DigitalInOut instance. The connection reconfigures
    ...
    def read(self):
        ...
        Returns:
            bytes: Exactly 5 bytes — [hum_int, hum_dec, temp_int, temp_dec, checksum],
                or 5 zero bytes if this connection is disabled.
