...
class MPU9255Minimal:
    ...
    def __init__(self, connection):
        ...
        if who != self._WHO_AM_I_VALUE:
            raise ValueError('MPU9255 WHO_AM_I: expected 0x{:02X}, got 0x{:02X}'.format(
...
class MPU9255Full(MPU9255Minimal):
    ...
    Args:
        connection: Configured I²C or SPI connection pointing at the MPU-9255.
    ...
    def configure_wake_on_motion(self, threshold_mg=64, odr_hz=31.25):
        ...
        Args:
            threshold_mg: Motion threshold in milligrams (4–1020 mg, quantized
                to 4 mg steps). Values outside this range are clamped.
            odr_hz: Wake-up output data rate in Hz (0.24–500 Hz). Mapped to
                the LP_ACCEL_ODR register's Lposc_clksel field via the chip's
                standard 16-entry table.
        """
        threshold_lsb = int(round(threshold_mg / 4.0))
        if threshold_lsb < 1:
            threshold_lsb = 1
        if threshold_lsb > 255:
            threshold_lsb = 255
        ...
        best_sel = 0
        best_diff = abs(odr_hz - lposc_table[0])
        for sel, rate in enumerate(lposc_table):
            diff = abs(odr_hz - rate)
            if diff < best_diff:
                best_diff = diff
                best_sel = sel
        ...
        self._write_reg(self._REG_LP_ACCEL_ODR, best_sel & 0x0F)
        self._write_reg(self._REG_PWR_MGMT_1, 0x21)

    def motion_detected(self):
        """Check if a wake-on-motion interrupt has fired.

        Returns:
            bool: True when WOM_INT (bit 6) is set in INT_STATUS. Reading
            INT_STATUS clears the interrupt.
        """
        return bool(self._read_reg(self._REG_INT_STATUS) & 0x40)