class AD7706Minimal:
    """AD7706 3-channel, 16-bit sigma-delta ADC — minimal interface.

    Reads calibrated voltage from Channel 1 (AIN1 relative to COMMON) with
    sensible defaults. The driver uses the two-phase register-access protocol
    (Communication Register write followed by the data transfer within a
    single CS assertion) and polls `0/DRDY` over SPI rather than requiring
    a dedicated DRDY GPIO.
    ...
    def __init__(self, connection, vref, mclk_hz, reset_pin=None):
        """Initialize AD7706Minimal: configure Clock and Setup Registers and self-calibrate.
    ...
    def read_voltage(self):
        ...
        (the Minimal default). Use AD7706Full for arbitrary gain/bipolar.
...
class AD7706Full(AD7706Minimal):
    """AD7706 full interface — extends AD7706Minimal with full chip functionality.

    Adds per-channel configuration (Channel 1 = AIN1/COMMON, Channel 2 =
    AIN2/COMMON, Channel 3 = AIN3/COMMON), all eight PGA gains,
    unipolar/bipolar selection, buffered/unbuffered analog inputs, all
    four output rates, self/system calibration, direct access to the
    24-bit calibration coefficients, standby/wakeup power control, and
    ...
    def configure(self, channel=1, gain=1, bipolar=True, buffered=False, output_rate_hz=50):
        ...
        Args:
            channel: 1, 2, or 3 (AIN1, AIN2, AIN3 — all relative to COMMON).
    ...
    def _validate_channel(self, channel):
        ...
        Returns:
            int: _CH1, _CH2, or _CH3.

        Raises:
            ValueError: If channel is not 1, 2, or 3.
        """
        if channel == 1:
            return self._CH1
        if channel == 2:
    ...
    def read_raw_channel3(self):
        """Read the raw 16-bit Data Register code on Channel 3 (AIN3 vs COMMON).
