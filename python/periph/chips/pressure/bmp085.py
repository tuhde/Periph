...
class BMP085Minimal:
    """BMP085 piezo-resistive pressure + temperature sensor — minimal interface.

    Provides calibrated temperature (°C) and pressure (Pa) readings with no
    configuration beyond the connection. The BMP085 has a fixed I²C address (0x77)
    and no programmable address pin; only one BMP085 can exist on a single bus.
...
class BMP085Full(BMP085Minimal):
    """BMP085 full interface — extends BMP085Minimal with oversampling control and altitude helpers.
    ...
    def altitude(self, sea_level_pa=101325.0):
        ...
        """
        p = self.pressure()
        return 44330 * (1 - (p / sea_level_pa) ** (1 / 5.255))
    ...
    def sea_level_pressure(self, altitude_m):
        ...
        Returns:
            float: Sea-level pressure in Pa.
        """
        p = self.pressure()
