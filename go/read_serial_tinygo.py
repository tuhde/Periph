#!/usr/bin/env python3
"""Read serial output from a Raspberry Pi Pico W after flashing.

Usage: read_serial_tinygo.py <port> <baud> <timeout_secs>

Waits for the USB CDC device to (re-)enumerate after a UF2 flash
reboot, then reads lines until ===DONE: ... === appears or the
timeout expires. Exits 0 on full pass, 1 on failures, 2 on timeout.
"""
import glob
import os
import sys
...
def wait_for_port(hint, total_deadline):
    # First let the boot ROM reset disconnect the port
    time.sleep(1.0)
