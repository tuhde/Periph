...
// BME280 complete example — TinyGo / Raspberry Pi Pico W.
//
// Exercises every method in the BME280Full API on the Pico W: configuration,
// status, chip ID, individual reads, altitude and dew-point helpers, and reset.
package main
...
func main() {
	...
	_, err = chip.DewPoint() // Compute dew point, () → (float32 °C, error)
	if err != nil {
		panic(err)
	}
	...
	fmt.Printf("T=%.1f C, P=%.1f hPa, RH=%.1f %%RH, alt=%.1f m\n", t, p, h, alt)
}
