...
// VL53L1X demo — TinyGo (Raspberry Pi Pico W). Long-range doorway people counter with a split
...
func main() {
	...
	println("optical centre SPAD", oc, "zone centres", zoneCentres[0], zoneCentres[1])
	...
	println("baseline left (mm)", int32(baseline[0]), "right (mm)", int32(baseline[1]))
	...
	for events < maxEvents && time.Since(start) < maxTime {
		...
		if !occupied[0] && !occupied[1] && len(sequence) > 0 {
			...
			events++
			println("IN", countIn, "OUT", countOut, "left (mm)", readings[0], "right (mm)", readings[1])
			sequence = sequence[:0]
		}
