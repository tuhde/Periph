...
// VL53L1X complete example — TinyGo (Raspberry Pi Pico W). Exercises every method in the Full
...
func main() {
	...
	conn := connection.NewI2CConnection(machine.I2C0, tof.VL53L1XI2CAddress, nil, nil) // Create I2C connection, (i2c, addr=0x29) → *I2CConnection
	defer conn.Close()
	...
	// mask revision, 0x10
	println("model", id, "module", mt, "revision", rev)

	d, err := s.Distance() // Measure distance, () → (uint16 mm, error)
	// single shot; blocks for about one timing budget
	must(err)
	println("distance", d, "mm, valid", s.RangeValid()) // Check last measurement, () → bool
	// mapped range status == 0
	println("range status", s.RangeStatus()) // Read last range status, () → uint8
	...
	must(err)
	println("long mode", mode == tof.VL53L1XDistanceModeLong)
	...
	must(err)
	println("record", m.DistanceMM, "mm")
	...
	// 0 = off
	println("crosstalk (uMCPS)", int32(xt*1e6))
	...
	must(err)
	println("calibrated offset (0.01 mm)", int32(calOffset*100), "crosstalk (uMCPS)", int32(calXtalk*1e6))
