...
// HX710B complete example — TinyGo / Raspberry Pi Pico W.
//
// Wires HX710B DOUT to GP2 and PD_SCK to GP3. Exercises every method
// in the HX710BFull API: raw and averaged reads, output rate
// selection, tare, scale factor, weight reading, supply-difference
...
func main() {
	...
	if err := hx.SetRate(adcdac.HX710BRate40SPS); err != nil { // Select 40 SPS, (rate=40) → error
		fmt.Printf("set_rate 40: %v\n", err)
	}
	if err := hx.SetRate(adcdac.HX710BRate10SPS); err != nil { // Select 10 SPS, (rate=10) → error
	...
	suppRaw, err := hx.ReadSupplyDiffRaw() // Read raw DVDD−AVDD supply-difference code, () → (int32, error)
	if err != nil {
		fmt.Printf("read_supply_diff_raw: %v\n", err)
		return
	}
	fmt.Printf("read_supply_diff_raw: %d\n", suppRaw)
