...
// AD7706 complete example — TinyGo target.
package main
...
func main() {
	...
	if err := chip.Configure(3, adcdac.GAIN8, true, true, 60); err != nil {        // Configure channel 3, (channel=3, gain=GAIN8, bipolar=true, buffered=true, output_rate_hz=60) → error
		fmt.Println("cfg3:", err)
		return
	}
	if err := chip.SelfCalibrate(3); err != nil {                                  // Self-calibrate channel, (channel=3) → error
		fmt.Println("cal3:", err)
		return
	}
	...
	v3, err := chip.ReadVoltageChannel(3)                                          // Read voltage, (channel=3) → (float64, error)
	if err != nil {
		fmt.Println("v3:", err)
		return
	}
	fmt.Printf("ch1 raw=%d ch1 v=%.4f ch2 v=%.4f ch3 v=%.4f\n", raw1, v1, v2, v3)
