...
// MFRC522 demo example — Linux host.
//
...
func main() {
	...
	if err != nil {
		...
		if ok, err := chip.WriteBlock(4, seed); err != nil || !ok { // Write MIFARE block, (block, data) → (bool, error)
			fmt.Fprintln(os.Stderr, "write_block:", err)
			os.Exit(1)
		}
		if ok, err := chip.RestoreValue(4); err != nil || !ok { // Restore + transfer, (block) → (bool, error)
			fmt.Fprintln(os.Stderr, "restore_value:", err)
			os.Exit(1)
		}
		value = 10
	}
	...
	for i := 0; i < 20; i++ {
		...
		if err != nil || !ok {
			fmt.Fprintln(os.Stderr, "decrement_value:", err)
			break
		}
		block, err = chip.ReadBlock(4) // Read MIFARE block, (block_address) → ([]byte, error)
		if err != nil {
