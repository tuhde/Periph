...
// MPU9255 complete example — Linux host.
//
// Exercises every method in the MPU9255Full API: configuration,
// temperature, raw readings, data-ready polling, magnetometer enable
// and read, sleep control, FIFO management, and wake-on-motion.
package main
...
func main() {
	...
	// The MPU9255 exposes the on-board AK8963 magnetometer at I2C
	...
	// disables gyro, sets 184 Hz accel DLPF, arms WOM_THR/INT_ENABLE,
	// enables CYCLE mode at 31.25 Hz wake-up rate
	motion, err := chip.MotionDetected() // Check motion interrupt, () → (bool, error)
	if err != nil {
		panic(err)
	}
	fmt.Printf("motion_detected: %v\n", motion) // reads WOM_INT bit from INT_STATUS
}
