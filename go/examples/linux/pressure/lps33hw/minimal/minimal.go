...
// LPS33HW minimal example — Linux host.
//
// Constructs the driver on /dev/i2c-N, then reads temperature and
// pressure in a loop using the chip's 1 Hz default configuration.
package main
...
func main() {
	...
	chip, err := pressure.NewLPS33HWMinimal(conn, uint8(addr)) // Create LPS33HW driver, (connection, addr=0x5C) → (*LPS33HWMinimal, error)
	if err != nil {
		panic(err)
	}
