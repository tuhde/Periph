...
// ENS160 complete example — Linux host.
//
...
func main() {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "read_air_quality: %v\n", err)
	} else {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "read_tvoc: %v\n", err)
	} else {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "read_eco2: %v\n", err)
	} else {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "read_aqi: %v\n", err)
	} else {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "read_ethanol: %v\n", err)
	} else {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "read_raw_resistance(1): %v\n", err)
	} else {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "read_raw_resistance(4): %v\n", err)
	} else {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "read_compensation_actuals: %v\n", err)
	} else {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "get_firmware_version: %v\n", err)
	} else {
