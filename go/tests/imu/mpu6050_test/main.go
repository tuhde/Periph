...
// MPU6050 hardware test — Linux host.
//
// Reads from /dev/i2c-N and runs the MPU6050 check sequence. Prints
...
func main() {
	...
	chip, err := imu.NewMPU6050Full(tr)
	if err != nil {
	...
	if err := chip.SetStandby(false, false, false, false, false, false); err != nil {
		fmt.Fprintln(os.Stderr, "set_standby:", err)
	}
