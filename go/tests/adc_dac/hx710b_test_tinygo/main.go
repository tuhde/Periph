...
// HX710B hardware test — TinyGo / Raspberry Pi Pico W.
//
// Flashed to a Pico W with HX710B DOUT on GP2 and PD_SCK on GP3.
// Runs the HX710B check sequence: smoke test for both Minimal and
...
func main() {
	...
	// --- HX710BMinimal ---
	{
		hx, err := adcdac.NewHX710BMinimal(conn) // Create HX710B driver, (connection) → (*HX710BMinimal, error)
	...
	// --- HX710BFull ---
	{
		...
		check("set_rate_40_ok", hx.SetRate(adcdac.HX710BRate40SPS) == nil) // Select rate, (rate=40) → error
