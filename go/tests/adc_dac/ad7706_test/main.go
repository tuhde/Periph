...
// AD7706 hardware test — Linux host.
package main
...
func main() {
	...
	chip, err := adcdac.NewAD7706Minimal(conn, 2.5, adcdac.MCLK2_4576MHz, nil)
	if err != nil {
	...
	chip2, err := adcdac.NewAD7706Full(conn, 2.5, adcdac.MCLK2_4576MHz, nil)
	if err != nil {
	...
	raw3, err := chip2.ReadRawChannel(3)
	check("readRawChannel(3) in [0, 65535]", err == nil && raw3 <= 65535)
	v3, err := chip2.ReadVoltageChannel(3)
	check("readVoltageChannel(3) returns float", err == nil)
	check("readVoltageChannel(3) in [-2.5, 2.5]", err == nil && v3 >= -2.5 && v3 <= 2.5)
	...
	err = chip2.Configure(3, adcdac.GAIN4, false, true, 60)
	check("configure(3, GAIN4) accepted", err == nil)
	...
	err = chip2.SelfCalibrate(3)
	check("selfCalibrate(3) accepted", err == nil)
	...
	off3, err := chip2.GetOffsetCalibration(3)
	check("getOffsetCalibration(3) in [0, 2^24-1]", err == nil && off3 <= 0xFFFFFF)
	gain3, err := chip2.GetGainCalibration(3)
	check("getGainCalibration(3) in [0, 2^24-1]", err == nil && gain3 <= 0xFFFFFF)
