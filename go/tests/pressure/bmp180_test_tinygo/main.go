...
// BMP180 hardware test — TinyGo / Raspberry Pi Pico W.
//
// Flashed to a Pico W connected to a BMP180 on I2C1 (GP4 = SDA, GP5 = SCL).
...
func main() {
	...
	conn := connection.NewI2CConnection(i2c, 0x77, nil, nil)
	chip, err := pressure.NewBmp180Full(conn)
	if err != nil {
	...
	check("default_oss", chip.Oversampling() == pressure.OssUlp)
	chip.SetOversampling(pressure.OssHighRes)
	check("set_oss", chip.Oversampling() == pressure.OssHighRes)

	alt, err := chip.AltitudeAt(1013.25)
